...
For $\gamma_{n, T} =?? $, where $C$ is a universal constant that does not depend on $n$ and $T$, we have
...
\end{align*}
Without loss of generality, we can regard the covariates $\mathbf{X}_{it}$ to be scalars $X_{it}$, not vectors. The rest of the proof in case of vectors proceeds analogously.
...
Now consider the term $\big| \sum_{t=1}^T w_{t,T}(u,h) \mathbf{X}_{it}  \big|$. 
%Note that by construction the weights $w_{t, T}(u, h)$ are not equal to $0$ if and only if $T(u-h) \le t \le T(u+h)$. We can use this fact to rewrite
%\begin{align*}
...
&= \sum_{t=1}^T\frac{\frac{1}{Th} K^2\Big(\frac{\frac{t}{T}-u}{h}\Big) \Big[ S_{T,2}(u,h) - \Big(\frac{\frac{t}{T}-u}{h}\Big) S_{T,1}(u,h) \Big]^2}{ \bigg\{\frac{1}{Th}\sum\nolimits_{s=1}^T  K^2\Big(\frac{\frac{s}{T}-u}{h}\Big) \Big[ S_{T,2}(u,h) - \Big(\frac{\frac{s}{T}-u}{h}\Big) S_{T,1}(u,h) \Big]^2 \bigg\} } =\\
&= 1
\end{align*}
and
\begin{align*}
\sum_{t=\lfloor T(u-h) \rfloor}^{\lceil T(u+h) \rceil} \big(\sqrt{2Th + 2}\cdot w_{t,T}(u,h)\big)^2 = 2Th+2.
\end{align*}
\textcolor{red}{
\begin{enumerate}[label=(C\arabic*),leftmargin=1.05cm]
\setcounter{enumi}{12}
	\item \label{C-reg4}  Additional assumption on the dependence adjusted norm here!
\end{enumerate}}
\begin{theorem}{\cite{Wu2016}}
...
\end{theorem}
Now according to Theorem 2 in \cite{Wu2016}, we can bound the single probability as follows
\begin{align*}
\pr&\left(\Big| \sum_{t=\lfloor T(u-h) \rfloor}^{\lceil T(u+h) \rceil} \sqrt{2Th + 2}\cdot w_{t,T}(u,h)X_{it}  \Big| \geq \sqrt{(2Th + 2)T}\cdot x\right) \leq\\
&\leq C_1 \frac{\big( \sum_{t=\lfloor T(u-h) \rfloor}^{\lceil T(u+h) \rceil} |\sqrt{2Th + 2}\cdot w_{t,T}(u,h)|^{q^\prime}\big) ||X_{i\cdot}||^{q^\prime}_{q^\prime, \alpha}}{(\sqrt{(2Th + 2)T}\cdot x)^{q^\prime}} + C_2 \exp \left(-\frac{C_3 (2Th + 2)T x^2}{(2Th + 2)||X_{i\cdot}||^{2}_{2, \alpha}}\right) = \\
&= C_1 \frac{\big( \sum_{t=\lfloor T(u-h) \rfloor}^{\lceil T(u+h) \rceil} |w_{t,T}(u,h)|^{q^\prime}\big) ||X_{i\cdot}||^{q^\prime}_{q^\prime, \alpha}}{T^{q^\prime/2}\cdot x^{q^\prime}} + C_2 \exp \left(-\frac{C_3 T x^2}{||X_{i\cdot}||^{2}_{2, \alpha}}\right) 
\end{align*}

Now take any $\epsilon>0$, $\delta>0$:
\begin{align*}
...
&\overset{\text{``normalisation''}}{=} \sum_{(u, h) \in \mathcal{G}_T} \pr \left( \Big| \sum_{t=\lfloor T(u-h) \rfloor}^{\lceil T(u+h) \rceil} \sqrt{2Th + 2}\cdot w_{t,T}(u,h)X_{it}  \Big| \geq \sqrt{(2Th + 2)T}\cdot \delta \right) \leq \\
&\overset{\text{Wu's Theorem}}{\leq} \sum_{(u, h) \in \mathcal{G}_T} \left[C_1 \frac{\big( \sum_{t=\lfloor T(u-h) \rfloor}^{\lceil T(u+h) \rceil} |w_{t,T}(u,h)|^{q^\prime}\big) ||X_{i\cdot}||^{q^\prime}_{q^\prime, \alpha}}{T^{q^\prime/2}\cdot \delta^{q^\prime}} + C_2 \exp \left(-\frac{C_3 T \delta^2}{||X_{i\cdot}||^{2}_{2, \alpha}}\right) \right] \leq\\
&\leq C_1 \frac{ T^\theta ||X_{i\cdot}||^{q^\prime}_{q^\prime, \alpha}}{T^{q^\prime/2}\cdot \delta^{q^\prime}} \max_{(u, h) \in \mathcal{G}_T} \left( \sum_{t=\lfloor T(u-h) \rfloor}^{\lceil T(u+h) \rceil} |w_{t,T}(u,h)|^{q^\prime}\right)+ C_2 T^\theta \exp \left(-\frac{C_3 T \delta^2}{||X_{i\cdot}||^{2}_{2, \alpha}}\right) = \\
...
\end{enumerate}
If we take $\theta - q^\prime/2 <0$, then the term on the RHS can be arbitrarily small for any fixed $\delta >0$ and for large enough $T$. Hence, 
\begin{align*}
\max_{(u, h) \in \mathcal{G}_T} \Big| \sum_{t=\lfloor T(u-h) \rfloor}^{\lceil T(u+h) \rceil} w_{t,T}(u,h)X_{it}  \Big| = o_P(\sqrt{T}).
\end{align*}
