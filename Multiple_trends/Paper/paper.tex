...
\begin{table}[h!]
  \begin{center}
    \caption{All the statistics used in the proofs}
    \label{tab:app-table1}
    \begin{tabular}{l|l|l} 
      \textbf{Statistic} & \textbf{Notation} & \textbf{Formula}\\
      \hline
      Multiscale statistics & $ \widehat{\Psi}_{n,T}$ & $\begin{array} {l@{}l@{}} \widehat{\Psi}_{n,T}  &{}= \max_{1 \le i < j \le n} \widehat{\Psi}_{ij,T}\\
      \widehat{\Psi}_{ij,T} & {}= \max_{(u,h) \in \mathcal{G}_T} \Big\{ \Big|\frac{\widehat{\psi}_{ij,T}(u,h)}{(\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2)^{1/2}}\Big| - \lambda(h) \Big\}\end{array}$ \\
	\hline
	``Gaussian'' statistic & $ \Phi_{n, T}$ & $\begin{array} {l@{}l@{}}       \Phi_{n, T} &{}= \max_{1 \le i < j \le n} \Phi_{ij,T},\\
        \Phi_{ij,T}  & {}= \max_{(u,h) \in \mathcal{G}_T} \Big\{ \Big| \frac{\phi_{ij,T}(u,h)} {\{ \widehat{\sigma}_i^2 + \widehat{\sigma}_j^2 \}^{1/2}} \Big| - \lambda(h) \Big \} \end{array}$  \\
        	\hline
	True Gaussian statistic & $ \Phi^\prime_{n, T}$ & $\begin{array} {l@{}l@{}}       \Phi^\prime_{n, T} &{}= \max_{1 \le i < j \le n} \Phi^\prime_{ij,T},\\
        \Phi^\prime_{ij,T}  & {}= \max_{(u,h) \in \mathcal{G}_T} \Big\{ \Big| \frac{\phi^\prime_{ij,T}(u,h)} {\{ \sigma_i^2 + \sigma_j^2 \}^{1/2}} \Big| - \lambda(h) \Big \} \end{array}$  \\
      \hline
      Auxiliary statistic & $ \widehat{\Phi}_{n, T}$ & $\begin{array} {l@{}l@{}}        \widehat{\Phi}_{n, T} &{}= \max_{1 \le i < j \le n} \widehat{\Phi}_{ij,T},\\
        \widehat{\Phi}_{ij,T}  & {}= \max_{(u,h) \in \mathcal{G}_T} \Big\{ \Big| \frac{\widehat{\phi}_{ij,T}(u,h)} {\{ \widehat{\sigma}_i^2 + \widehat{\sigma}_j^2 \}^{1/2}} \Big| - \lambda(h) \Big \} \end{array}$ 
      \\
      \hline
      \begin{tabular}[c]{@{}l@{}}
      Statistics that is identically\\
      distributed as $ \widehat{\Phi}_{n, T}$
      \end{tabular} & $\widetilde{\Phi}_{n, T}$ & \\
      \hline
      Intermediate statistic & $ \doublehattwo{\Phi}_{n, T}$ & $\begin{array} {l@{}l@{}} 
\doublehattwo{\Phi}_{n,T} &{}= \max_{1 \le i < j \le n} \doublehattwo{\Phi}_{ij,T}, \\
        \doublehattwo{\Phi}_{ij,T} &{}= \max_{(u,h) \in \mathcal{G}_T} \big\{ \Big|\frac{\doublehat{\phi}_{ij,T}(u,h)}{(\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2)^{1/2}}\Big| - \lambda(h) \Big\} \end{array}$ 
      \\     
      \hline
    \end{tabular}
  \end{center}
\end{table}
The kernel averages are defined as follows:
\begin{align*}
      &\widehat{\psi}_{ij,T}(u,h) = \sum\limits_{t=1}^T w_{t,T}(u,h)(\widehat{Y}_{it} - \widehat{Y}_{jt}),\\
      &\phi_{ij,T}(u,h) = \sum\nolimits_{t=1}^T w_{t,T}(u,h) \, \big\{ \widehat{\sigma}_i (Z_{it} - \bar{Z}_i) - \widehat{\sigma}_j (Z_{jt} - \bar{Z}_j) \big\},\\
      &\phi^\prime_{ij,T}(u,h) = \sum\nolimits_{t=1}^T w_{t,T}(u,h) \, \big\{ \sigma_i (Z_{it} - \bar{Z}_i) - \sigma_j (Z_{jt} - \bar{Z}_j) \big\},\\
...
\begin{itemize}
\item First, we look at the intermediate statistic $\doublehattwo{\Phi}_{n, T}$ (defined below) that can be regarded as the version of the statistics $\widehat{\Phi}_{n, T}$ but without the regressors. Then by Proposition \ref{propA-strong-approx-equality}, there exist statistics $\widetilde{\Phi}_{n, T}$ for $T = 1,2,\ldots$ which are distributed as $\doublehattwo{\Phi}_{n, T}$ for any $T \ge 1$ and which have the property that 
...
\item Second, by Proposition \ref{propA-anticon-equality} we show that $\Phi^\prime_{n,T}$ does not concentrate too strongly in small regions of the form $[x-\delta_T,x+\delta_T]$ with $\delta_T$ converging to zero. Or, in other words, it holds that 
\begin{align}\label{eq-theo-stat-equality-step2}
\sup_{x \in \reals} \pr \big( | \Phi^\prime_{n,T} - x | \le \delta_T \big) = o(1),
\end{align}
...
{\color{red} This part does not go through!
For this part to go through we need stricter assumption on $\rho_T$: $$\rho_T = O\left(\frac{1}{T^{1/2 - 1/q} \sqrt{\log T}}\right)!$$}
\end{proof}
...
According to Theorem \ref{theo-wu} (Theorem 2(ii) in \cite{Wu2016}), denoting the weights from the theorem as $a_t = \sqrt{D_{T, u, h}}\cdot w_{t,T}(u,h)$ we can bound the following probability:
\begin{align*}
...
\[ \sup_{x \in \reals} \big| \pr(\widehat{\Phi}_{n, T} \le x) - \pr(\Phi_{n,T} \le x) \big| = o(1). \]
\end{propA}
\begin{proof}[\textnormal{\textbf{Proof of Proposition \ref{propA-final}}}] 
First, we consider those $x\in \reals$ such that\linebreak $\pr(\widehat{\Phi}_{n,T} \le x) \geq \pr(\Phi_{n, T} \le x)$. Then by Proposition \ref{propA-intermediate-relation} and taking $\gamma_{n, T}$ from the Proposition \ref{propA-intermediate-relation-2} we have
\begin{align*}
