...
\begin{table}[h!]
  \begin{center}
	...
	``Gaussian'' statistic & $ \Phi_{n, T}$ & $\begin{array} {l@{}l@{}}       \Phi_{n, T} &{}= \max_{1 \le i < j \le n} \Phi_{ij,T},\\
      ...
      3 & 23.113231 & c\\
    \end{tabular}
      \hline          Intermediate statistic& $ \widehat{\Phi}_{n, T}$ & $\begin{array} {l@{}l@{}}        \widehat{\Phi}_{n, T} &{}= \max_{1 \le i < j \le n} \widehat{\Phi}_{ij,T},\\
        \widehat{\Phi}_{ij,T}  & {}= \max_{(u,h) \in \mathcal{G}_T} \Big\{ \Big| \frac{\widehat{\phi}_{ij,T}(u,h)} {\{ \widehat{\sigma}_i^2 + \widehat{\sigma}_j^2 \}^{1/2}} \Big| - \lambda(h) \Big \} \end{array}$ 
      \\
...
$\begin{array}{|r|l|}
\hline
\text{Notation} & \text{Exact formula} \\
\hline
& \\
\Phi_{n,T} & \\
 &  \widehat{\Phi}_{n, T} = \max_{1 \le i < j \le n} \widehat{\Phi}_{ij,T}, \quad \widehat{\Phi}_{ij,T} = \max_{(u,h) \in \mathcal{G}_T} \Big\{ \Big| \frac{\widehat{\phi}_{ij,T}(u,h)} {\{ \widehat{\sigma}_i^2 + \widehat{\sigma}_j^2 \}^{1/2}} \Big| - \lambda(h) \Big \}\\
&\\
\doublehattwo{\Phi}_{n, T} & x-y \\
\widetilde{\Phi}_{n, T} & \\
\hline
\end{array}$
