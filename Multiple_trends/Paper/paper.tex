...
\item Second, in Proposition \ref{propA-anticon-equality} we demonstrate that $\Phi_{n,T}$ does not concentrate too strongly in small regions of the form $[x-\delta_T,x+\delta_T]$ with $\delta_T$ converging to zero as $T\to \infty$. Or, in other words, it holds that 
\begin{align*}
...
\begin{align*}
\doublehattwo{\Phi}_{n,T} &= \max_{1 \le i < j \le n}\max_{(u,h) \in \mathcal{G}_T} \doublehattwo{\phi^0}_{ij,T}(u, h)\quad  \text{with} \\
\doublehattwo{\phi^0}_{ij,T}(u, h) &=  \bigg|\frac{\doublehat{\phi}_{ij,T}(u,h)}{\big\{\doublehattwo{\sigma}_i^2 + \doublehattwo{\sigma}_j^2\big\}^{1/2}}\bigg| - \lambda(h)
\end{align*}
...
The statistics $\doublehattwo{\Phi}_{n,T}$ can thus be viewed as a version of the statistic $\widehat{\Phi}_{n,T}$ without the covariates. However, we formally prove that these two statistics are close in Step 4. In this step, we are interested in another matter.
