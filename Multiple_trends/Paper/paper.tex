...
\subsection{The test procedure}\label{subsec-test-without-regs-test}
...
\subsection{Theoretical properties of the test}\label{subsec-test-without-regs-theo}
...
\section{Testing for equality of time trends in the presence of exogenous regressors}\label{sec-test-with-regs}


\subsection{Construction of the test statistic}\label{subsec-test-with-regs-stat}

We now extend the model \eqref{model-without-regs} to include the exogenous regressors:
\begin{equation}\label{model-with-regs}
Y_{it} = \bm{\beta}_i^\top \mathbf{X}_{it} + m_i \Big( \frac{t}{T} \Big) + \alpha_i + \varepsilon_{it} 
\end{equation}
...
\begin{align*}
Y_{it} - \bm{\beta}_i^\top \mathbf{X}_{it} & =: V_{it}\\
					& = m_i \Big( \frac{t}{T} \Big) + \alpha_i + \varepsilon_{it} 
\end{align*}
replacing $Y_{it}$ by $V_{it}$ in the construction of the test statistic. However, $\bm{\beta}_i$ is not known so we need to estimate it first. Moreover, as before, $\alpha_i$ is also not known. Given estimators $\widehat{\bm{\beta}}_i$ and $\widehat{\alpha}_i$, we then consider
\begin{align*}
	\widehat{V}_{it} := Y_{it} -\widehat{\alpha}_i - \widehat{\bm{\beta}}_i^\top \mathbf{X}_{it} =(\widehat{\bm{\beta}}_i - \bm{\beta}_i)^\top \mathbf{X}_{it} + m_i \Big( \frac{t}{T} \Big) + \big( \alpha_i - \widehat{\alpha}_i \big) + \varepsilon_{it} 
\end{align*}
...
\end{align*}
with the kernel weights defined in \eqref{weights}. Similar as in Section \ref{subsec-test-without-regs-stat}, we aggregate the kernel averages $\widehat{\psi}^{\prime}_{ij, T}(u, h)$ for all $(u, h)\in \mathcal{G}_T$ by the multiscale test statistic
\[ \widehat{\Psi}^\prime_{ij,T} = \max_{(u,h) \in \mathcal{G}_T} \Big\{ \Big|\frac{\widehat{\psi}^\prime_{ij,T}(u,h)}{(\widehat{\sigma}_{i,V}^2 + \widehat{\sigma}_{j, V}^2)^{1/2}}\Big| - \lambda(h) \Big\}. \]

Here we let $\widehat{\sigma}_{i, V}^2$ be an aprropriate estimator of the long-run error error variance $\sigma_i^2$ constructed from the sample $\{\widehat{V}_{i1}, \ldots, \widehat{V}_{iT}\}$. Hence, we regard $\widehat{\sigma}_{i, V}^2 = \widehat{\sigma}_{i, V}^2(\widehat{V}_{i1}, \ldots, \widehat{V}_{iT})$ as a function of variables $\widehat{V}_{it}$ for $1\leq t \leq T$. As in the previous section, we assume that $\widehat{\sigma}_{i, V}^2 = \sigma_i^2 + o_P(\rho_T)$ with $\rho_T = o(1/\log T)$. We shall omit the subscript $V$ in $\widehat{\sigma}_{i, V}^2$ hereafter if it does not lead to any confusion. 

\subsection{Estimation of the parameters $\bm{\beta}_i$ and $\alpha_i$}\label{subsec-test-with-regs-est}
...
Therefore, if we consider the unobserved variable $V_{it}^\circ := Y_{it} - \bm{\beta}_i^\top \mathbf{X}_{it} - \alpha_i = m_i(t/T) + \varepsilon_{it}$, they can be well approximated by $\widehat{V}_{it} = Y_{it} -\widehat{\alpha}_i - \widehat{\bm{\beta}}_i^\top \mathbf{X}_{it} = V_{it}^\circ + O_P(T^{-1/2})$. Hence, we our test statistic $\widehat{\Psi}_{n, T}^\prime$ constructed from the sample $\{\widehat{V}_{i1}, \ldots, \widehat{V}_{iT}\}$ can be used to test the hypotheses for $V^\circ_{it}$.


\subsection{The test procedure}\label{subsec-test-with-regs-test}


As in Section \ref{subsec-test-without-regs-test}, let $Z_{it}$ for $1 \le t \le T$ and $1 \le i \le n$ be independent standard normal random variables which are independent of the error terms $\varepsilon_{it}$ and of the covariates $\mathbf{X}_{it}$ and $\bar{Z}_{i} :=  \bar{Z}_{i,T} = T^{-1} \sum_{t=1}^T Z_{it}$ be the empirical average of the variables $Z_{i1},\ldots,Z_{iT}$. For each $i$ and $j$, we introduce the Gaussian statistic 
\[ \Phi_{ij,T}^\prime = \max_{(u,h) \in \mathcal{G}_T} \Big\{ \Big|\frac{\phi_{ij,T}^\prime(u,h)}{(\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2)^{1/2}}\Big| - \lambda(h) \Big\}, \] 
where \textcolor{red}{$\phi_{ij,T}^\prime(u,h) = \sum\nolimits_{t=1}^T w_{t,T}(u,h) \, \{ \widehat{\sigma}_i (Z_{it} - \bar{Z}_i) - \widehat{\sigma}_j (Z_{jt} - \bar{Z}_j) \}$}. Moreover, we define the statistic
\[ \Phi_{n,T}^\prime = \max_{1 \le i < j \le n} \Phi_{ij,T}^\prime \]
and denote its $(1-\alpha)$-quantile by $q_{n,T}^\prime(\alpha)$. Our multiscale test of the hypothesis $H_0: m_1 = m_2 = \ldots = m_n$ is defined as follows: For a given significance level $\alpha \in (0,1)$, we reject $H_0$ if $\widehat{\Psi}_{n,T}^\prime > q_{n,T}^\prime(\alpha)$. 


\subsection{Theoretical properties of the test}\label{subsec-test-with-regs-theo}


Similarly to Section \ref{subsec-test-without-regs-theo}, we introduce the auxiliary statistic 
\[ \widehat{\Phi}_{n,T}^\prime = \max_{1 \le i < j \le n} \widehat{\Phi}_{ij,T}^\prime, \]
where
\[ \widehat{\Phi}^\prime_{ij,T} = \max_{(u,h) \in \mathcal{G}_T} \Big\{ \Big| \frac{\widehat{\phi}^\prime_{ij,T}(u,h)} {\{ \widehat{\sigma}_i^2 + \widehat{\sigma}_j^2 \}^{1/2}} \Big| - \lambda(h) \Big \} \]
and \textcolor{red}{$\widehat{\phi}^\prime_{ij,T}(u,h) = \sum_{t=1}^T w_{t,T}(u,h) \{ (\varepsilon_{it} - \bar{\varepsilon}_i) - (\varepsilon_{jt} - \bar{\varepsilon}_j) \}$} with $\bar{\varepsilon}_i := \bar{\varepsilon}_{i,T} = T^{-1} \sum_{t=1}^T \varepsilon_{it}$. 
First, we characterize the asymptotic behaviour of this auxiliary statistic $\widehat{\Phi}_{n,T}^\prime$. 
\begin{theorem}\label{theo-stat-equality-with-regs}
Suppose that the error processes $\mathcal{E}_i = \{ \varepsilon_{it}: 1 \le t \le T \}$ are independent across $i$ and satisfy \ref{C-err1}--\ref{C-err3} for each $i$. Moreover, let \ref{C-ker}--\ref{C-h} be fulfilled and assume that $\widehat{\sigma}_i^2 = \sigma_i^2 + o_p(\rho_T)$ with $\rho_T = o(1/\log T)$ for each $i$. Then 
\[ \pr \big( \widehat{\Phi}_{n,T}^\prime \le q_{n,T}^\prime(\alpha) \big) = (1 - \alpha) + o(1). \]
\end{theorem}
...
\begin{prop}\label{prop-test-equality}
Let the conditions  of Theorem \ref{theo-stat-equality-with-regs} be satisfied. 
\begin{enumerate}[label=(\alph*),leftmargin=0.75cm]
\item Under the null hypothesis $H_0: m_1 = m_2 = \ldots = m_n$, it holds that 
\[ \pr \big( \widehat{\Psi}_{n,T}^\prime \le q_{n,T}^\prime(\alpha) \big) = (1 - \alpha) + o(1). \]
%\item Let $m_i = m_{i,T}$ be a Lipschitz continuous function with $\int_0^1 m_{i,T}(w) dw = 0$ for any $i$. In particular, suppose that $|m_{i,T}(v) - m_{i,T}(w)| \le L |v - w|$ for all $v,w \in [0,1]$ and some fixed constant $L < \infty$ which does not depend on $T$. Moreover, assume that for some pair of indices $i$ and $j$, the functions $m_{i,T}$ and $m_{j,T}$ have the following property: There exists $(u,h) \in \mathcal{G}_T$ with $[u-h,u+h] \subseteq [0,1]$ such that $m_{i,T}(w) - m_{j,T}(w) \ge c_T \sqrt{\log T/(Th)}$ for all $w \in [u-h,u+h]$ or $m_{j,T}(w) - m_{i,T}(w) \ge c_T \sqrt{\log T/(Th)}$ for all $w \in [u-h,u+h]$, where $\{c_T\}$ is any sequence of positive numbers with $c_T \rightarrow \infty$. Then 
