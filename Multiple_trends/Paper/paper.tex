...
In order to invesitgate the theoretical properties of our multiscale test, we introduce an auxiliary test statistic 
\begin{align}\label{eq:Phi_hat}
\widehat{\Phi}_{n,T} = \max_{1 \le i < j \le n}  \max_{(u,h) \in \mathcal{G}_T} \widehat{\phi}^0_{ij,T}(u,h),
\end{align}
where
\begin{align*}
\widehat{\phi}^0_{ij,T}(u,h) &=  \Big| \frac{\widehat{\phi}_{ij,T}(u,h)} {\{ \widehat{\sigma}_i^2 + \widehat{\sigma}_j^2 \}^{1/2}} \Big| - \lambda(h),\\
...
\section{Estimation of the parameters $\bm{\beta}_i$}\label{sec:para}
...
\subsection{Estimation of  $\sigma^2_i$}\label{subsec:para:sigma}
...
Here, we prove the theoretical results from Sections \ref{sec:theo} and \ref{sec:para}. We use the following notation: The symbol $C$ denotes a universal real constant which may take a different value on each occurrence. For $a,b \in \reals$, we write $a_+ = \max \{0,a\}$ and $a \vee b = \max\{a,b\}$. For $x \in \reals, x \geq 0$, we write $\lfloor x \rfloor$ and $\lceil x \rceil$ to denote the greatest integer less than or equal and the smallest integer greater than or equal to $x$, respectively. For any set $A$, the symbol $|A|$ denotes the cardinality of $A$. The notation $X \stackrel{\mathcal{D}}{=} Y$ means that the two random variables $X$ and $Y$ have the same distribution. Finally, $f_0(\cdot)$ and $F_0(\cdot)$ denote the density and the distribution function of the standard normal distribution, respectively.
...
%  \end{center}
%\end{table}
In the proof of Theorem \ref{theo:stat:global}, we use a number of different test statistics, either already defined in Section \ref{sec:test} or the auxiliary statistics defined below. Each of these statistics plays an important role in one or more steps of the proof. In the following list, we present these test statistics, describe how they are constructed and explain in which parts of the proof they are used. %Table \ref{tab:app-table1} is a useful guide for connecting these statistics with their places in the proof strategy presented below.
\begin{itemize}
\item Our main multiscale statistics defined in \eqref{eq:Psi_hat}. It is calculated based on data.
\begin{align}\label{eq:stat1}
	\widehat{\Psi}_{n,T} & = \max_{1 \le i < j \le n} \max_{(u,h) \in \mathcal{G}_T}\widehat{\psi}^0_{ij,T}(u,h), \nonumber\\
      	\widehat{\psi}^0_{ij,T}(u,h) &= \Big|\frac{\widehat{\psi}_{ij,T}(u,h)}{(\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2)^{1/2}}\Big| - \lambda(h),\\
        \widehat{\psi}_{ij,T}(u,h) &= \sum\limits_{t=1}^T w_{t,T}(u,h)(\widehat{Y}_{it} - \widehat{Y}_{jt}). \nonumber
\end{align}

\item The Gaussian statistics that is used to calculate the critical values (defined in \eqref{eq:Phi}).
\begin{align}\label{eq:stat2}
	\Phi_{n,T}  &= \max_{1 \le i < j \le n} \max_{(u,h) \in \mathcal{G}_T} \phi^0_{ij,T}(u,h) , \nonumber\\
      	\phi^0_{ij,T}(u,h)  &= \Big|\frac{\phi_{ij,T}(u,h)}{({\sigma}_i^2 + {\sigma}_j^2)^{1/2}}\Big| - \lambda(h),\\
	 \phi_{ij,T}(u,h) & = \sum\nolimits_{t=1}^T w_{t,T}(u,h) \, \big\{{\sigma}_i (Z_{it} - \bar{Z}_i) - {\sigma}_j (Z_{jt} - \bar{Z}_j) \big\}.\nonumber
\end{align}

\item Auxiliary statistics that can be regarded as the version of our multiscale statistic under $H_0$ (defined in \eqref{eq:Phi_hat}). Our main theoretical result (Theorem \ref{theo:stat:global}) investigates the distribution of $\widehat{\Phi}_{n,T}$.
\begin{align}\label{eq:stat3}
	\widehat{\Phi}_{n,T} &= \max_{1 \le i < j \le n} \max_{(u,h) \in \mathcal{G}_T} \widehat{\phi}^0_{ij,T}(u,h),\nonumber \\
	\widehat{\phi}^0_{ij,T}(u,h)&=\Big| \frac{\widehat{\phi}_{ij,T}(u,h)} {\{ \widehat{\sigma}_i^2 + \widehat{\sigma}_j^2 \}^{1/2}} \Big| - \lambda(h), \\
	\widehat{\phi}_{ij,T}(u,h) &= \sum_{t=1}^T w_{t,T}(u,h) \big\{ (\varepsilon_{it} - \bar{\varepsilon}_i) + (\bm{\beta}_i - \widehat{\bm{\beta}}_i)^\top (\mathbf{X}_{it} - \bar{\mathbf{X}}_{i})  \nonumber \\
	&\quad\quad - (\varepsilon_{jt} - \bar{\varepsilon}_j) -  (\bm{\beta}_j - \widehat{\bm{\beta}}_j)^\top (\mathbf{X}_{jt} - \bar{\mathbf{X}}_{j})\big\}.\nonumber 
...
\item Intermediate statistics that is close to $\widehat{\Phi}_{n, T}$ but is based on the kernel averages $\doublehat{\phi}_{ij,T}(u,h)$ that are different from $\widehat{\phi}_{ij,T}(u,h)$ only by the fact that they do not include the covariates $\X_{it}$. 
\begin{align}\label{eq:stat4}
	\doublehattwo{\Phi}_{n,T} &= \max_{1 \le i < j \le n} \max_{(u,h) \in \mathcal{G}_T} \doublehat{\phi}^0_{ij,T}(u,h),\nonumber \\
	\doublehat{\phi}^0_{ij,T}(u,h) &= \Bigg| \frac{\doublehat{\phi}_{ij,T}(u,h)} {\{ \doublehattwo{\sigma}_i^2 + \doublehattwo{\sigma}_j^2 \}^{1/2}} \Bigg| - \lambda(h), \\
	 \doublehat{\phi}_{ij,T}(u,h) &= \sum_{t=1}^T w_{t,T}(u,h) \big\{ (\varepsilon_{it} - \bar{\varepsilon}_i) - (\varepsilon_{jt} - \bar{\varepsilon}_j)  \big\}.\nonumber 
\end{align}

We can view these kernel averages as constructed (under the null) based on the unobserved variables $\doublehattwo{Y}_{it}$ that are defined by 
...
\item Auxiliary statistics that has the same distribution as $\doublehattwo{\Phi}_{n, T}$ for each $T = 1, 2, \ldots$. We prove that there exists such statistics with the property that they are close to the Gaussian test statistics $\Phi_{n,T}$ in Proposition \ref{propA:strong_approx} using the strong approximation theory by \cite{BerkesLiuWu2014}. 
\begin{align}\label{eq:stat5}
\widetilde{\Phi}_{n,T} &= \max_{1 \le i < j \le n}  \max_{(u,h) \in \mathcal{G}_T}\widetilde{\phi}^0_{ij, T}(u,h) ,\nonumber\\
\widetilde{\phi}^0_{ij, T}(u,h) &=  \bigg|\frac{\widetilde{\phi}_{ij, T}(u,h)}{\{\widetilde{\sigma}_i^2 + \widetilde{\sigma}_j^2 \}^{1/2}} \bigg| - \lambda(h), \\
...
\begin{enumerate}
\item First, we introduce the intermediate statistics $\doublehattwo{\Phi}_{n, T}$ that can be regarded as the version of the statistics $\widehat{\Phi}_{n, T}$ where we excluded the regressors $\X_{it}$ from the construction of the kernel averages. By Proposition \ref{propA:strong_approx}, we show that there exist statistics $\widetilde{\Phi}_{n, T}$ for $T = 1,2,\ldots$ which are distributed as $\doublehattwo{\Phi}_{n, T}$ for any $T \ge 1$ and which have the property that 
\begin{align*}
\big| \widetilde{\Phi}_{n, T} - \Phi_{n,T} \big| = o_p \Big( \frac{T^{1/q}}{\sqrt{T h_{\min}}} + \rho_T\sqrt{\log T} \Big),
\end{align*}
where $\Phi_{n, T}$ is the Gaussian statistic defined in \eqref{eq:Phi}. This approximation result allows us to replace the multiscale statistic $\doublehattwo{\Phi}_{n, T}$ by an identically distributed version $\widetilde{\Phi}_{n, T}$ which is close to $\Phi_{n, T}$.

\item Second, by Proposition \ref{propA-anticon-equality} we show that $\Phi_{n,T}$ does not concentrate too strongly in small regions of the form $[x-\delta_T,x+\delta_T]$ with $\delta_T$ converging to zero as $T\to \infty$. Or, in other words, it holds that 
\begin{align*}
...
\begin{align*}
\doublehattwo{\Phi}_{n,T} &= \max_{1 \le i < j \le n}\max_{(u,h) \in \mathcal{G}_T} \doublehattwo{\phi}^0_{ij,T}(u, h)\quad  \text{with} \\
\doublehattwo{\phi}^0_{ij,T}(u, h) &=  \bigg|\frac{\doublehat{\phi}_{ij,T}(u,h)}{\big\{\doublehattwo{\sigma}_i^2 + \doublehattwo{\sigma}_j^2\big\}^{1/2}}\bigg| - \lambda(h)
\end{align*}
...
The statistics $\doublehattwo{\Phi}_{n,T}$ can thus be viewed as an approximation of the statistic $\widehat{\Phi}_{n,T}$. However, formally we prove that these two statistics are close in Step 4. In this step, we are interested in another matter.
...
Specifically, the main purpose of this part of the proof is to show that there is a version of the multiscale statistics $\doublehattwo{\Phi}_{n,T}$ which is close to the Gaussian statistics $\Phi_{n,T}$ (defined in \eqref{eq:Phi}) which distribution is known. More specifically, we prove the following result. 

\begin{propA}\label{propA:strong_approx}
Under the conditions of Theorem \ref{theo:stat:global}, there exist statistics $\widetilde{\Phi}_{n,T}$ for $T = 1,2,\ldots$ with the following two properties: (i) $\widetilde{\Phi}_{n, T}$ has the same distribution as $\doublehattwo{\Phi}_{n, T}$ for any $T$, and (ii)
...
In this section, we establish some properties of the Gaussian statistic $\Phi_{n,T}$ defined in \eqref{eq:Phi}. We in particular show that $\Phi_{n,T}$ does not concentrate too strongly in small regions of the form $[x-\delta_T,x+\delta_T]$ with $\delta_T$ converging to zero.  
...
\end{equation}
\end{lemmaA}
Proof of this lemma is provided in \cite*{KhismatullinaVogt2020}.
...
\end{alignat*}
Since $\mathcal{P}_{i, t}(\Delta \mathbf{X}_{it}\Delta \varepsilon_{it}) $, $2 \leq t \leq T$, are martingale differences, we have that 
\begin{align*}
...
&= \frac{1}{(T-1)^2} \sum_{t=2}^T \Big|\Big| \mathcal{P}_{i, t-s}\big( \Delta \mathbf{X}_{it}\Delta \varepsilon_{it}\big) \Big|\Big|^2\\
