...
Thus, the unobserved variables $Y_{it}^\circ$ can be approximated by $\widehat{Y}_{it}$, and in what follows we show under some mild conditions on $\widehat{\alpha}_i$ and $\widehat{\bm{\beta}}_i$, this approximation is indeed sufficient for our analysis. %provide the algorithm for computing our test statistic based on $\widehat{Y}_{it}$.
