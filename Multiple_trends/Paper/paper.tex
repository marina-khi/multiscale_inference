...
\textcolor{blue}{\textbf{JEL classification:} C12; C23; C38 (cluster analysis?).}
...
%clusters the observed time series into groups with the same trend.
...
%Without this condition, one can freely shift the functions $m_i$ by any (positive or negative) constant $c_i$ while simultaneously subtracting this constant from $\alpha_i$: $Y_{it} = [m_i(t/T) + c_i] + \bfbeta_i^\top \X_{it} + [\alpha_i - c_i] + \varepsilon_{it}$.
...
%In addition, Section \ref{subsec:test:loc} discusses which formal confidence statements can be made based on our test, and 
...
%To start with, we construct estimators of the fixed effects $\alpha_i$ and the coefficient vectors $\bfbeta_i$. 
...
%However, in reality the variables $Y_{it}^\circ$ are not observed since the intercept $\alpha_i$ and the coefficients $\bfbeta_i$ are not known. Nevertheless, given appropriate estimators $\widehat{\alpha}_i$ and $\widehat{\bfbeta}_i$, we can consider
\begin{align*}
...
%In addition, we discuss how to estimate the long-run error variance $\sigma^2 := \sum\nolimits_{\ell=-\infty}^{\infty} \cov(\varepsilon_{i0}, \varepsilon_{i\ell})$, which will also become relevant in the construction of our test statistics later on. 

%But before we proceed further, we show how to construct consistent estimates $\widehat{\alpha}_i$ and $\widehat{\bfbeta}_i$. 
...
&= \big(\bfbeta_i - \widehat{\bfbeta}_i \big)^\top\frac{1}{T}\sum_{t=1}^T  \X_{it} + \alpha_i + \frac{1}{T}\sum_{i=1}^T m_i(t/T) + \frac{1}{T}\sum_{i=1}^T \varepsilon_{it} = O_p(T^{-1/2}), %\label{eq:alpha:est1}
\end{align*}
...
%Plugging all these results into \eqref{eq:alpha:est1}, we get that $\widehat{\alpha}_i - \alpha_i = O_p(T^{-1/2})$ for each $i$. %, which shows that $\widehat{\alpha}_i$ is a reasonable estimator of $\alpha_i$. 
%Thus, the unobserved variables \linebreak $Y_{it}^\circ := Y_{it} - \bfbeta_i^\top \X_{it} - \alpha_i = m_i(t/T) + \varepsilon_{it}$ can be well approximated by $\widehat{Y}_{it} $ since \linebreak $\widehat{Y}_{it} = Y_{it} -\widehat{\alpha}_i - \widehat{\bfbeta}_i^\top \X_{it} = Y_{it}^\circ + O_p(T^{-1/2})$.
...
% which is necessary for the construction of the test statistics later on. 
...
%For the smaller class of AR($p$) error processes with a given finite $p$, estimators of the long-run error variance can for example be found in \cite{Hall2003}. 

 
%The subseries variance estimator just described is that it can be used without imposing any additional assumptions on the error processes $\mathcal{E}_i$. However, as noted in \cite{KhismatullinaVogt2020}, estimating the long-run error variance in the presence of a pronounced trend and under general weak dependence conditions is a particularly difficult problem. Estimators often tend to be quite imprecise. Hence, in practice, it may pay off to estimate the long-run error variance under additional restrictions on the error process. 
% we opt for imposing certain some time series models on the error processes $\mathcal{E}_i$. 
%For example, we may assume that for each $i$, the error process $\mathcal{E}_i$ has an AR($\infty$) structure which covers a wide range of applications. In this case, it is possible to obtain more precise estimates $\widehat{\sigma}_i^2$ using the difference-based method described in \cite{KhismatullinaVogt2020}. %In our application examples, we choose to impose this restriction on the error terms.
...
%To start with, we design a test statistic for the local null hypothesis $H_0^{[i, j]}(u, h)$, where $(u,h)$ is a given location-scale point and $(i,j)$ a given pair of time series. 
Let
%We are now ready to introduce the multiscale statistic for testing the hypothesis \linebreak $H_0: m_1 = m_2 = \ldots = m_n$. For any pair of time series $i$ and $j$ and for any location-bandwidth pair $(u, h)$, we define
...
%Notably, we do not only want to test $H_0^{[i, j]}(u, h)$ for a single location-scale point $(u,h)$ and a single pair $(i,j)$ of time series. 
...
%Note that the value $\max_{(u, h) \in \mathcal{G}_T} \widehat{\psi}^0_{ij, T}(u, h)$ simultaneously takes into account all intervals $\interval = [u- h, u+h]$ with $(u, h) \in \mathcal{G}_T$. Thus, it can be interpreted as a global distance measure between the two curves $m_i$ and $m_j$, and the test statistics $\widehat{\Psi}_{n,T}$ is then defined as the maximal distance between any pair of curves $m_i$ and $m_j$ with $i \ne j$.
...
%For this purpose, we employ the notion of the family-wise error rate (FWER) which is equal to the probability of making one or more type I errors. Formally, the FWER is defined as
\begin{align*} 
\text{FWER}(\alpha) 
...
% i, j \in \{1, \ldots, n \} \text{ and } (u, h) \in \grid : & \ \widehat{\psi}^0_{ij, T}(u, h) > q_{n,T}(\alpha) %\interval \in \mathcal{S}^{[i, j]}(\alpha) 
%\\ & \ \text{and } H_0^{[i, j]}(u, h) \text { is true}\Big)
\end{align*}
...
%\[ \Pi_{n,T}(\ell,\ell^\prime) = \big\{ \interval = [u-h, u+h]: (u, h) \in \mathcal{A}_{n,T}(\ell,\ell^\prime) \big\} \]
...
%We also assume that the fixed--effect term $\alpha_i$ is equal to $0$ in all time series. 
...
%Given $\widehat{\beta}_i$, we obtain an estimator $\widehat{\alpha}_i$ of the fixed effect term $\alpha_i$ calculated as in \eqref{eq:alpha:est}. 
%We then work with the augmented time series $\widehat{Y}_{it} = Y_{it} - \widehat{\alpha}_i - \widehat{\beta}_i X_{it}$. 
...
%Since the errors $\varepsilon_{it}$ follow an AR($1$) process, we estimate the long-run variances $\sigma_i^2$ by difference-based estimator proposed in \cite{KhismatullinaVogt2020}, setting the tuning parameters $q$ and $r$ to $10$ and $25$, respectively. 
...
%Table \ref{tab:power} reports the power of the test under different alternatives, where the power is computed in the same way as the acual size under the null. All in all, the test shows reasonable power against the alternatives under consideration. 
%Moreover, as can be seen from Table \ref{tab:power}, the test also has reasonable power against the alternatives considered. 
%For the slope $b=0.75$ and small sample sizes $T = 100$ and $T = 250$, the power is only moderate, reflecting the fact that the alternative is not very far away from the null. This is also the case for larger values of the slope $b = 1, 1.25$ and the smallest sample size $T = 100$. However, as we increase the slope $b$ and the sample size $T$, the power quickly increases. Already for the slope value $b = 1.00$, we reach a power of $0.99$ for $T = 500$ and for all nominal sizes $\alpha$.
...
%\textcolor{red}{We repeat the analysis using the quarterly data on Gross Fixed Capital Formation instead of Capital Stock in the Appendix.}
...
%\textcolor{red}{We repeat the analysis using not seasonally adjusted data for all of the $11$ countries as a robustness check and we report the results in the Appendix.}
...
\begin{align}
 \Delta \ln GDP_{it}
% & = m_i \Big( \frac{t}{T} \Big) + \bfbeta^\top_i \X_{it} + \alpha_i + \varepsilon_{it} \nonumber \\
...
%Similarly to \cite{Zhang2012}, we rewrite the model \eqref{eq:model:app} as
...
%More precisely, we proceed as follows: We assume that for each $i$, the error process $\mathcal{E}_i$ follows an AR($p_i$) model $\varepsilon_{it} = \sum_{j=1}^{p_i} a_{i, j} \varepsilon_{i(t-j)} + \eta_{it}$, where the order of the process $p_i$ is country-specific and not known and $\eta_{it}$ are i.i.d.\ innovations with mean zero. We choose $p_i$ as the minimiser of the Bayesian Information Criterion (BIC).\footnote{We also calculate the values of other information criteria such as FPE, AIC and HQ which, in most of the cases, resulted in the same values of $p_i$.} This yields $p_i = 3$ for Australia, Canada and the UK, and $p_i = 1$ for all other countries. This assumption allows us to use a difference-based long-run variance estimator proposed in \cite{KhismatullinaVogt2020}.% with the choice of the tuning parameters described below.
%The details are summarised in Section \ref{subsec:app:lrv} of the Appendix. % with the following specification of the parameters: $\underline{r}=1$, $\overline{r}=10$ and $q = 15$.
...
%\item The significance levels are taken to be $\alpha = 0.05$ and $\alpha = 0.1$. (Note that \citet{Zhang2012} report results for the significance level $\alpha = 0.1$.) 
...
%for which our tests finds differences between the trends $m_i$ and $m_j$ 
...
%In all these cases, our test detects differences between the Norwegian and the other trends at the end of the observation period. In particular, the test rejects the local null for at least one interval which covers the last $10$ years of the analysed time period (from the first quarter in $2000$ up to the third quarter in $2010$). Inspecting the trend estimates in panels (b) of Figures \ref{fig:Australia:Norway}--\ref{fig:Japan:Norway}, the Norwegian trend function appears to move downwards at the end of the observation period, whereas all of the other countries appear to have a slightly increasing trend. According to our test, this is a significant difference between the Norwegian and the other trend functions rather than an artefact of the sampling noise. Apart from these differences at the end of the sampling period, our test also finds differences in the beginning, however, only for part of the pairwise comparisons. 
...
%The results of applying our test are in line with our conclusions from the visual inspection: 
...
%The number of clusters ist estimated to be 
%. Since we do not know the true number of clusters, we estimate the number of clusters using the $1- \alpha$ quantile of the Gaussian test statistics $\Phi_{n, T}$. 
%This results in 
%$\widehat{N} = 3$. 
...
%, we also see that our test detected a cluster that consists only of Norway (plotted in red). This result supports the observation stated above that Norway is the only country that has a decreasing GDP trend at the end of the considered time period, while all of the other countries have a slight increase in the same time region. The clustering algorithm places Norway in a separate cluster, hence, it is able to detect this difference. 
%Furthermore, as can be seen in Figure \ref{fig:gdp:dend}, 
...
%Finally, analysing the results of the clustering algorithm depicted in Figures \ref{fig:gdp:dend} and \ref{fig:gdp:all_clusters}, we also see that our test detected a cluster that consists only of Norway (plotted in red). This result supports the observation stated above that Norway is the only country that has a decreasing GDP trend at the end of the considered time period, while all of the other countries have a slight increase in the same time region. The clustering algorithm places Norway in a separate cluster, hence, it is able to detect this difference. Furthermore, as can be seen in Figure \ref{fig:gdp:dend}, the algorithm determines a cluster that contains three countries: Australia, Finland and the USA (plotted in blue in Figure \ref{fig:gdp:all_clusters}). Based on the visual inspection, we see that the time trends in these three countries exhibit much more pronounced decreases and increases than the GDP time trends in the other countries. The other seven countries (Austria, Canada, France, Germany, Japan, Switzerland and the UK) have less drastic decreases and increases over the course of 35 years. Therefore, the results of applying the clustering procedure from Section \ref{sec:clustering} support the observations obtained from the visual inspection and from the pairwise comparison of the time trends described above.
...
%To ensure minimal manipulation with the data for all time series, 
...
%Our goal is to test whether the time trend $m_i$ is the same in all $8$ countries. In other words, we would like to test the null hypothesis $H_0: m_1 = \ldots = m_n$ with $n = 8$ in the model \eqref{eq:model:app4}. 
...
%However, contrary to the observations in \cite{Knoll2017}, our proposed procedure is able to account for the effects of the other factors that can influence the real house prices, such as real GDP and population growth. Hence, we are able to pick the signinficant differences between trends in Belgium and Denmark and Belgium and Netherlands in the first half of the observed time period as well, that were not noted in \cite{Knoll2017}.
...
%\section{Conclusion {\small \textcolor{red}{I'd suggest we skip the conclusion to save space.}}}\label{sec:conclusion}
%
%
