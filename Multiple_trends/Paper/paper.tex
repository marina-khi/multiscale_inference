...
\begin{table}[h!]
  ...
  \end{center}
\end{table}
In the proof of Theorem \ref{theo-stat-equality}, we use a number of different test statistics, either defined in Section \ref{sec-test} or auxuliary ones. Each of these statistics plays an important role in one of the steps of the proof. In what follows, we present these test statistics, describe how they are constructed and explain in which parts of the proof they are used. Table \ref{tab:app-table1} is a useful guide for connecting these statistics with their places in the proof strategy presented below.

Multiscale statistics that is calculated based on data (defined in \eqref{Psi-hat-statistic}):
...
Auxiliary statistics that can be regarded as the version of our multiscale statistic $\widehat{\Psi}_{n, T}$ under $H_0$ (defined in \eqref{Phi-hat-statistic}):
...
Intermediate statistics that is based on the kernel averages that do not include the covariates:
...
\end{align}
Auxiliary statistic that has the same distribution as $\doublehattwo{\Phi}_{n, T}$ for each $T = 1, 2, \ldots$
...
The true Gaussian statistics that differs from $\pseudogaussianstat_{n,T}$ only by using true long-run variances $\sigma_i^2$ in construction of the statistics instead of the estimated ones $\widehat{\sigma}_i^2$:  
...
The ``Gaussian'' statistics that is calculated for the critical values (defined in \eqref{Phi-statistic}):
...
\item In the fourth step, by the means of Proposition \ref{propA-final} we prove that  
\begin{equation}\label{eq-theo-stat-equality-step5}
...
\subsection*{Connection between $\widehat{\Phi}_{n, T}$ and $\doublehattwo{\Phi}_{n, T}$}
...
\subsection*{Connection between $\doublehattwo{\Phi}_{n, T}$, $\widetilde{\Phi}_{n, T}$ and $\gaussianstat_{n,T}$}

The main purpose of this section is to prove that there is a version of the multiscale statistic $\doublehattwo{\Phi}_{n,T}$ which is close to a true Gaussian statistic $\gaussianstat_{n,T}$ (defined in \eqref{eq-stat-3}) whose distribution is known. More specifically, we prove the following result. 
...
\big| \widetilde{\Phi}_{n, T} - \gaussianstat_{n,T} \big| = o_p \Big( \frac{T^{1/q}}{\sqrt{T h_{\min}}} &+ \frac{\rho_T}{\sqrt{h_{\min}}}\sqrt{\log T} \Big),
\end{align*}
where $\gaussianstat_{n,T}$ is a Gaussian statistic as defined in \eqref{eq-stat-3}. 
\end{propA}
