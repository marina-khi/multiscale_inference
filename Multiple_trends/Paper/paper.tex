...
Note that in general the statistic $\Phi_{n,T}$ depends on the long-run error variances $\sigma_i^2$. However, as stated in the Remark \ref{rem:simpl}, under the assumption that the long-run error variance is known to be the same across $i$ (that is, $\sigma_i^2 = \sigma^2$ for all $i$), we can simplify the formula for the kernel averages $\phi^0_{ij, T}(u, h)$ so that the distribution of $\Phi_{n, T}$ does not depend on any unknown quantities, including the value of the long-run variance $\sigma_i^2$. We employ this fact to (approximately) calculate the gaussian quantiles $q_{n, T}(\alpha)$ which are further used for testing.
