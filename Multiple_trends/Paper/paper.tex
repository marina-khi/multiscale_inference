...
\item \label{C-h} $h_{\min} \gg T^{-(1-\frac{2}{q})} \log T$, that is, $h_{\min} / \{ T^{-(1-\frac{2}{q})} \log T \} \rightarrow \infty$ with $q > 4$ defined in \ref{C-err2} and {\color{red}$h_{\max} =o(1)$}.
...
For $\gamma_{n, T} = C h_{max}$, where $C$ is a universal constant that does not depend on $n$ and $T$, we have
...
Now consider the term $\big| \sum_{t=1}^T w_{t,T}(u,h) (\mathbf{X}_{it} - \bar{\mathbf{X}}_{i})  \big|$. Without loss of generality, we can regard the covariates $\mathbf{X}_{it}$ to be scalars $X_{it}$, not vectors. The rest of the proof in case of a vector proceeds analogously.
Note that by construction the weights $w_{t, T}(u, h)$ are not equal to $0$ if and only if $T(u-h) \le t \le T(u+h)$. We can use this fact together with the Cauchy-Schwarz inequality to obtain
\begin{align*}
...
Further note that
\begin{align*}
\sum_{t=1}^T w^2_{t,T}(u,h) = \sum_{t=1}^T\frac{\frac{1}{Th} K^2\Big(\frac{\frac{t}{T}-u}{h}\Big) \Big[ S_{T,2}(u,h) - \Big(\frac{\frac{t}{T}-u}{h}\Big) S_{T,1}(u,h) \Big]^2}{ \bigg\{\frac{1}{Th}\sum\nolimits_{s=1}^T  K^2\Big(\frac{\frac{s}{T}-u}{h}\Big) \Big[ S_{T,2}(u,h) - \Big(\frac{\frac{s}{T}-u}{h}\Big) S_{T,1}(u,h) \Big]^2 \bigg\} } = 1. 
\end{align*}
Hence, 
...
\end{align}
Now consider random variables $(X_{it} - \bar{X}_{i})^2$. In order to apply the Markov inequality, we need to estimate $E\big[(X_{it} - \bar{X}_{i})^2\big]$:
\begin{align*}
