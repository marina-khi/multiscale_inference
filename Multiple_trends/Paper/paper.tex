...
\item \label{C-reg3} Let $\ex [\mathbf{H}_{i}(\mathcal{U}_{i0})]=\mathbf{0}$ and $||\mathbf{H}_{i}(\mathcal{U}_{it})||_{q^\prime} <\infty$ for $q^\prime > \theta/2$.
\item \label{C-reg4} $\sum_{s=0}^\infty \delta_4(\mathbf{H}_i, s)<\infty$.
\item \label{C-reg5} $\sup_{m\geq 0} (m+1)^{\alpha} \sum_{t=m}^{\infty} \delta_{q^\prime}(\mathbf{H}_{i}, t) < \infty$ for $\alpha > 1/2 - 1/{q^\prime}$.
\textcolor{red}{Dependence adjusted norm!}
\end{enumerate}
...
\item \label{C-reg-err2} Let $\zeta_{i, t} = (u_{it}, \eta_{it})^\top$. Define $\mathcal{I}_{i, t} = (\ldots, \zeta_{i, t-1}, \zeta_{i, t})$ and $\mathbf{U}_i(\mathcal{I}_{i, t}) =  \mathbf{H}_i(\mathcal{U}_{it})G_i(\mathcal{J}_{it})$. Then, $\sum_{s=0}^\infty \delta_2(\mathbf{U}_i, s)<\infty$.
...
\begin{itemize}
\item First, by Proposition \ref{propA-strong-approx-equality}, there exist statistics $\widetilde{\Phi}_{n, T}$ for $T = 1,2,\ldots$ which are distributed as $\doublehattwo{\Phi}_{n, T}$ for any $T \ge 1$ and which have the property that 
...
\end{align}
where $\delta_T = T^{1/q} / \sqrt{T h_{\min}} + \rho_T \sqrt{\log T}/ \sqrt{h_{\min}}$.
...
\item And finally, in Proposition \ref{propA-final} we prove that  
...
are close to the kernel averages $\widehat{\phi}_{ij,T}(u,h)$ because of the properties of our estimators $\widehat{\bbeta}_i$ and assumptions on $\mathbf{X}_{it}$ but we will prove it later in this Section.
...
\begin{align*}
\Phi_{n, T} & = \max_{1\leq i < j \leq n} \Phi_{ij, T}= \max_{1\leq i < j \leq n} \max_{(u,h) \in \mathcal{G}_T} \Big\{ \Big|\frac{\phi_{ij, T}(u,h)}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2 \}^{1/2}}\Big| - \lambda(h) \Big\} \text{ as in \eqref{Phi-statistic}}, \\
\text{ and }\Phi_{n, T}^{\diamond} & =\max_{1\leq i < j \leq n} \Phi_{ij, T}^{\diamond} = \max_{1\leq i< j \leq n}\max_{(u,h) \in \mathcal{G}_T} \Big\{ \Big|\frac{\phi_{ij, T}(u,h)}{\{\widetilde{\sigma}_i^2 + \widetilde{\sigma}_j^2 \}^{1/2}}\Big| - \lambda(h) \Big\} 
\end{align*}
...
\begin{align*}
\max_{1 \le t \le T} |A_{ij, t}|  \le & \max_{1 \le t \le T} \Big| \sum\limits_{s=1}^t (\widetilde{\varepsilon}_{is} - \bar{\widetilde{\varepsilon}}_{i}) - \widehat{\sigma}_i \sum\limits_{s=1}^t \big\{ Z_{is} - \bar{Z_i} \big\} \Big| + \\
&+ \max_{1 \le t \le T} \Big| \sum\limits_{s=1}^t (\widetilde{\varepsilon}_{js} - \bar{\widetilde{\varepsilon}}_{j}) - \widehat{\sigma}_j \sum\limits_{s=1}^t \big\{ Z_{js} - \bar{Z_j} \big\} \Big| \le \\
...
and (ii) $\sigma_{l}^2 := \var(U^\prime_{l}) = 1$ for all $1 \leq 1 \leq (n-1)np$. Hence, $a_{(n-1)np} = b_{(n-1)np}$. Moreover, as the variables $(U^\prime_l - \mu_l)/\sigma_l$ are standard normal, we have that $a_{(n-1)np} = b_{(n-1)np} \le C\sqrt{\log ((n-1)np)} \leq C \sqrt{\log T}$. With this notation at hand, we can apply Proposition \ref{theo-anticon} to obtain that 
\[ \sup_{x \in \reals} \pr \Big( \big| \Phi_T - x \big| \le \delta_T \Big) \le C \delta_T \Big[ \sqrt{\log T} + \sqrt{ \log(1/\delta_T) } \Big] = o(1) \]
with $\delta_T = T^{1/q} / \sqrt{T h_{\min}} + \rho_T \sqrt{\log T}/\sqrt{h_{\min}}$, which is the statement of Proposition \ref{propA-anticon-equality}.
...
$||X_{\cdot}||^{q^\prime}_{q^\prime, \alpha} = \sup_{m\geq 0} (m+1)^{\alpha} \sum_{t=m}^{\infty} \delta_{q^\prime}(X, t)$.
\end{definitionA}

\begin{theoremA}{\cite{Wu2016}}\label{theo-wu}
Assume that $||X_{\cdot}||^{q^\prime}_{q^\prime, \alpha} < \infty$, where $q^\prime > 2$ and $\alpha >0$, and $\sum_{t=1}^T a_t^2 = T.$ Moreover, assume that $\alpha > 1/2 - 1/q^\prime$. Denote $S_T = a_1 X_1 + \ldots + a_T X_T$. Then for all $x>0$,
\begin{align*}
	\pr(|S_T| \geq x) \leq C_1 \frac{|a|_{q^\prime}^{q^\prime}||X_{\cdot}||^{q^\prime}_{q^\prime, \alpha}  }{x^{q^\prime}} + C_2 \exp \left( - \frac{C_3 x^2} {T ||X_\cdot||^2_{2, \alpha}}\right),
\end{align*}
wjere $C_1, C_2, C_3$ are constants that only depend on $q^\prime$ and $\alpha$.
\end{theoremA}
...
There exists a sequnce of random numbers $\{\gamma_{n, T}\}_T$, that converges to $0$ as $T\to \infty$, such that
...
To begin with, we consider the second summand in \eqref{ineq-diff-1}.

First, by our assumptions $\widehat{\sigma}_i^2 = \sigma_i^2 + o_P(\rho_T)$. Hence, $\max_{1\le i < j \le n}\{\widehat{\sigma}_i^2+ \widehat{\sigma}_j^2 \}^{-1/2}  = O_P(1)$.

Then, by Theorem \ref{theo-regs}, we know that $|\bm{\beta}_i - \widehat{\bm{\beta}}_i| = O_P(1/\sqrt{T})$.

Next, we want to show that $\bar{\X}_i = O_P(1)$. 


For every element $\bar{X}_{i, j}, j = 1, \ldots, d$ of a vector $\bar{\X}_{i}$ we have 
\[\ex[\bar{X}_{i, j}] =  \ex \Big[ \frac{1}{T}\sum_{t=1}^T H_{i,j}(\mathcal{U}_{it})\Big] = \frac{1}{T}\sum_{t=1}^T \ex [H_{i,j}(\mathcal{U}_{it})] = 0\]
and 
\begin{align*}
\var [\bar{X}_{i, j}] &= \var \Big[ \frac{1}{T}\sum_{t=1}^T H_{i,j}(\mathcal{U}_{it})\Big] =\frac{1}{T^2} \ex\bigg[ \Big(\sum_{t=1}^T H_{i,j}(\mathcal{U}_{it})\Big)^2\bigg] = \\
&=\frac{1}{T^2} \sum_{t=1}^T \ex [H^2_{i,j}(\mathcal{U}_{it})] + \frac{1}{T^2} \sum_{t=1}^T \sum_{s = 1, s\neq t}^T \ex [H_{i,j}(\mathcal{U}_{it})H_{i,j}(\mathcal{U}_{is})] = \\
&=\frac{1}{T} \ex [H^2_{i,j}(\mathcal{U}_{i0})] + \frac{1}{T^2} \sum_{t=1}^T \sum_{s = 1, s\neq t}^T \ex [H_{i,j}(\mathcal{U}_{it})H_{i,j}(\mathcal{U}_{is})] \leq \\
&\leq \frac{1}{T} \ex [H^2_{i,j}(\mathcal{U}_{i0})] + \frac{1}{T^2} \sum_{t=1}^T \sum_{s = 1, s\neq t}^T \sqrt{\ex [H^2_{i,j}(\mathcal{U}_{it})]}\sqrt{\ex [H^2_{i,j}(\mathcal{U}_{is})]} = \\
&= \frac{1}{T} \ex [H^2_{i,j}(\mathcal{U}_{i0})] + \frac{T-1}{T} \ex [H^2_{i,j}(\mathcal{U}_{i0})] = \\
&= \ex\big[H_{i,j}^2(\mathcal{U}_{i0})\big],
\end{align*}
by Chebyshev's inequality we have that $|\bar{X}_{i,j}| = \Big|\frac{1}{T}\sum_{t=1}^T H_{i, j}(\mathcal{U}_{it})\Big| = O_P(1)$ for each $j\in\{1, \ldots, d\} $. And this in turn implies that
\begin{align*}
\big| \bar{\X}_i\big| = \Big|\frac{1}{T}\sum_{t=1}^T \mathbf{H}_i (\mathcal{U}_{it}) \Big| = O_P(1).
\end{align*}
\textcolor{red}{Prove here that the value above is $o_P(1)$!!!}

Finally, consider the local linear kernel weights $w_{t,T}(u,h)$ defined in \eqref{weights}. By construction the weights $w_{t, T}(u, h)$ are not equal to $0$ if and only if \linebreak $T(u-h) \le t \le T(u+h)$. We can use this fact to bound  $\max_{(u,h) \in \mathcal{G}_T}  \Big| \sum_{t=1}^T w_{t,T}(u,h)  \Big|$ using the Cacuhy Schwarz inequality:
\begin{align*}
\Big| \sum_{t=1}^T w_{t,T}(u,h)   \Big| & = \Big| \sum_{t=\lfloor T(u-h) \rfloor}^{\lceil T(u+h) \rceil} w_{t,T}(u,h)   \Big| \leq \sqrt{\sum_{t=\lfloor T(u-h) \rfloor}^{\lceil T(u+h) \rceil} w^2_{t,T}(u,h)}\sqrt{\sum_{t=\lfloor T(u-h) \rfloor}^{\lceil T(u+h) \rceil} 1^2} =\\
&=1 \cdot \sqrt{\lceil T(u+h) \rceil -  \lfloor T(u-h) \rfloor +1 } \leq \sqrt{2Th + 2} \leq \sqrt{2Th_{\max} +2}.
\end{align*}
...
Now, back to the first summand in \eqref{ineq-diff-1}. 

Similarly as before, by our assumptions $\widehat{\sigma}_i^2 = \sigma_i^2 + o_P(\rho_T)$. Hence, $$\max_{1\le i < j \le n}\{\widehat{\sigma}_i^2+ \widehat{\sigma}_j^2 \}^{-1/2}  = O_P(1).$$

Then, by Theorem \ref{theo-regs}, we know that $|\bm{\beta}_i - \widehat{\bm{\beta}}_i| = O_P(1/\sqrt{T})$.


Consider the term $\big| \sum_{t=1}^T w_{t,T}(u,h) \mathbf{X}_{it}  \big|$. Without loss of generality, we can regard the covariates $\mathbf{X}_{it}$ to be scalars $X_{it}$, not vectors. The proof in case of vectors proceeds analogously.
...
&= 1.
\end{align*}
Denoting by $D_{T, u, h}$ the number of integers between $\lfloor T(u-h) \rfloor$ and $\lceil T(u+h) \rceil$ incl. (with obvious bounds $2Th \leq D_{T, u, h} \leq 2Th + 2$), we can rewrite the weights as follows:
\begin{align*}
\sum_{t=\lfloor T(u-h) \rfloor}^{\lceil T(u+h) \rceil} \big(\sqrt{D_{T, u, h}}\cdot w_{t,T}(u,h)\big)^2 = D_{T, u, h}.
\end{align*}

According to Theorem \ref{theo-wu} (Theorem 2 in \cite{Wu2016}), using the weights $a_t = \sqrt{D_{T, u, h}}\cdot w_{t,T}(u,h)$ we can bound the following probability:
\begin{align*}
...
We want to prove that $\max_{(u, h) \in \mathcal{G}_T} \Big| \sum_{t=\lfloor T(u-h) \rfloor}^{\lceil T(u+h) \rceil} w_{t,T}(u,h)X_{it}  \Big| = o_P(\sqrt{T})$. For that, take any $\delta>0$:
\begin{align*}
...
&\leq C_1 \frac{ T^\theta ||X_{i\cdot}||^{q^\prime}_{q^\prime, \alpha}}{T^{q^\prime/2}\cdot \delta^{q^\prime}} \max_{(u, h) \in \mathcal{G}_T} \left( \sum_{t=\lfloor T(u-h) \rfloor}^{\lceil T(u+h) \rceil} |w_{t,T}(u,h)|^{q^\prime}\right)+ C_2 T^\theta \exp \left(-\frac{C_3 \delta^2 T}{D_{T, u, h}||X_{i\cdot}||^{2}_{2, \alpha}}\right) = \\
&\leq C \frac{ T^{\theta - q^\prime/2}}{\delta^{q^\prime}} + C T^\theta \exp \left(-C T \delta^2\right).
\end{align*}
where in the last equality we used the following facts:
\begin{enumerate}
	\item $||X_{i\cdot}||^{q^\prime}_{q^\prime, \alpha} < \infty$ by the assumption \ref{C-reg5};
	\textcolor{red}{\item $\max_{(u, h) \in \mathcal{G}_T} \left( \sum_{t=\lfloor T(u-h) \rfloor}^{\lceil T(u+h) \rceil} |w_{t,T}(u,h)|^{q^\prime}\right) < \infty$; (still need to fomally write down)}
	\item $||X_{i\cdot}||^{2}_{2, \alpha} < \infty$ (follows from $(1)$).
\end{enumerate}
If we take $\theta - q^\prime/2 <0$, then the term on the RHS is converging to can $0$ as $T \to \infty$ for any fixed $\delta >0$. Hence, 
\begin{align*}
\max_{(u, h) \in \mathcal{G}_T} \Big| \sum_{t=\lfloor T(u-h) \rfloor}^{\lceil T(u+h) \rceil} w_{t,T}(u,h)X_{it}  \Big| = o_P(\sqrt{T}).
\end{align*}

Plugging all the equations derived above together, we can establish that the first summand is also $o_P(1)$. Hence, $\big|\doublehattwo{\Phi}_{n,T} - \widehat{\Phi}_{n,T}\big| = o_P(1)$ and the statement of the theorem follows.
\end{proof}

\begin{propA}\label{propA-final}
Under the conditions of Theorem \ref{theo-stat-equality}, it holds that 
\[ \sup_{x \in \reals} \big| \pr(\widehat{\Phi}_{n, T} \le x) - \pr(\Phi_{n,T} \le x) \big| = o(1). \]
\end{propA}
\begin{proof}
First, we consider those $x\in \reals$ such that $\pr(\widehat{\Phi}_{n,T} \le x) \geq \pr(\Phi_{n, T} \le x)$. Then by Proposition \ref{propA-intermediate-relation} and taking $\gamma_{n, T}$ from the Proposition \ref{propA-intermediate-relation-2} we have
\begin{align*}
...
Now consider such $x\in \reals$ that $\pr(\widehat{\Phi}_{n,T}\le x) < \pr(\Phi_{n, T}\le x)$. Then again by Proposition \ref{propA-intermediate-relation} for $\gamma_{n, T}$ from Proposition \ref{propA-intermediate-relation-2} we have
...
\end{align*}
Now note that since $\gamma_{n,T} \to 0$, we can use the anticoncentration results for the Gaussian statistic ${\Phi}_{n,T}$ to get $\pr\Big(\big|{\Phi}_{n,T} - x \big| \le \gamma_{n, T}\Big) = o(1)$. Thus, 
\begin{align*}
...
%\end{align*}
%Now according to \ref{C-reg1}, we have $ E X_{it}^2 = E X_{i0}^2$. Moreover, by the Cauchy-Scwarz inequality 
%\begin{align*}
%\big|E (X_{it}\bar{X}_i)\big| &= \Big|\frac{1}{T} \sum_{s=1}^T E (X_{it} X_{is}) \Big| \leq \frac{1}{T} \sum_{s=1}^T \big| E (X_{it} X_{is}) \big| \leq \\
%&\leq \frac{1}{T} \sum_{s=1}^T \sqrt{E X_{it}^2 E X_{is}^2} = \frac{1}{T} \sum_{s=1}^T E X_{i0}^2  = E X_{i0}^2
%\end{align*}
%and
%\begin{align*}
