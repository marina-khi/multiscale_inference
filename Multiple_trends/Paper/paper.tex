...
\item \label{C-reg5} \textcolor{red}{Dependence adjusted norm!}
\end{enumerate}
...
\item \label{C-h} $h_{\min} \gg T^{-(1-\frac{2}{q})} \log T$, that is, $h_{\min} / \{ T^{-(1-\frac{2}{q})} \log T \} \rightarrow \infty$ with $q > 4$ defined in \ref{C-err2} and $h_{\max} \leq 1/2$.
...
%\subsection*{Auxiliary results concerning the local linear estimators}
%Consider the local linear kernel weights $w_{t,T}(u,h)$ defined in \eqref{weights}. We can rewrite them as follows.
%\begin{align*}
...
Finally, by Cauchy-Schwarz inequliaty 
\begin{align*}
 \sum_{t=1}^T w_{t,T}(u,h)  = \sum_{t=1}^T\frac{ K\Big(\frac{\frac{t}{T}-u}{h}\Big) \Big[ S_{T,2}(u,h) - \Big(\frac{\frac{t}{T}-u}{h}\Big) S_{T,1}(u,h) \Big]}{ \bigg\{\sum\nolimits_{s=1}^T  K^2\Big(\frac{\frac{s}{T}-u}{h}\Big) \Big[ S_{T,2}(u,h) - \Big(\frac{\frac{s}{T}-u}{h}\Big) S_{T,1}(u,h) \Big]^2 \bigg\}^{1/2} }
\end{align*}

\textcolor{red}{Prove here that the value above is $o(\sqrt{T})$!!!}
...
\begin{enumerate}
	\item $||X_{i\cdot}||^{q^\prime}_{q^\prime, \alpha} < \infty$;
	\textcolor{red}{\item $\max_{(u, h) \in \mathcal{G}_T} \left( \sum_{t=\lfloor T(u-h) \rfloor}^{\lceil T(u+h) \rceil} |w_{t,T}(u,h)|^{q^\prime}\right) < \infty$;}
...
\end{propA}
\begin{proof}
First, we consider those $x\in \reals$ such that $\pr(\widehat{\Phi}_{n,T} \le x) \geq \pr(\Phi_{n, T} \le x)$. Then by Proposition \ref{propA-intermediate-relation} we have
\begin{align*}
...
Now consider such $x\in \reals$ that $\pr(\widehat{\Phi}_{n,T}\le x) < \pr(\Phi_{n, T}\le x)$. Then again by Proposition \ref{propA-intermediate-relation} for $\gamma_{n, T} = C h_{max}$ we have
...
\end{align*}
Now note that since $\gamma_{n,T} \to 0$ by \ref{C-h}, we can use the anticoncentration results for the Gaussian statistic ${\Phi}_{n,T}$ to get $\pr\Big(\big|{\Phi}_{n,T} - x \big| \le \gamma_{n, T}\Big) = o(1)$. Thus, 
\begin{align*}
