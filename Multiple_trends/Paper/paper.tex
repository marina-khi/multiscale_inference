...
\begin{align*}
Y_{ij, 2k-1}^\prime & = \frac{\pseudogaussiankernel_{ij, T}(x_{k1},x_{k2})}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}} - \lambda(x_{k2}) \\
Y_{ij, 2k}^\prime & = -\frac{\pseudogaussiankernel_{ij, T}(x_{k1},x_{k2})}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}} - \lambda(x_{k2}) 
\end{align*}
with $x_k = (x_{k1},x_{k2})$. This notation allows us to write
\[ \pseudogaussianstat_{n, T} = \max_{1\le i < j \le n} \max_{1 \le k \le p} \{Y_{ij, 2k-1}^\prime, Y_{ij, 2k}^\prime \} = \max_{1 \leq l \leq (n-1)np} Y_l\]
where $(Y_{1},\ldots,Y_{(n-1)np})^\top \in \reals^{n(n-1)p}$ is a Gaussian random vector conditional on $\{\X_{it}\}_{i, t}$ with the following properties: 
\[\mu_{l}|\X_{it} := \ex\left[Y_l|\X_{it}\right] = \{\ex[Y^\prime_{ij, 2k}|\X_{it}] \text{ or }\ex[Y^\prime_{ij, 2k-1}|\X_{it}]\}= - \lambda(x_{k2}) \]
and by the LIE 
\[\mu_{l} := \ex\left[Y_l\right] = \ex\left[\ex\left[Y_l|\X_{it}\right]\right] = - \lambda(x_{k2}). \]
Now consider the following random value which we will need to calculate the entries of the covariance matrix:
\begin{align*}
...
&=	\ex\left[\frac{\widehat{\sigma}_i \widehat{\sigma}_{i^\prime}}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\right] \ex \left[\left(\sum\nolimits_{t=1}^T w_{t,T}(u,h) \, (Z_{it} - \bar{Z}_i)\right)\left( \sum\nolimits_{t=1}^T w_{t,T}(u,h) \,  (Z_{{i^\prime}t} - \bar{Z}_{i^\prime}) \right)\right]
\end{align*}
and
\begin{align*}
&\ex\left[\frac{\widehat{\sigma}_i\widehat{\sigma}_{i^\prime}\left(\sum\nolimits_{t=1}^T w_{t,T}(u,h) \, (Z_{it} - \bar{Z}_i)\right) \left( \sum\nolimits_{t=1}^T w_{t,T}(u,h) \,  (Z_{{i^\prime}t} - \bar{Z}_{i^\prime}) \right) }
{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\bigg| \X_{it}\right] =\\
&=	\frac{\widehat{\sigma}_i \widehat{\sigma}_{i^\prime}}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\ex \left[\left(\sum\nolimits_{t=1}^T w_{t,T}(u,h) \, (Z_{it} - \bar{Z}_i)\right)\left( \sum\nolimits_{t=1}^T w_{t,T}(u,h) \,  (Z_{{i^\prime}t} - \bar{Z}_{i^\prime}), \right)\right]
\end{align*}
...
the entries of the conditional covariance matrices are
\begin{align*}
\Sigma^Y_{l l^\prime}|\X_{it} :&= \ex[Y_l Y_{l^\prime} | \X_{it}] - \ex[Y_l|\X_{it}]\ex[Y_{l^\prime}|\X_{it}] = \\
&= \ex[Y^\prime_{ij, k} Y^\prime_{i^\prime j^\prime, k^\prime} | \X_{it}] - \ex[Y^\prime_{ij, k}|\X_{it}]\ex[Y^\prime_{i^\prime j^\prime, k^\prime}|\X_{it}] =\\
&=\ex \left[\left(\pm \frac{\pseudogaussiankernel_{ij, T}(x_{k1},x_{k2})}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}} - \lambda(x_{k2})\right) \left(\pm \frac{\pseudogaussiankernel_{i^\prime j^\prime , T}(x_{k^\prime 1},x_{k^\prime 2})}{\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}} - \lambda(x_{k^\prime 2})\right) \bigg|\X_{it}\right] - \lambda(x_{k2}) \lambda(x_{k^\prime 2}) = \\
&=\pm \ex \left[\frac{\pseudogaussiankernel_{ij, T}(x_{k1},x_{k2})}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}} \frac{\pseudogaussiankernel_{i^\prime j^\prime , T}(x_{k^\prime 1},x_{k^\prime 2})}{\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}} \bigg|\X_{it}\right] + \lambda(x_{k2}) \lambda(x_{k^\prime 2})- \lambda(x_{k2}) \lambda(x_{k^\prime 2}) =\\
&=\pm \frac{\ex \left[\pseudogaussiankernel_{ij, T}(x_{k1},x_{k2})\pseudogaussiankernel_{i^\prime j^\prime , T}(x_{k^\prime 1},x_{k^\prime 2}) \big|\X_{it}\right]}
{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}  =\\
&=\pm \frac{\ex \left[\sum\nolimits_{t=1}^T w_{t,T}(u,h) \, \big\{ \widehat{\sigma}_i (Z_{it} - \bar{Z}_i) - \widehat{\sigma}_j (Z_{jt} - \bar{Z}_j) \big\}\sum\nolimits_{t=1}^T w_{t,T}(u,h) \, \big\{ \widehat{\sigma}_i (Z_{it} - \bar{Z}_i) - \widehat{\sigma}_j (Z_{jt} - \bar{Z}_j) \big\}\big|\X_{it}\right]}
{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}} =\\
&= \begin{cases}
0, \text{ if } i \neq i^\prime, j^\prime \text{ and } j \neq i^\prime, j^\prime;\\
\pm \frac{\ex \left[\pseudogaussiankernel^2_{ij, T}(x_{k1},x_{k2})\big|\X_{it}\right]}
{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2}, \text{ if } i = i^\prime \text{ and } j = j^\prime;\\
 \text{ if } i = i^\prime \text{ but } j \neq j^\prime
\end{cases} 
\end{align*}
for all $1 \leq l \leq (n-1)np$. Hence, $a_{(n-1)np} = b_{(n-1)np}$. Moreover, as the variables $(U^\prime_l - \mu_l)/\sigma_l$ are standard normal, we have that $a_{(n-1)np} = b_{(n-1)np} \le C\sqrt{\log ((n-1)np)} \leq C \sqrt{\log T}$. With this notation at hand, we can apply Proposition ?? to obtain that 
\[ \sup_{x \in \reals} \pr \Big( \big| \gaussianstat_{n, T} - x \big| \le \delta_T \Big) \le C \delta_T \Big[ \sqrt{\log T} + \sqrt{ \log(1/\delta_T) } \Big] = o(1) \]
with $\delta_T = T^{1/q} / \sqrt{T h_{\min}} + \rho_T \sqrt{\log T}$, which is the statement of Proposition \ref{propA-anticon-equality}.
