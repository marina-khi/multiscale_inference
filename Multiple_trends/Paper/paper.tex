...
which means that $\Phi_{ij,T}$ does not depend on any unknown quantities such as $\sigma^2$ and the distribution of this random valuable is fully known. However, we will stick to the notation in \eqref{Phi-ij-statistic} for the sake of similarity to $\widehat{\Psi}_{ij,T}$.
...
\item The Gaussian statistics that is used to calculate the critical values (defined in \eqref{Phi-statistic}):
\begin{align}\label{eq-stat-6}
	\gaussianstat_{n,T}  &= \max_{1 \le i < j \le n} \gaussianstat_{ij,T}, \nonumber\\
      	\gaussianstat_{ij,T} &= \max_{(u,h) \in \mathcal{G}_T} \Big\{ \Big|\frac{\gaussiankernel_{ij,T}(u,h)}{({\sigma}_i^2 + {\sigma}_j^2)^{1/2}}\Big| - \lambda(h) \Big\},\\
...
\item In the fifth step, by the means of Proposition \ref{propA-gaussian-relation} we prove that  
\begin{equation*}
\sup_{x \in \reals} \big| \pr(\widehat{\Phi}_{n, T} \le x) - \pr(\gaussianstat_{n,T} \le x) \big| = o(1).
\end{equation*}
\item And finally, in Proposition \ref{propA-pseudogaussian-relation} we connect the quantiles of the true Gaussian statistics $\gaussianstat_{n, T}$ (that can not be simulated because they include the unknown parameters $\sigma_i^2$) to the quantiles of the ``Gaussian'' statistic $\pseudogaussianstat_{n, T}$ (that can be simulated and used for testing):
\begin{equation*}
...
\textcolor{red}{Here we need conditional distribution for the ``Gaussian'' statistics! NOT DONE YET}
\end{enumerate}
...
\max_{(u,h) \in \mathcal{G}_T}\left|\doublehat{\phi}_{ij, T}(u, h)\right| =o_P(\sqrt{\log{T}})
\end{align}
...
\end{split}
\end{align}
since by our assumption $\rho_T = O(\sqrt{h_{\min}}/\log T)$. Plugging \eqref{ineq-diff-5}, \eqref{ineq-diff-9} and \eqref{ineq-diff-13} in \eqref{ineq-diff-1}, we get that $\big|\doublehattwo{\Phi}_{n,T} - \widehat{\Phi}_{n,T}\big| = o_P(1)$ and the statement of the theorem follows.
\end{proof}
...
It is clear that we can bound $\big| \pseudogaussianstat_{n, T} - \gaussianstat_{n,T} \big|$ as follows
\begin{align}\label{eq-pseudogaussian-relation-1}
\begin{split}
\big| \pseudogaussianstat_{n, T} - \gaussianstat_{n,T} \big| \leq &  \max_{1 \le i < j \le n} \max_{(u,h) \in \mathcal{G}_T} \bigg|\frac{\gaussiankernel_{ij,T}(u,h)}{(\sigma_i^2 + \sigma_j^2)^{1/2}} - \frac{\gaussiankernel_{ij,T}(u,h)}{(\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2)^{1/2}}\bigg|+\\
...
\begin{align}\label{eq-pseudogaussian-relation-3}
\begin{split}
\max_{1 \le i < j \le n} &\max_{(u,h) \in \mathcal{G}_T}\bigg| \frac{\gaussiankernel_{ij,T}(u,h)}{(\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2)^{1/2}}- \frac{\pseudogaussiankernel_{ij,T}(u,h)}{(\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2)^{1/2}}\bigg| = \\
...
\begin{align}\label{eq-pseudogaussian-relation-4}
...
\textcolor{red}{Since by our assumption we have $\rho_T = O(\sqrt{h_{\min}}/\log T)$, by plugging \eqref{eq-pseudogaussian-relation-4} in \eqref{eq-pseudogaussian-relation-3} we can bound the second part in \eqref{eq-pseudogaussian-relation-1}:}
\begin{align*}
\max_{1 \le i < j \le n} &\max_{(u,h) \in \mathcal{G}_T}\bigg| \frac{\gaussiankernel_{ij,T}(u,h)}{(\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2)^{1/2}}- \frac{\pseudogaussiankernel_{ij,T}(u,h)}{(\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2)^{1/2}}\bigg| = O_P(1) \cdot o_P \left(\frac{\rho_T\log T}{\sqrt{h_{\min}}} \right) = o_P \left(\frac{\rho_T\log T}{\sqrt{h_{\min}}} \right),
\end{align*}
which together with \eqref{eq-pseudogaussian-relation-2} yileds the following
\begin{align}\label{eq-pseudogaussian-relation-5}
\begin{split}
\big| \pseudogaussianstat_{n, T} - \gaussianstat_{n,T} \big| = o_P(\rho_T \sqrt{\log T}) + o_P \left(\frac{\rho_T\log T}{\sqrt{h_{\min}}} \right) = o_P \left(\frac{\rho_T\log T}{\sqrt{h_{\min}}} \right).
\end{split}
\end{align}

Now we can apply Lemma \ref{lemma1-theo-stat} to $\pseudogaussianstat_{n, T}$ and $\gaussianstat_{n,T}$ together with the results \eqref{eq-pseudogaussian-relation-5} derived above and \eqref{eq-anticon-equality} (anticoncentration result for the true Gaussian statistics $\gaussianstat_{n, T}$), which leads us to the desired conclusion
\begin{equation*}
\sup_{x \in \reals} \big| \pr(\pseudogaussianstat_{n, T} \le x) - \pr(\gaussianstat_{n,T} \le x) \big| = o(1). 
\end{equation*}
\end{proof}
...
\begin{propA}\label{propA-pseudogaussian}
Under the conditions of Theorem \ref{theo-stat-equality}, it holds that 
\begin{equation}\label{eq-pseudogaussian}
\sup_{x \in \reals} \big| \pr(\pseudogaussianstat_{n, T} \le x | \X_{it} ) - \pr(\pseudogaussianstat_{n,T} \le x) \big| = ??.
\end{equation}
\end{propA}
\begin{proof}[\textnormal{\textbf{Proof of Proposition \ref{propA-pseudogaussian}}}] 
This proof will be based on Lemma \ref{lemma-chern} (Lemma 3.1 in \cite{Chernozhukov2013}).

We proceed in the same way as in the proof of Proposition \ref{propA-anticon-equality}. We write $x = (u,h)$ along with $\mathcal{G}_T = \{ x : x \in \mathcal{G}_T \} = \{x_1,\ldots,x_p\}$, where $p := |\mathcal{G}_T| \le O(T^\theta)$ for some large but fixed $\theta > 0$ by Assumption \ref{C-grid}. Moreover, for $k = 1,\ldots,p$, we set 
\begin{align*}
Y_{ij, 2k-1} & = \frac{\pseudogaussiankernel_{ij, T}(x_{k1},x_{k2})}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}}, \\
Y_{ij, 2k} & = -\frac{\pseudogaussiankernel_{ij, T}(x_{k1},x_{k2})}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}} 
\end{align*}
with $x_k = (x_{k1},x_{k2})$. This notation allows us to write
\[ \pseudogaussianstat_{n, T} = \max_{1\le i < j \le n} \max_{1 \le k \le p} \{Y_{ij, 2k-1}^\prime - \lambda(x_{k2}), Y_{ij, 2k}^\prime - \lambda(x_{k2}) \}\]
where $(Y_{12, 1},\ldots,Y_{(n-1)n, 2p})^\top \in \reals^{n(n-1)p}$ is a centered Gaussian random vector conditional on $\{\X_{it}\}_{i, t}$ with the following properties: 
\[\ex\left[Y_{ij, 2k-1}|\X_{it}\right] = \ex[Y_{ij, 2k}|\X_{it}] = 0 \]
and by the LIE 
\[\ex\left[Y_{ij, 2k-1}\right] = \ex\left[Y_{ij, 2k}\right]  = 0. \]
Now consider the following random value which we will need further to calculate the entries of the covariance matrix:
\begin{align*}
&\frac{\pseudogaussiankernel_{ij, T}(x_{k1},x_{k2})}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}} \frac{\pseudogaussiankernel_{i^\prime j^\prime , T}(x_{k^\prime 1},x_{k^\prime 2})}{\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}} =\\
&= \frac{\sum_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \, \big\{ \widehat{\sigma}_i (Z_{it} - \bar{Z}_i) - \widehat{\sigma}_j (Z_{jt} - \bar{Z}_j) \big\}\sum_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \, \big\{ \widehat{\sigma}_{i^\prime} (Z_{{i^\prime}t} - \bar{Z}_{i^\prime}) - \widehat{\sigma}_{j^\prime} (Z_{{j^\prime}t} - \bar{Z}_{j^\prime}) \big\}}
{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}} = \\
&= \frac{\widehat{\sigma}_i\widehat{\sigma}_{i^\prime}\left(\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \, (Z_{it} - \bar{Z}_i)\right) \left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{i^\prime}t} - \bar{Z}_{i^\prime}) \right) }
{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}} - \\
&\quad - \frac{\widehat{\sigma}_j\widehat{\sigma}_{i^\prime}\left(\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \, (Z_{jt} - \bar{Z}_j)\right) \left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{i^\prime}t} - \bar{Z}_{i^\prime}) \right) }
{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}} - \\
&\quad - \frac{\widehat{\sigma}_i\widehat{\sigma}_{j^\prime}\left(\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \, (Z_{it} - \bar{Z}_i)\right) \left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{j^\prime}t} - \bar{Z}_{j^\prime}) \right) }
{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}} + \\
&\quad + \frac{\widehat{\sigma}_j\widehat{\sigma}_{j^\prime}\left(\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \, (Z_{jt} - \bar{Z}_j)\right) \left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{j^\prime}t} - \bar{Z}_{j^\prime}) \right) }
{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}.
\end{align*}
Now remember that $Z_{it}$ are independent of $\X_{js}$ for all possible combinations of $i, j, s$ and $t$. Hence, we write the unconditional and conditional expectations as
\begin{align*}
&\ex\left[\frac{\widehat{\sigma}_i\widehat{\sigma}_{i^\prime}\left(\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \, (Z_{it} - \bar{Z}_i)\right) \left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{i^\prime}t} - \bar{Z}_{i^\prime}) \right) }{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\right] =\\
&=	\ex\left[\frac{\widehat{\sigma}_i \widehat{\sigma}_{i^\prime}}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\right] \ex \left[\sum_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \, (Z_{it} - \bar{Z}_i)\sum_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{i^\prime}t} - \bar{Z}_{i^\prime}) \right]
\end{align*}
and
\begin{align*}
&\ex\left[\frac{\widehat{\sigma}_i\widehat{\sigma}_{i^\prime}\left(\sum\nolimits_{t=1}^T w_{t,T}(u,h) \, (Z_{it} - \bar{Z}_i)\right) \left( \sum\nolimits_{t=1}^T w_{t,T}(u,h) \,  (Z_{{i^\prime}t} - \bar{Z}_{i^\prime}) \right) }
{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\bigg| \X_{it}\right] =\\
&=	\frac{\widehat{\sigma}_i \widehat{\sigma}_{i^\prime}}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\ex \left[\left(\sum_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \, (Z_{it} - \bar{Z}_i)\right)\left( \sum_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{i^\prime}t} - \bar{Z}_{i^\prime}), \right)\right]
\end{align*}
respectively. Consider two cases:
\begin{enumerate}
\item $i \neq i^\prime$. Then 
\begin{align*}
&\ex \left[\left(\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \, (Z_{it} - \bar{Z}_i)\right)\left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{i^\prime}t} - \bar{Z}_{i^\prime}) \right)\right] = \\
&\quad = \ex \left[\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \, (Z_{it} - \bar{Z}_i)\right] \ex \left[ \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{i^\prime}t} - \bar{Z}_{i^\prime})\right] =\\
&\quad = 0
\end{align*}
and
\begin{align*}
&\ex\left[\frac{\widehat{\sigma}_i\widehat{\sigma}_{i^\prime}\left(\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \, (Z_{it} - \bar{Z}_i)\right) \left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{i^\prime}t} - \bar{Z}_{i^\prime}) \right) }
{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\right] =\\
& =\ex\left[\frac{\widehat{\sigma}_i\widehat{\sigma}_{i^\prime}\left(\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \, (Z_{it} - \bar{Z}_i)\right) \left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{i^\prime}t} - \bar{Z}_{i^\prime}) \right) }
{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\bigg| \X_{it}\right] =\\
&=	0.
\end{align*}
...
\item $i = i^\prime$. Then
\begin{align*}
&\ex \left[\left(\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \, (Z_{it} - \bar{Z}_i)\right)\left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{i}t} - \bar{Z}_{i}) \right)\right] = \\
&\quad = \ex \left[\sum_{t=1}^T w_{t,T}(x_{k1},x_{k2})w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \, (Z_{it} - \bar{Z}_i)^2\right] +\\
&\quad \quad + \ex \left[\sum_{t, s =1, t\neq s}^T w_{t,T}(x_{k1},x_{k2})w_{s,T}(x_{k^\prime 1},x_{k^\prime 2}) \, (Z_{it} - \bar{Z}_i)(Z_{is} - \bar{Z}_i)\right] =\\
&\quad =\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) w_{t, T}(x_{k^\prime 1},x_{k^\prime 2}) \ex \left[Z_{it}^2 \right] -\\
&\quad \quad - 2\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) w_{t, T} (x_{k^\prime 1},x_{k^\prime 2}) \ex \left[Z_{it}\bar{Z}_i \right] +\\
&\quad \quad + \quad \sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) w_{t, T}(x_{k^\prime 1},x_{k^\prime 2}) \ex \left[\bar{Z}_{i}^2  \right] + \\
&\quad \quad + \sum_{t, s =1, t\neq s}^T w_{t,T}(x_{k1},x_{k2})w_{s,T}(x_{k^\prime 1},x_{k^\prime 2})\ex [Z_{it} Z_{is}] -\\
&\quad \quad - \sum_{t, s =1, t\neq s}^T w_{t,T}(x_{k1},x_{k2})w_{s,T}(x_{k^\prime 1},x_{k^\prime 2})\ex [\bar{Z}_{i} Z_{is}] -\\
&\quad \quad - \sum_{t, s =1, t\neq s}^T w_{t,T}(x_{k1},x_{k2})w_{s,T}(x_{k^\prime 1},x_{k^\prime 2})\ex [Z_{it} \bar{Z}_{i}] +\\
&\quad \quad +  \sum_{t, s =1, t\neq s}^T w_{t,T}(x_{k1},x_{k2})w_{s,T}(x_{k^\prime 1},x_{k^\prime 2})\ex [\bar{Z}_{i}^2].
\end{align*}
Since $\{Z_{it}\}_{t}$ are i.i.d. standard normal random variables, we have obvious equalities: $\ex \left[Z_{it}^2 \right] = 1$, $\ex \left[\bar{Z}_{i}^2\right] = 1/T$, $\ex [Z_{it} Z_{is}] = \ex[Z_{it}] \ex[Z_{is}] = 0$ for $t \neq s$ and $\ex [Z_{it} \bar{Z}_{i}] = 1/T$. Then, 
\begin{align*}
&\ex \left[\left(\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \, (Z_{it} - \bar{Z}_i)\right)\left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{i}t} - \bar{Z}_{i}) \right)\right] = \\
&\quad =\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) w_{t, T}(x_{k^\prime 1},x_{k^\prime 2}) - \frac{2}{T}\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2})w_{t, T}(x_{k^\prime 1},x_{k^\prime 2}) +\\
&\quad \quad +  \frac{1}{T}\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2})w_{t,T}(x_{k^\prime 1},x_{k^\prime 2})  - \\
&\quad \quad  -\frac{2}{T}\sum_{t, s =1, t\neq s}^T w_{t,T}(x_{k1},x_{k2})w_{s,T}(x_{k^\prime 1},x_{k^\prime 2}) +\\
&\quad \quad + \frac{1}{T}\sum_{t, s =1, t\neq s}^T w_{t,T}(x_{k1},x_{k2})w_{s,T}(x_{k^\prime 1},x_{k^\prime 2}) = \\
&\quad =  \sum_{t=1}^T w_{t,T}(x_{k1},x_{k2}) w_{t, T}(x_{k^\prime 1},x_{k^\prime 2}) - \frac{1}{T} \left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \right)\left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \right).
\end{align*}
For ease of notation, we denote the above value by $\mathcal{W}_{k, k^\prime, T}$. Then,
\begin{align*}
&\ex\left[\frac{\widehat{\sigma}_i\widehat{\sigma}_{i^\prime}\left(\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \, (Z_{it} - \bar{Z}_i)\right) \left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{i^\prime}t} - \bar{Z}_{i^\prime}) \right) }
{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\right] =\\
&=	\ex\left[\frac{\widehat{\sigma}_i^2}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\right] \cdot \mathcal{W}_{k, k^\prime, T}.
\end{align*}
and
\begin{align*}
&\ex\left[\frac{\widehat{\sigma}_i\widehat{\sigma}_{i^\prime}\left(\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \, (Z_{it} - \bar{Z}_i)\right) \left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{i^\prime}t} - \bar{Z}_{i^\prime}) \right) }
{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\bigg| \X_{it}\right] =\\
&=	\frac{\widehat{\sigma}_i^2}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\cdot \mathcal{W}_{k, k^\prime, T}.
\end{align*}


\end{enumerate}
Now we are ready to calculate the elements of the unconditional and conditional covariance matrices. In what follows, we consider the case of $Y_{i, j, 2k-1}$. The case $Y_{i, j, 2k}$ is calculated analogously, the only difference being the sign.
\begin{align*}
&\Sigma^Y_{(ij, 2k-1), (i^\prime j^\prime, 2k^\prime-1)}|\X_{it} :=  \ex[Y_{ij, 2k-1} Y_{i^\prime j^\prime, 2k^\prime-1} | \X_{it}] - \ex[Y_{ij, 2k-1}|\X_{it}]\ex[Y_{i^\prime j^\prime, 2k^\prime-1}|\X_{it}] =\\
&=\ex \left[\frac{\pseudogaussiankernel_{ij, T}(x_{k1},x_{k2})}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}} \frac{\pseudogaussiankernel_{i^\prime j^\prime , T}(x_{k^\prime 1},x_{k^\prime 2})}{\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}} \bigg|\X_{it}\right] =\\
& = \ex \left[\frac{\widehat{\sigma}_i\widehat{\sigma}_{i^\prime}\left(\sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \, (Z_{it} - \bar{Z}_i)\right) \left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{i^\prime}t} - \bar{Z}_{i^\prime}) \right) }{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}} \bigg|\X_{it}\right]- \\
&\quad - \ex \left[\frac{\widehat{\sigma}_j\widehat{\sigma}_{i^\prime}\left(\sum\nolimits_{t=1}^T w_{t,T}(x_{k 1},x_{k 2}) \, (Z_{jt} - \bar{Z}_j)\right) \left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{i^\prime}t} - \bar{Z}_{i^\prime}) \right) }{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\bigg|\X_{it}\right] - \\
&\quad - \ex \left[\frac{\widehat{\sigma}_i\widehat{\sigma}_{j^\prime}\left(\sum\nolimits_{t=1}^T w_{t,T}(x_{k 1},x_{k2}) \, (Z_{it} - \bar{Z}_i)\right) \left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{j^\prime}t} - \bar{Z}_{j^\prime}) \right) }{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}} \bigg|\X_{it}\right]+ \\
&\quad + \ex \left[\frac{\widehat{\sigma}_j\widehat{\sigma}_{j^\prime}\left(\sum\nolimits_{t=1}^T w_{t,T}(x_{k 1},x_{k2}) \, (Z_{jt} - \bar{Z}_j)\right) \left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k^\prime 1},x_{k^\prime 2}) \,  (Z_{{j^\prime}t} - \bar{Z}_{j^\prime}) \right) } {\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\bigg|\X_{it}\right]=\\
&= \begin{cases}
0, \text{ if } i \neq i^\prime, j^\prime \text{ and } j \neq i^\prime, j^\prime;\\
\mathcal{W}_{k, k^\prime, T}, \text{ if } i = i^\prime \text{ and } j = j^\prime;\\
\frac{\widehat{\sigma}_i^2}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}} \mathcal{W}_{k, k^\prime, T}, \text{ if } i = i^\prime \text{ but } j \neq j^\prime;\\
\frac{\widehat{\sigma}_j^2}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j}^2\}^{1/2}}\mathcal{W}_{k, k^\prime, T}, \text{ if } j = j^\prime \text{ but } i \neq i^\prime;\\
-\frac{\widehat{\sigma}_j^2}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{j}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}} \mathcal{W}_{k, k^\prime, T}, \text{ if } j = i^\prime \text{ but } i \neq j^\prime;\\
-\frac{\widehat{\sigma}_i^2}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{i}^2\}^{1/2}} \mathcal{W}_{k, k^\prime, T}, \text{ if } i = j^\prime \text{ but }  j\neq i^\prime
\end{cases} 
\end{align*}
since $i<j$ and $i^\prime < j^\prime$.
Similarly,
\begin{align*}
&\Sigma^Y_{(ij, 2k-1), (i^\prime j^\prime, 2k^\prime-1)} := \ex[Y_{ij, 2k-1} Y_{i^\prime j^\prime, 2k^\prime-1}] - \ex[Y_{ij, 2k-1}]\ex[Y_{i^\prime j^\prime, 2k^\prime-1}] =\\
&\quad= \begin{cases}
0, \text{ if } i \neq i^\prime, j^\prime \text{ and } j \neq i^\prime, j^\prime;\\
\mathcal{W}_{k, k^\prime, T}, \text{ if } i = i^\prime \text{ and } j = j^\prime;\\
\ex\left[\frac{\widehat{\sigma}_i^2}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\right]\cdot \mathcal{W}_{k, k^\prime, T}, \text{ if } i = i^\prime \text{ but } j \neq j^\prime;\\
\ex\left[\frac{\widehat{\sigma}_j^2}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{j}^2\}^{1/2}}\right]\cdot \mathcal{W}_{k, k^\prime, T}, \text{ if } j = j^\prime \text{ but } i \neq i^\prime;\\
-\ex\left[\frac{\widehat{\sigma}_j^2}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{j}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\right]\cdot \mathcal{W}_{k, k^\prime, T}, \text{ if } j = i^\prime \text{ but } i \neq j^\prime;\\
-\ex\left[\frac{\widehat{\sigma}_i^2}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i^\prime}^2 + \widehat{\sigma}_{i}^2\}^{1/2}}\right]\cdot \mathcal{W}_{k, k^\prime, T}, \text{ if } i = j^\prime \text{ but }  j\neq i^\prime.
\end{cases} 
\end{align*}
In order to apply Lemma \ref{lemma-chern}, we need to check that there exist two constants such that the elements on the diagonal of the unconditional covariance matrix are bounded from below and above by these constants. Specifically,
\begin{align*}
\Sigma_{(ij, 2k-1), (ij, 2k-1)} &= \mathcal{W}_{k, k, T} = \\
&= \sum_{t=1}^T w^2_{t,T}(x_{k1},x_{k2}) - \frac{1}{T} \left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \right)^2 = \\
& = 1 - \frac{1}{T} \left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \right)^2.
\end{align*}
Now remember that due to \eqref{weights-cauchy-schwarz}, for all $(u, h) \in \mathcal{G}_T$ we have
\begin{align*}
\Big| \sum_{t=1}^T w_{t,T}(u,h)  \Big| \leq \sqrt{2Th + 2} \leq \sqrt{2Th_{\max} +2}.
\end{align*}
 then we can bound the diagonal elements of the unconditional covariance matrix as follows:
\begin{align*}
1 - 2h_{\max} - 2/T \leq \Sigma_{(ij, 2k-1), (ij, 2k-1)} = 1 - \frac{1}{T} \left( \sum\nolimits_{t=1}^T w_{t,T}(x_{k1},x_{k2}) \right)^2 \leq 1.
\end{align*}
\textcolor{red}{Now we take $h_{\max} \leq 1/3$ (for example! anything smaller than 1/2 will do)}, then for all $T \ge 7$ and for all $1 \le i < j \le n$ and $k\in \{1, \ldots, p\}$ we have
\begin{align*}
0 < 1/21 \leq 1 - 2 h_{\max} - 2/T \leq \Sigma_{(ij, 2k-1), (ij, 2k-1)} \leq 1.
\end{align*}
Now we can calculate 
\begin{align*}
\Delta_0 := \max_{\substack{1 \leq i < j \leq n,\\ 1 \le i^\prime < j^\prime < n, 1 \le k \le p}} \big\{&\left|\Sigma^{Y}_{(ij, 2k-1), (i^\prime j^\prime, 2k^\prime-1)}|\X_{it} - \Sigma^{Y}_{(ij, 2k-1), (i^\prime j^\prime, 2k^\prime-1)}\right|,\\
&\left|\Sigma^{Y}_{(ij, 2k), (i^\prime j^\prime, 2k^\prime-1)}|\X_{it} - \Sigma^{Y}_{(ij, 2k), (i^\prime j^\prime, 2k^\prime-1)}\right|,\\
&\left|\Sigma^{Y}_{(ij, 2k-1), (i^\prime j^\prime, 2k^\prime)}|\X_{it} - \Sigma^{Y}_{(ij, 2k-1), (i^\prime j^\prime, 2k^\prime)}\right|,\\
&\left|\Sigma^{Y}_{(ij, 2k-1), (i^\prime j^\prime, 2k^\prime-1)}|\X_{it} - \Sigma^{Y}_{(ij, 2k-1), (i^\prime j^\prime, 2k^\prime-1)}\right|\big\} = \\
\max_{\substack{1 \leq i < j \leq n,\\ 1 \le j^\prime \le n, j^\prime \neq i, j^\prime \neq j}} &\left| \frac{\widehat{\sigma}_i^2}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}-  \ex\left[\frac{\widehat{\sigma}_i^2}{\{\widehat{\sigma}_i^2 + \widehat{\sigma}_j^2\}^{1/2}\{\widehat{\sigma}_{i}^2 + \widehat{\sigma}_{j^\prime}^2\}^{1/2}}\right] \right|\cdot  \max_{1 \le k, k^\prime \le p} |\mathcal{W}_{k, k^\prime, T}|
\end{align*}
\textcolor{red}{Here the first factor is $O_P(1)$ (since we are taking the maximum over the finite number of random values) and the second factor is $O(1)$ (definitely not converging to $0$ as was shown in the calculations directly above). So the question: where am I going wrong?}
\end{proof}
...
\begin{lemmaA}{\cite{Chernozhukov2013}}\label{lemma-chern}
Let $V$ and $Y$ be centered Gaussian random vectors in $\reals^p$ with covariance matrices $\Sigma^V$ and $\Sigma^Y$, respectively. Denote
\begin{align*}
\Delta_0 = \max_{1 \leq j, k \leq p} |\Sigma^V_{jk} - \Sigma^Y_{jk}|.
\end{align*}
Suppose that there are some constants $0<c_1< C_1$ such that $c_1 \leq \Sigma^Y_{jj} \leq C_1$ for all $1\leq j \leq p$. Then there exists a constant $C>0$ depending only on $c_1$ and $C_1$ such that
\begin{align*}
\sup_{t\in \reals} \left| \pr\left(\max_{1\leq j \leq p} V_j \leq t\right) -  \pr\left(\max_{1\leq j \leq p} Y_j \leq t\right) \right| \leq C \Delta_0^{1/3} \left(1 \vee \log(p/\Delta_0) \right)^{2/3}.
\end{align*} 
\end{lemmaA}
...
\]
Under Assumptions \ref{C-err2} - \ref{C-err3}, \ref{C-reg5} - \ref{C-reg-err2}, we have that $\sum_{s=0}^\infty \delta_2(\Delta \mathbf{U}_i, s) < \infty$.
\end{propA}
