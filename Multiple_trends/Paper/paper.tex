...
Suppose that the error processes $\mathcal{E}_i = \{ \varepsilon_{it}: 1 \le t \le T \}$ are independent across $i$ and satisfy \ref{C-err1}--\ref{C-err3} for each $i$. Moreover, let \ref{C-reg1}--\ref{C-h} be fulfilled and assume that $\widehat{\sigma}_i^2 = \sigma_i^2 + o_p(\rho_T)$ and $\doublehattwo{\sigma}_i^2 = \sigma_i^2 + o_p(\rho_T)$ with $\rho_T = o(\sqrt{h_{\min}}/\log T)$ (\textcolor{red}{RESTRICTION}) for each $i$. Then 
  ...
  \begin{center}
    ...
    \begin{tabular}{c|c|c|c|c|c} 
	 ...
	 $ \gaussianstat_{n, T}$ & & &&\begin{tabular}[c]{@{}l@{}} Lemma \ref{lemma1-theo-stat} \\ with the help of \\Prop. \ref{propA-strong-approx-equality} and \\ Prop. \ref{propA-anticon-equality}\end{tabular}&      \\
      \hline
	$ \pseudogaussianstat_{n, T}$    & & &&& \begin{tabular}[c]{@{}l@{}} Prop. \ref{propA-gaussian-relation} and \\
	Prop. \ref{propA-pseudogaussian-relation}\end{tabular} \\
        	\hline
    \end{tabular}
  \end{center}
\end{table}
