...
\item {\color<3>{mLightBrown}$Z_{it}$} are independent (across $i$ and $t$) standard normal RVs;\pause
...
\textbf{Dissimilarity measure:} let $S \subseteq \{1,\ldots, n\}$ and $S^\prime \subseteq \{1,\ldots, n\}$ be two sets of time series from our sample. We define a dissimilarity measure between $S$ and $S^\prime$ by setting 
\begin{equation*}
\widehat{\Delta}(S,S^\prime) = \max_{\substack{i \in S, \\ j \in S^\prime}} \Big({\color<4>{mLightBrown}\max_{(u, h) \in \mathcal{G}_T}}{\color<3-4>{mLightBrown}\hat{\psi}^0_{ij, T}(u, h)}\Big). 
\end{equation*}
\end{frame}
...
\begin{enumerate}
	\item For each pair of time series $(i, j)$, calculate the dissimilarity measure $\max_{(u, h) \in \mathcal{G}_T}\hat{\psi}^0_{ij, T}(u, h)$.
	\item Using the dissimilarity measure and the complete linkage method, perform HAC. The result is a tree of nested partitions:\linebreak  $\{\widehat{G}_1^{[r]},\ldots$ $\ldots,\widehat{G}_{n-r}^{[r]}\}$ for $r = 1,\ldots, n-1$.
