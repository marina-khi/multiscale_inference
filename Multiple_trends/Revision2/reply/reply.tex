...
\begin{enumerate}
	...
	\item \textbf{spurious} power, i.e., the percentage of simulation runs in which our test indicates difference either between $m_1$ and \textit{some} $m_i$ for $i = 2, \ldots, n$ on some interval $[u-h, u+h]$ where the difference is \textit{not} present, that is, on some interval $[u-h, u+h]$ such that $[u-h, u+h] \cap ([0.2, 0.4] \cup [0.6, 0.8]) = \emptyset$, or between $m_i$ and $m_j$ for some $i, j = 2, \ldots, n$ anywhere on the interval $[0, 1]$. The values of the actual power for different simulation scenarios are presented in Table \ref{tab:spuriouspower}.
\end{enumerate}
...
In order to better mimc behaviour of the stochastic volatility, we have included size and power simulation results with time-varying error variance. Specifically, we assume that for each $i$, the idiosyncratic error process $\{\varepsilon_{it}: 1 \le t \le T\}$ follows the AR(1) model $\varepsilon_{it} = a \varepsilon_{i t-1} + \sigma(t/T)\eta_{it}$, where $a = 0.25$, the innovations $\eta_{it}$ are i.i.d.\ normally distributed with mean $0$ and standard deviation $1$ and $\sigma^2(u) = -\frac{3}{8}(x-0.5)^2 + \frac{3}{32}$ for $u \in [0, 1]$ (see also Figure \ref{fig:sigma_TV} for $T=500$). The choice of the coefficients is driven by the requirement $\int_0^1 \sigma^2(u)du = 0.25^2$ which was introduced so that this simulation scenario is comparable to the ones with constant variance. The calculation of the critical values is also performed using the time-varying variance. All of the other parameters of the DGP remain the same. The results are presented in Table \ref{tab:sizepower_SV}.
