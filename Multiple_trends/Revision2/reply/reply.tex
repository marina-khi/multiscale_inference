...
\end{align*}
with $\vartheta(u,c,d) =  \ind \left\{\frac{|u - c|}{d}\leq 1 \right\} \left(1 - \frac{(u - c)^2}{d^2}\right)^2$ for $b \in \{ 0.25, 0.5, 0.75 \}$. The empirical size for two cases (with zero and bump functions under the null, respectively) is presented in Tbale \ref{tab:sizes}. {\color{red}Marina: The size numbers are a bit more conservative, it seems that the convergence is slower.}
...
Each panel gives the results for a diffe rent bump height $b$, where $b = 0.25,0.5,0.75$ corresponds to three different alternatives of $m_1$.}\label{tab:actualpower}
...
\footnotesize{
\begin{center}
\caption{Empirical size of the multiscale test for different time series lengths $T$ and nominal sizes $\alpha$ for different specification of $m_i$ under the null. The left panel gives the results for $m_i = 0$ for all $i$, whereas the right panel presents the results for $m_i$ being a bump function with height $0.5$. {\color{red}Marina: the left panel is one of the panels from the Supplement.}}\label{tab:sizes}
\begin{subtable}[b]{0.4\textwidth}
\centering  
...
\includegraphics{../output/1_vs_15_with_b_75.pdf}
  ...
  \caption{An example of two uniform confidence bands that do not overlap at least at one point ($T=100$ and $b=0.75$).}\label{fig:UCB}%
}
\end{floatrow}
\end{figure}
...
\item For each of the time series $i = 1, \ldots, n$, we calculate the UCB as described in \cite{Kim2016} using bandwidth $10/T$. {\color{red}Marina: optimal bandwidth using cross-validation was not that easy to calculate and the theoretical optimal bandwidths were $0.4, 0.33, 0.29$ for $T=100, 250, 500$, respectively.}.
...
As can be seen, for short sample size $T=100$, the naive competitor tend to reject in more than half of the cases. The reason for this can be seen in Figure \ref{fig:UCB}: the estimation precision in the boundary region is quite low, and in many cases, the UCBs for $m_i(1/T)$ and $m_j(1/T)$ for some $i$ and $j$ do not overlap. However, for larger sample sizes $T=250, 500$, while the estimation presicion increases, the UCBs for two trend functions remain relatively wide compared to the strength of the signal so that we almost never detect any differences.
