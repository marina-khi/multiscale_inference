...
\begin{enumerate}
	\item \textbf{actual} power, i.e., the percentage of simulation runs in which our test indicates difference between $m_1$ and \textit{some} $m_i$ for $i = 2, \ldots, n$ on some interval $[u-h, u+h]$ where the difference is indeed present, that is, on some interval $[u-h, u+h]$ such that $[u-h, u+h] \cap ([0.2, 0.4] \cup [0.6, 0.8]) \neq \emptyset$. The values of the actual power for different simulation scenarios are presented in Table \ref{tab:actualpower};
	\item \textbf{majority} power, i.e., the percentage of simulation runs in which our test indicates difference between $m_1$ and \textit{most of} $m_i$ for $i = 2, \ldots, n$ (in our case, at least $7$) on some interval $[u-h, u+h]$ where the difference is indeed present, that is, on some interval $[u-h, u+h]$ such that $[u-h, u+h] \cap ([0.2, 0.4] \cup [0.6, 0.8]) \neq \emptyset$. The values of the majority power for different simulation scenarios are presented in Table \ref{tab:majoritypower};
	\item \textbf{full} power, i.e., the percentage of simulation runs in which our test indicates difference between $m_1$ and \textit{all} $m_i$ for $i = 2, \ldots, n$ on some interval $[u-h, u+h]$ where the difference is indeed present, that is, on some interval $[u-h, u+h]$ such that $[u-h, u+h] \cap ([0.2, 0.4] \cup [0.6, 0.8]) \neq \emptyset$. The values of the full power for different simulation scenarios are presented in Table \ref{tab:fullpower};
	\item \textbf{spurious} power, i.e., the percentage of simulation runs in which our test indicates difference between $m_1$ and \textit{some} $m_i$ for $i = 2, \ldots, n$ on some interval $[u-h, u+h]$ where the difference is \textit{not} present, that is, on some interval $[u-h, u+h]$ such that $[u-h, u+h] \cap ([0.2, 0.4] \cup [0.6, 0.8]) = \emptyset$, or our test indicates a difference between $m_i$ and $m_j$ for some $i, j = 2, \ldots, n$. The values of the actual power for different simulation scenarios are presented in Table \ref{tab:spuriouspower}.
\end{enumerate}

	 {\color{red}Marina: The power numbers actually look pretty good, we can report them in the simulation study instead of the global power (given in Table \ref{tab:sizepower} for comparison).}

We have also included size simulation results with a bump function under the null $H_0: m_1 = \ldots = m_n$ instead of a zero function. Specifically, we assume that under the null, all $m_i$ are generated according to the following specification:
\begin{align*}
m_i(u) =  b \cdot  \vartheta(u, 0.3, 0.1) - b  \cdot  \vartheta(u,0.7, 0.1) \quad \forall i = 1, \ldots, n,
\end{align*}
with $\vartheta(u,c,d) =  \ind \left\{\frac{|u - c|}{d}\leq 1 \right\} \left(1 - \frac{(u - c)^2}{d^2}\right)^2$ for $b = 0.25$. The empirical size for two cases (with zero and bump functions under the null, respectively) is presented in Table \ref{tab:sizes}. {\color{red}Marina: The size numbers are a bit more conservative, it seems that the convergence is slower.}

{\color{red}I have also added how precise we are estimating the coefficients and the long-run variance. The results for various sample sizes $T=100, 250, 500$ and two different specifications of the null function ($m_i = 0$ for all $i$ and $m_i$ being the bump function as specified above.) are presented in Figures \ref{fig:hist_coefficients:zero} - \ref{fig:hist_beta:bump}}.
...
\footnotesize{
\begin{center}
\caption{Majority power of the multiscale test for different time series lengths $T$ and nominal sizes $\alpha$.
Each panel gives the results for a different bump height $b$, where $b = 0.25,0.5,0.75$ corresponds to three different alternatives of $m_1$.}\label{tab:majoritypower}
\begin{subtable}[b]{0.32\textwidth}
\centering  
\caption{$b=0.25$}
...
\footnotesize{
\begin{center}
\caption{Full power of the multiscale test for different time series lengths $T$ and nominal sizes $\alpha$.
Each panel gives the results for a different bump height $b$, where $b = 0.25,0.5,0.75$ corresponds to three different alternatives of $m_1$.}\label{tab:fullpower}
\begin{subtable}[b]{0.32\textwidth}
\centering  
\caption{$b=0.25$}
...
\footnotesize{
\begin{center}
\caption{Empirical size of the multiscale test for different time series lengths $T$ and nominal sizes $\alpha$ for different specification of $m_i$ under the null. The left panel gives the results for $m_i = 0$ for all $i$, whereas the right panel presents the results for $m_i$ being a bump function with height $0.25$. {\color{red}Marina: the left panel is one of the panels from the Supplement.}}\label{tab:sizes}
\begin{subtable}[b]{0.4\textwidth}
\centering  
...
\footnotesize{
\begin{center}
\caption{Histogram of the estimated coefficients $\beta_1$, $\beta_2$ and $\beta_3$ under the null of $m_i = 0$ for all $i$.}\label{fig:hist_coefficients:zero}
\begin{subfigure}[b]{\textwidth}
    \centering
    \includegraphics[scale=0.33]{../output/beta_histogram_zero_function_T_100_b_0.pdf}
    \caption{$T= 100$}
    \end{subfigure}
\begin{subfigure}[b]{\textwidth}
    \centering
    \includegraphics[scale=0.33]{../output/beta_histogram_zero_function_T_250_b_0.pdf}
    \caption{$T= 250$}
    \end{subfigure}
\begin{subfigure}[b]{\textwidth}
    \centering
    \includegraphics[scale=0.33]{../output/beta_histogram_zero_function_T_500_b_0.pdf}
    \caption{$T= 500$}
    \end{subfigure}
    \end{center}}
\end{figure}
...
\footnotesize{
\begin{center}
\caption{Histogram of the estimated coefficients $\beta_1$, $\beta_2$ and $\beta_3$ under the null of $m_i$ being the bump function.}\label{fig:hist_coefficients:bump}
\begin{subfigure}[b]{\textwidth}
    \centering
    \includegraphics[scale=0.33]{../output/beta_histogram_bump_function_T_100_b_0.pdf}
    \caption{$T= 100$}
    \end{subfigure}
\begin{subfigure}[b]{\textwidth}
    \centering
    \includegraphics[scale=0.33]{../output/beta_histogram_bump_function_T_250_b_0.pdf}
    \caption{$T= 250$}
    \end{subfigure}
\begin{subfigure}[b]{\textwidth}
    \centering
    \includegraphics[scale=0.33]{../output/beta_histogram_bump_function_T_500_b_0.pdf}
    \caption{$T= 500$}
    \end{subfigure}
    \end{center}}
\end{figure}
...
\footnotesize{
\begin{center}
\caption{Histogram of the estimated square root of the long-run variance $\sigma$ under the null of $m_i=0$ for all $i$.}\label{fig:hist_lrv:zero}
\begin{subfigure}[b]{0.31\textwidth}
    \centering
    \includegraphics[scale=0.33]{../output/sigma_histogram_zero_function_T_100_b_0.pdf}
    \caption{$T= 100$}
    \end{subfigure}
\begin{subfigure}[b]{0.31\textwidth}
    \centering
    \includegraphics[scale=0.33]{../output/sigma_histogram_zero_function_T_250_b_0.pdf}
    \caption{$T= 250$}
    \end{subfigure}
\begin{subfigure}[b]{0.31\textwidth}
    \centering
    \includegraphics[scale=0.33]{../output/sigma_histogram_zero_function_T_500_b_0.pdf}
    \caption{$T= 500$}
    \end{subfigure}
    \end{center}}
\end{figure}
...
\footnotesize{
\begin{center}
\caption{Histogram of the estimated square root of the long-run variance $\sigma$ under the null of $m_i$ being the bump function.}\label{fig:hist_lrv:bump}
\begin{subfigure}[b]{0.31\textwidth}
    \centering
    \includegraphics[scale=0.33]{../output/sigma_histogram_bump_function_T_100_b_0.pdf}
    \caption{$T= 100$}
    \end{subfigure}
\begin{subfigure}[b]{0.31\textwidth}
    \centering
    \includegraphics[scale=0.33]{../output/sigma_histogram_bump_function_T_250_b_0.pdf}
    \caption{$T= 250$}
    \end{subfigure}
\begin{subfigure}[b]{0.31\textwidth}
    \centering
    \includegraphics[scale=0.33]{../output/sigma_histogram_bump_function_T_500_b_0.pdf}
    \caption{$T= 500$}
    \end{subfigure}
    \end{center}}
\end{figure}
...
\item For each of the time series $i = 1, \ldots, n$, we calculate the UCB as described in \cite{Kim2016} using bandwidth $0.1$. {\color{red}Marina: optimal bandwidth using cross-validation was not that easy to calculate and the theoretical optimal bandwidths were $0.4, 0.33, 0.29$ for $T=100, 250, 500$, respectively. I am currently repeating the analysis with bandwidth equal to $0.2$, do not expect anything to change.}
...
\item We calculate how many times we reject the null for our method and for the naive competitor. The results are presented in Table \ref{tab:sizepower_UCB}.
\end{itemize}

Almost for all sample sizes (and especially for $T=250, 500$) the UCBs for two trend functions remain relatively wide compared to the strength of the signal so that we almost never detect any differences. One of such examples for $T=500$ and different $b$ is presented in Figure \ref{fig:UCB}.

\begin{table}[t!]
\setcounter{table}{5}
\renewcommand{\arraystretch}{1.2}  
\begin{center}
\caption{Empirical size and power of the naive competitor for different time series lengths $T$ and bump heights $b$, where $b = 0$ corresponds to the null and $b = 0.25,0.5,0.75$ to three different alternatives}\label{tab:sizepower_UCB}
\centering  
\input{../output/15_ts_UCB}
\end{center}
\end{table}
...
%}
%\ffigbox{\vspace{3mm}
%\includegraphics{../output/1_vs_15_with_b_75.pdf}
%}{%
%  \caption{An example of two uniform confidence bands for $T=500$ and $b=0.75$.}\label{fig:UCB}%
%}
%\end{floatrow}
%\end{figure}
...
\footnotesize{
\begin{center}
\caption{An example of two uniform confidence bands for $T=500$ and $b=0.25, 0.50, 0.75$.}\label{fig:UCB}
\begin{subfigure}[b]{0.31\textwidth}
    \centering
