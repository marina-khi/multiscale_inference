...
\subsection{Size and power properties of the multiscale test}\label{subsec:sim:main} 
...
To generate data under the null $H_0: m_1 = \ldots = m_n$, we let $m_i = 0$ for all $i$. (A robustness check with a different choice of $m_i$ is discussed below.) To produce data under the alternative, we use the bump function
\begin{align}
...
Each panel gives the results for a different bump height $b$, where $b = 0$ corresponds to the null $H_0$ and $b = 0.25,0.5,0.75$ to three different alternatives.}\label{tab:sizepower}
...
Table \ref{tab:sizepower} reports the empirical size and power of the multiscale test. Both the empirical size and the empirical power are computed as the number of simulations in which the test rejects the global null $H_0$ divided by the total number of simulations.
...
Inspecting panels (b)--(d), one can further see that the test has good power properties. For the smallest time series length $T=100$, the power numbers are still very low, reflecting the fact that the noise level in the data is extremely high, even for the largest bump size $b=0.75$. 
However, as we increase the time series length $T$, the power increases quickly. For $T=250$ and $b=0.5$, for instance, we already have power of over $95\%$ when $\alpha \ge 0.05$.


The empirical power computed in Table \ref{tab:sizepower} is a very rough measure of performance. We now refine it to get more insight into the performance of our test methods. To do so, we introduce the following notation. We say that our test finds
\begin{itemize}[itemsep=0pt, parsep=0pt, topsep=3pt]
\item a \textbf{real difference} between $i$ and $j$ on the interval $[u-h,u+h]$ if the local null hypothesis $H_0^{[i,j]}(u,h)$ is violated (i.e., if the trends $m_i$ and $m_j$ are indeed different on $[u-h,u+h]$),
...
\end{itemize}
Notably, the power numbers in Table \ref{tab:sizepower} do not give any information on
(i) whether the differences found by the test are real or spurious and
(ii) which real differences have been found, in particular, whether all or only part of them have been found.
We now introduce refined notions of power which give this kind of information. The simulation setup is the same as before. In particular, $m_1$ is the bump function from \eqref{eq:bump-fct} with $b \in \{0.25,0.5,0.75\}$ and $m_i = 0$ for all $i \ne 1$. Hence, $m_1$ differs from the other trends only in the time period $\mathcal{I}^* = [0.2, 0.4] \cup [0.6, 0.8]$. We compute the following notions of power in this setup: 
\begin{itemize}[itemsep=0pt, parsep=0pt, topsep=3pt]
...
\caption{Real power$^{[1]}$ of the multiscale test.}\label{tab:realpower1}
%Each panel gives the results for a different bump height $b$, where $b = 0.25,0.5,0.75$ corresponds to three different alternatives of $m_1$.
\vspace{-0.25cm}
...
\caption{Real power$^{{[50]}}$ of the multiscale test.}\label{tab:realpower50}
\vspace{-0.25cm}

\begin{subtable}[b]{0.32\textwidth}
\centering  
\caption{$b=0.25$}
\input{../output/15_ts_25_25_majority_power_b_25}
\end{subtable}
\begin{subtable}[b]{0.32\textwidth}
\setcounter{subtable}{0}  
\centering  
\vspace{0.25cm} 
\caption{$b=0.5$}
\input{../output/15_ts_25_25_majority_power_b_50}
\end{subtable}
\begin{subtable}[b]{0.32\textwidth}
\centering  
\vspace{0.25cm} \caption{$b=0.75$}
\input{../output/15_ts_25_25_majority_power_b_75}
\end{subtable}
\vspace{0.5cm}

\caption{Real power$^{[100]}$ of the multiscale test.}\label{tab:realpower100}
\vspace{-0.25cm}

\begin{subtable}[b]{0.32\textwidth}
\centering  
\caption{$b=0.25$}
\input{../output/15_ts_25_25_full_power_b_25}
\end{subtable}
\begin{subtable}[b]{0.32\textwidth}
\setcounter{subtable}{0}  
\centering  
\vspace{0.25cm} 
\caption{$b=0.5$}
\input{../output/15_ts_25_25_full_power_b_50}
\end{subtable}
\begin{subtable}[b]{0.32\textwidth}
\centering  
\vspace{0.25cm} \caption{$b=0.75$}
\input{../output/15_ts_25_25_full_power_b_75}
\end{subtable}
\vspace{0.5cm}

\caption{Spurious power of the multiscale test.}\label{tab:spuriouspower}
\vspace{-0.25cm}

\begin{subtable}[b]{0.32\textwidth}
\centering  
\caption{$b=0.25$}
\input{../output/15_ts_25_25_spurious_power_b_25}
\end{subtable}
\begin{subtable}[b]{0.32\textwidth}
\setcounter{subtable}{0}  
\centering  
\vspace{0.25cm} 
\caption{$b=0.5$}
\input{../output/15_ts_25_25_spurious_power_b_50}
\end{subtable}
\begin{subtable}[b]{0.32\textwidth}
\centering  
\vspace{0.25cm} 
\caption{$b=0.75$}
\input{../output/15_ts_25_25_spurious_power_b_75}
\end{subtable}

\end{table} 


The results for real power$^{[1]}$, real power$^{[50]}$, real power$^{[100]}$ and spurious power are presented in Tables \ref{tab:realpower1}--\ref{tab:spuriouspower}. 
The real power numbers in Tables \ref{tab:realpower1}--\ref{tab:realpower100} demonstrate that our test is very capable of finding the real differences between the trends. As can be seen, the numbers increase very quickly as either the sample size $T$ or the signal strength $b$ gets larger. In particular, real power$^{[1]}$ (Table \ref{tab:realpower1}) exceeds $82\%$ for all nominal sizes $\alpha$ already for moderate sample sizes $T\geq 250$ and moderate signals $b\geq 0.5$, and approaches $100\%$ for larger $T$ or stronger signals. Real power$^{[50]}$ (Table \ref{tab:realpower50}) and real power$^{[100]}$ (Table \ref{tab:realpower100}) are naturally more demanding but still exceed $78\%$ for all nominal sizes $\alpha$, $T=500$ and $b\geq 0.5$.
In addition, the spurious power numbers in Table \ref{tab:spuriouspower} demonstrate that the test controls the probability of finding a false positive at the nominal level $\alpha$. 
Taken together, the results in Tables \ref{tab:realpower1}--\ref{tab:spuriouspower} illustrate the main advantage of our test: it provides information on which trends are different and where they differ from each other. 


\subsection{Robustness checks for the multiscale test} 


\subsubsection{Additional size simulations}\label{subsec:size-further}
...
\footnotesize{
\begin{center}
\caption{Empirical size of the multiscale test under $H_0$ with (a) $m_i = 0$ for all $i$ and (b) $m_i$ the bump function from \eqref{eq:bump-fct} with $b=0.25$ for all $i$.}\label{tab:size2}
\begin{subtable}[b]{0.45\textwidth}
\centering  
\caption{$m_i = 0$ for all $i$}
\input{../output/15_ts_25_25_size}
\end{subtable}
\begin{subtable}[b]{0.45\textwidth}
\centering  
\caption{$m_i$ bump function with $b=0.25$ for all $i$}
...
\begin{figure}[t!]
\caption{Histograms of the estimated (square root of the) long-run error variance with $m_i$ the zero and the bump function. The red vertical line is the true value of the (square root of the) long-run error variance.}\label{fig:lrv}
...
As a first robustness check, we re-run the size simulations from Section \ref{subsec:sim:main}  with a different choice of the function $m_i$. In particular, rather than choosing $m_i = 0$ for all $i$, we choose $m_i$ to be the bump function from \eqref{eq:bump-fct} with $b=0.25$. The resulting size numbers are presented in Table \ref{tab:size2}(b). For ease of comparison, we have reproduced the size numbers from Table \ref{tab:sizepower}(a) for the case with $m_i=0$ in Table \ref{tab:size2}(a). The main findings are as follows: (i) When $m_i$ is the bump function, the test is conservative. In particular, the size numbers are substantially below the target $\alpha$ for the smallest sample size $T=100$, gradually getting better as the sample size increases. (ii) When $m_i$ is the zero function, in contrast, the size numbers tend to be rather on the liberal than the conservative side. (iii) For smaller sample sizes, in particular for $T=100$, the size numbers tend to be more accurate when $m_i$ is the zero function. For larger sample sizes, in contrast, in particular for $T=500$, the size numbers are fairly accurate no matter whether $m_i$ is the zero or the bump function. 
How can these differences in size numbers be explained? Importantly, estimating the long-run error variance $\sigma^2$ is more difficult in the presence of a non-trivial (i.e., non-constant) trend $m_i$. In particular, if we do not manage to eliminate the trend $m_i$ accurately from the data (e.g.\ by differencing or nonparametric regression techniques), this will bias the estimator of the long-run error variance. This is illustrated in Figure \ref{fig:lrv}: whereas the estimates of the long-run error variance are nearly unbiased in the case with $m_i = 0$ for all $i$, there is a clear bias in the case where $m_i$ is the bump function (the bias getting smaller as the sample size increases). As the long-run error variance enters our multiscale test statistic $\widehat{\Psi}_{n,T}$ as a normalization constant, a biased estimate of it will in general lead to size distortions. 


\subsubsection{Performance of the test for large $\boldsymbol{n}$}\label{subsec:sim:large_n}
...
\centering 
\footnotesize{
\caption{Empirical size and power of the multiscale test. Each column corresponds to a particular choice of $n$. Panels (a)--(d) of each column have the same structure as in Table \ref{tab:sizepower}.}\label{tab:large-n}
  
\begin{subtable}[c]{0.24\textwidth}
\centering  
\caption*{$n=15$}\vspace{0.1cm} 
\caption{$b=0$}
\input{../output/15_ts_25_25_size_dyadic_grid}\vspace{0.25cm}
\caption{$b=0.25$}
\input{../output/15_ts_25_25_power_b_25_dyadic_grid}\vspace{0.25cm}
\caption{$b=0.5$}
\input{../output/15_ts_25_25_power_b_50_dyadic_grid}\vspace{0.25cm}
\caption{$b=0.75$}
\input{../output/15_ts_25_25_power_b_75_dyadic_grid}
\end{subtable}
\begin{subtable}[c]{0.24\textwidth}
\setcounter{subtable}{0}  
\centering  
...
\caption{$b=0$}
\input{../output/25_ts_25_25_dyadic_grid_size}\vspace{0.25cm}
\caption{$b=0.25$}
\input{../output/25_ts_25_25_dyadic_grid_power_b_25}\vspace{0.25cm}
\caption{$b=0.5$}
\input{../output/25_ts_25_25_dyadic_grid_power_b_50}\vspace{0.25cm}
\caption{$b=0.75$}
\input{../output/25_ts_25_25_dyadic_grid_power_b_75}
\end{subtable}
\begin{subtable}[c]{0.24\textwidth}
\setcounter{subtable}{0}  
\centering  
...
\caption{$b=0$}
\input{../output/50_ts_25_25_dyadic_grid_size}\vspace{0.25cm}
\caption{$b=0.25$}
\input{../output/50_ts_25_25_dyadic_grid_power_b_25}\vspace{0.25cm}
\caption{$b=0.5$}
\input{../output/50_ts_25_25_dyadic_grid_power_b_50}\vspace{0.25cm}
\caption{$b=0.75$}
\input{../output/50_ts_25_25_dyadic_grid_power_b_75}
\end{subtable}
\begin{subtable}[c]{0.24\textwidth}
\setcounter{subtable}{0}  
\centering  
...
\caption{$b=0$}
\input{../output/100_ts_25_25_dyadic_grid_size}\vspace{0.25cm}
\caption{$b=0.25$}
\input{../output/100_ts_25_25_dyadic_grid_power_b_25}\vspace{0.25cm}
\caption{$b=0.5$}
\input{../output/100_ts_25_25_dyadic_grid_power_b_50}\vspace{0.25cm}
\caption{$b=0.75$}
\input{../output/100_ts_25_25_dyadic_grid_power_b_75}
\end{subtable}}

\end{sidewaystable}


So far, we have fixed the number of time series at $n = 15$. We now examine the performance of our test as $n$ gets larger. To do so, we re-run the simulation exercises from Section \ref{subsec:sim:main} for $n \in \{25,50,100\}$ and $T$ as before, i.e., $T \in \{100,250,500\}$. We work with precisely the same simulation setup as in Section \ref{subsec:sim:main} except that we use the smaller grid $\mathcal{G}_T^{\text{sparse}}$ defined in \eqref{eq:grid-sparse} to decrease the computational burden. Overall, our simulation results suggest that the test performs well for larger $n$. The results are presented in Table \ref{tab:large-n}, with each column corresponding to a particular choice of $n$. 
The upper panels (a) of Table \ref{tab:large-n} show that the empirical size of the test provides a reasonably good approximation to the target $\alpha$ across all scenarios under consideration. In particular, the size numbers remain fairly stable as $n$ increases, even though they appear to get a little less precise for larger $n$. 
Panels (b)--(d) further show that the test has reasonable power across all considered scenarios. The power numbers, however, decrease somewhat as $n$ gets larger, which may be explained as follows: Let $\mathcal{M} = \{ (u,h,i,j): (u,h) \in \grid \text{ and } 1 \le i < j \le n \}$ be the collection of all location-scale points $(u,h)$ and all pairs of time series $(i,j)$ under consideration and let $\mathcal{M}_1 \subseteq \mathcal{M}$ be the set of tuples $(u,h,i,j)$ for which the local null hypothesis $H_0^{[i, j]}(u, h)$ is false. Since in the simulation setup at hand, all trend functions are the same except the first one ($m_1$ being the bump function from \eqref{eq:bump-fct} with bump size $b$ and $m_i = 0$ for all $i \ne 1$), it holds that $|\mathcal{M}_1| / |\mathcal{M}| \to 0$ quite fast as $n \to \infty$ (where $| \mathcal{M} |$ denotes the cardinality of the set $\mathcal{M}$). Hence, as $n$ increases, the total number of hypotheses to be tested grows much faster than the number of hypotheses which are violated. As a consequence, it gets harder to detect these violations, which is reflected in somewhat lower power numbers. 


\subsubsection{Robustness checks on the choice of tuning parameters}\label{subsec:sim:grid}
...
\end{align}
with $H_T^{\text{sparse}} = \{ h = 2^k h_{\min} \text{ for } k=0,\ldots,K \}$, where $h_{\min} = \frac{\lceil \log T \rceil}{T}$ and $K$ is such that $2^K h_{\min} \le \frac{1}{4}$, i.e., $K \le \lfloor \log(\frac{T}{4 \lceil \log T \rceil }) \frac{1}{\log(2)} \rfloor$. This is a dyadic scheme with scales in the set $H_T^{\text{sparse}}$ as commonly used in Wavelet analysis. 
...
\footnotesize{
\begin{center}
\caption{Empirical size and power of the multiscale test for three different grids. Each column corresponds to a particular choice of grid. Panels (a)--(d) of each column have the same structure as in Table \ref{tab:sizepower}.}\label{tab:grids}
\begin{subtable}[b]{0.32\textwidth}
\setcounter{subtable}{0}    
\centering  
...
\caption{$b=0$}
\input{../output/15_ts_25_25_size}\vspace{0.25cm}
\caption{$b=0.25$}
\input{../output/15_ts_25_25_power_b_25}\vspace{0.25cm}
\caption{$b=0.5$}
\input{../output/15_ts_25_25_power_b_50}\vspace{0.25cm}
\caption{$b=0.75$}
\input{../output/15_ts_25_25_power_b_75}
\end{subtable}
\begin{subtable}[b]{0.32\textwidth}
...
\caption{$b=0$}
\input{../output/15_ts_25_25_size_fine_grid}\vspace{0.25cm}
\caption{$b=0.25$}
\input{../output/15_ts_25_25_power_b_25_fine_grid}\vspace{0.25cm}
\caption{$b=0.5$}
\input{../output/15_ts_25_25_power_b_50_fine_grid}\vspace{0.25cm}
\caption{$b=0.75$}
\input{../output/15_ts_25_25_power_b_75_fine_grid}
\end{subtable}
\end{center}}
\end{table}
...
\subsubsection{Performance of the test in the presence of time-varying error variance}\label{subsec:sim:SV}
...
% \caption{Empirical size of the multiscale test in case of time-varying error variance.}\label{tab:size:SV}

% \centering
% \input{../output/15_ts_25_25_size_SV}
% \vspace{0.6cm}

% \caption{Real power$^{[1]}$ in case of time-varying error variance for the three different alternatives with bump height $b = 0.25,0.5,0.75$.}\label{tab:realpower1:SV}
% \vspace{-0.25cm}

% \begin{subtable}[b]{0.32\textwidth}
% \centering  
% \caption{$b=0.25$}
% \input{../output/15_ts_25_25_actual_power_b_25_SV}
% \end{subtable}
% \begin{subtable}[b]{0.32\textwidth}
% \setcounter{subtable}{0}  
% \centering  
% \vspace{0.25cm} 
% \caption{$b=0.5$}
% \input{../output/15_ts_25_25_actual_power_b_50_SV}
% \end{subtable}
% \begin{subtable}[b]{0.32\textwidth}
% \centering  
% \vspace{0.25cm} \caption{$b=0.75$}
% \input{../output/15_ts_25_25_actual_power_b_75_SV}
% \end{subtable}
% \vspace{0.5cm}

% \caption{Real power$^{[50]}$ in case of time-varying error variance for the three different alternatives with bump height $b = 0.25,0.5,0.75$.}\label{tab:realpower50:SV}
% \vspace{-0.25cm}

% \begin{subtable}[b]{0.32\textwidth}
% \centering  
% \caption{$b=0.25$}
% \input{../output/15_ts_25_25_majority_power_b_25_SV}
% \end{subtable}
% \begin{subtable}[b]{0.32\textwidth}
% \setcounter{subtable}{0}  
% \centering  
% \vspace{0.25cm} 
% \caption{$b=0.5$}
% \input{../output/15_ts_25_25_majority_power_b_50_SV}
% \end{subtable}
% \begin{subtable}[b]{0.32\textwidth}
% \centering  
% \vspace{0.25cm} \caption{$b=0.75$}
% \input{../output/15_ts_25_25_majority_power_b_75_SV}
% \end{subtable}
% \vspace{0.5cm}

% \caption{Real power$^{[100]}$ in case of time-varying error variance for the three different alternatives with bump height $b = 0.25,0.5,0.75$.}\label{tab:realpower100:SV}
% \vspace{-0.25cm}

% \begin{subtable}[b]{0.32\textwidth}
% \centering  
% \caption{$b=0.25$}
% \input{../output/15_ts_25_25_full_power_b_25_SV}
% \end{subtable}
% \begin{subtable}[b]{0.32\textwidth}
% \setcounter{subtable}{0}  
% \centering  
% \vspace{0.25cm} 
% \caption{$b=0.5$}
% \input{../output/15_ts_25_25_full_power_b_50_SV}
% \end{subtable}
% \begin{subtable}[b]{0.32\textwidth}
% \centering  
% \vspace{0.25cm} \caption{$b=0.75$}
% \input{../output/15_ts_25_25_full_power_b_75_SV}
% \end{subtable}

% \end{table} 

In what follows, we investigate the performance of our test in the presence of time-varying error variance. We consider the same simulation setup as in Section \ref{subsec:sim:main} except that we model the error processes $\{\varepsilon_{it}: 1 \le t \le T\}$ slightly differently. Specifically, for each $i$, we consider an AR(1) process of the form $\varepsilon_{it} = a \varepsilon_{i t-1} + \nu(t/T)\eta_{it}$ with $a = 0.25$ and i.i.d.\ standard normal innovations $\eta_{it}$. In the simulation setup of Section \ref{subsec:sim:main}, the function $\nu$ was assumed to be constant, in particular, $\nu(u) = c_\nu$ with $c_\nu = 0.25$ for all $u \in [0,1]$. Now, in contrast, we allow $\nu$ to vary over time, specifically, we let $\nu(u) = \{ -0.15(u-0.5)^2 + 0.075 \}^{1/2}$ for $u \in [0, 1]$. Note that with this choice, the average short-run error variance $\int_0^1 \nu^2(u)du$ is equal to $c_\nu^2 = 0.25^2$ and thus matches the short-run error variance in the setup of Section \ref{subsec:sim:main}. Notably, the time variation induced by our choice of $\nu$ is quite strong. In particular, we have $\max_{u \in [0,1]} \nu^2(u) = 2 \min_{u \in [0,1]} \nu^2(u)$. 
The simulation results are presented in Table \ref{tab:sizepower:SV}. Even though the test has reasonable power properties (panels (b)--(d) in Table \ref{tab:sizepower:SV}), the empirical size lies substantially below the target $\alpha$ (panel (a) in Table \ref{tab:sizepower:SV}), implying that the test is quite conservative in the case at hand. This demonstrates that the test should be treated with caution when the (short-run and/or long-run) error variance is expected to be strongly time-varying. The issue is that the scaling of our test statistics depends on the long-run error variance (see equation \eqref{eq:psi_zero_ij}) and that this scaling is theoretically not correct any more if the long-run error variance is time-varying. We may therefore expect non-negligible size distortions when the long-run error variance strongly varies over time. 

\begin{table}[t!]
\renewcommand{\arraystretch}{1.2}  
\footnotesize{
\begin{center}
\caption{Empirical size and power of the multiscale test in case of time-varying error variance for different time series lengths $T$ and nominal sizes $\alpha$. Each panel gives the results for a different bump height $b$, where $b = 0$ corresponds to the null $H_0$ and $b = 0.25,0.5,0.75$ to three different alternatives.}\label{tab:sizepower:SV}
\begin{subtable}[b]{0.4\textwidth}
\centering  
\caption{$b=0$}
\input{../output/15_ts_25_25_size_SV}
...
\centering  
\caption{$b=0.25$}
\input{../output/15_ts_25_25_power_b_25_SV}
\end{subtable}
\begin{subtable}[b]{0.4\textwidth}
\centering  
\vspace{0.25cm} \caption{$b=0.5$}
\input{../output/15_ts_25_25_power_b_50_SV}
\end{subtable}
\begin{subtable}[b]{0.4\textwidth}
\centering  
\vspace{0.25cm} \caption{$b=0.75$}
\input{../output/15_ts_25_25_power_b_75_SV}
\end{subtable}
\end{center}}
\end{table}

\subsection{Comparison of our test with competing methods}


We compare our methods with two competitors. First, we consider the following simple procedure:  (i) compute a uniform confidence band (UCB) at level $(1-\alpha)$ for each trend function $m_i$ and (ii) reject the hypothesis that $m_i$ and $m_j$ are the same if their UCBs do not overlap at some time point. This procedure may be run with a Bonferroni correction to take into account that multiple comparisons are carried out. In addition, we compare our test method with the SiZer test from \cite{Park2009}. 


\subsubsection{Comparison with a simple competitor} 


To set up the first competitor, we compute uniform confidence bands (UCBs) for the trends $m_i$ as described in \cite{Kim2016}. For our comparison, we consider the simplified model $Y_{it} = m_i(\frac{t}{T}) + \bfbeta_i^\top \X_{it} +  \varepsilon_{it}$ without fixed effects since the specification in \cite{Kim2016} does not allow for them. Other than that, the simulation setup is as specified in Section \ref{subsec:sim:main}. 
In particular, to generate data under the null $H_0: m_1 = \ldots = m_n$, we let $m_i = 0$ for all $i$, and to produce data under the alternative, we use the bump function $m_1$ defined in \eqref{eq:bump-fct} with bump size $b \in \{ 0.25, 0.5, 0.75 \}$ while setting $m_i = 0$ for $i \ne 1$.
We implement our multiscale test as in Section \ref{subsec:sim:main}, where we fix the nominal size at $\alpha = 0.05$. To implement the UCBs, we set $\alpha = 0.05$ and estimate the trend function $m_i$ separately for each time series $i$ by a local linear estimate with bandwidth $h = 0.2$.\footnote{\cite{Kim2016} proposes a data-driven way to calculate the bandwidth using cross-validation, which needs to be repeated for each realisation of the data and is  therefore computationally very expensive. Hence, we have chosen several fixed bandwidths ($h = 0.1, 0.2, 0.3$) and report here the results for the best-performing one (which is $h=0.2$).} We then construct the UCB of $m_i$ as described in \cite{Kim2016}. In what follows, we denote our multiscale test by $\mathcal{T}_{\text{MS}}$ and the test based on the UCBs from \cite{Kim2016} by $\mathcal{T}_{\text{UCB}}$. 


Table \ref{tab:sizepower_UCB} compares the size and power properties of the two tests: panel (a) reports the empirical size numbers, while panels (b)--(d) specify the empirical power numbers under the three different alternatives with bump size $b \in \{0.25,0.5,0.75\}$. To calculate the empirical size and power for $\mathcal{T}_{\text{UCB}}$, we consider the null hypothesis $H_0$ as rejected if there exists at least one pair $(i, j)$ and at least one time point $t/T$ such that the UCBs of $m_i$ and $m_j$ do not overlap at $t/T$. As the estimation procedure from \cite{Kim2016} turned out to perform extremely poorly at the boundaries,  we restrict attention to points $t/T \in [0.05, 0.95]$ (which is fine as in our simulation setting, the trend $m_1$ differs from the other trends only in the time period $\mathcal{I}^* = [0.2, 0.4] \cup [0.6, 0.8]$ anyway).
Table \ref{tab:sizepower_UCB} demonstrates that our test has reasonable size and power properties. The test $\mathcal{T}_{\text{UCB}}$, in contrast, is extremely conservative (with empirical size exactly equal to $0$ for all considered levels $\alpha$!) and has extremely low power for smaller sample sizes, the power moderately increasing with the sample size. Notably, we have run $\mathcal{T}_{\text{UCB}}$ without any Bonferroni correction which would make things even worse! The reason for this poor performance is illustrated in Figure \ref{fig:UCB} which shows the estimated trends $m_1$ and $m_2$ together with their UCBs in a representative simulation run for the largest sample size $T=500$. Even for this sample size, the bands are still very wide, overlapping everywhere for the bump sizes $b=0.25$ and $b=0.5$. Only for the largest bump size $b=0.75$, the bands do not overlap at all time points (but only hardly so). Hence, the computed UCBs are much too wide to produce a good test performance. 
...
\footnotesize{
\begin{center}
\caption{Size and power comparisons (with $\alpha$ fixed at $0.05$) for our multiscale test $\mathcal{T}_{\text{MS}}$ and the procedure $\mathcal{T}_{\text{UCB}}$ based on UCBs from \cite{Kim2016}.}\label{tab:sizepower_UCB}
\vspace{-0.25cm}


\begin{subtable}[b]{0.23\textwidth}
\centering  
\caption{$b=0$}
\begin{tabular}{ccc}
  \hline
 $T$ & $\mathcal{T}_{\text{MS}}$ & $\mathcal{T}_{\text{UCB}}$ \\
 \hline
100 & 0.036 & 0.000 \\ 
  250 & 0.087 & 0.000 \\ 
  500 & 0.067 & 0.000 \\ 
...
\begin{subfigure}[b]{0.31\textwidth}
    \centering
    \includegraphics[scale=0.33]{../output/UCB_plot_with_T_500_and_b_25.pdf}
    \caption{$b = 0.25$}
    \end{subfigure}
\begin{subfigure}[b]{0.31\textwidth}
    \centering
    \includegraphics[scale=0.33]{../output/UCB_plot_with_T_500_and_b_50.pdf}
    \caption{$b = 0.5$}
    \end{subfigure}
\begin{subfigure}[b]{0.31\textwidth}
    \centering
    \includegraphics[scale=0.33]{../output/UCB_plot_with_T_500_and_b_75.pdf}
    \caption{$b = 0.75$}
    \end{subfigure}
\end{center}
\vspace{-0.5cm}

\caption{Estimated trends $m_1$ (solid line in red) and $m_2 = 0$ (solid line in blue) and their corresponding UCBs (dashed lines) for $T=500$ and $b=0.25, 0.5, 0.75$.}\label{fig:UCB}
\end{figure}


%To calculate the size and power for the competitor based on UCBs, we consider the null hypothesis rejected if there exist at least one pair $(i, j)$ and at least one point $t/T$ such that the UCBs of $m_i$ and $m_j$ do not overlap at $t/T$. In our experience, the estimation procedure from \cite{Kim2016} has poor performance at the boundaries, hence, to make the comparison as fair as possible, we restrict our attention only to the points $t/T \in [0.05, 0.95]$.

%In what follows, we denote our multiscale test by $\mathcal{T}_{\text{MS}}$ and the test based on the UCBs from \cite{Kim2016} by $\mathcal{T}_{\text{UCB}}$. Table \ref{tab:sizepower_UCB} presents the results of our comparison study. Panel (a) compares the empirical size of the two tests, while panels (b), (c) and (d) provide a power comparison. As can be expected, our test has good power properties compared to the competing procedure. For example, for the smallest sample size $T=100$, the UCBs for two trend functions remain relatively wide compared to the strength of the signal so that we almost never detect any differences. For moderate sample size $T=250$ our test perform well already for a moderate signal $b=0.5$ while $\mathcal{T}_{\text{UCB}}$ produces wide UCBs that the test barely has any power. Only for the largest sample size $T=500$, the test $\mathcal{T}_{\text{UCB}}$ reaches a power of $1$ for the strongest signal, with our test being more powerful in all of the other settings. Furthermore, in this comparison we do not apply any correction for multiple testing, but clearly any type of such correction would result in UCBs being wider and the test $\mathcal{T}_{\text{UCB}}$ needing even stronger signals and larger sample sizes to reach considerable power.
...
%We consider different time series lengths $T \in \{100,250,500\}$.
...
As before, we use the notation $\mathcal{T}_{\text{MS}}$ for our multiscale test and we denote the SiZer test of \cite{Park2009} by $\mathcal{T}_{\text{SiZer}}$. Table \ref{tab:sizer} presents the results of our comparison study. Panel (a) compares the empirical size of the two tests, while panels (b) and (c) provide a power comparison. 
...
\renewcommand{\arraystretch}{1.2}
\input{../output/15_ts_correct_group_number}
\end{subtable}
\begin{subtable}[b]{0.48\textwidth}
\centering
...
\renewcommand{\arraystretch}{1.2}
\input{../output/15_ts_correct_group_structure}
\end{subtable}
\end{center}}
\vspace{-0.5cm}
\end{table}
...
Figures \ref{fig:clustering:1} and \ref{fig:clustering:2} give more insight into what happens for $T=100$ and $T=250$. Figure \ref{fig:clustering:1} shows histograms of the $5000$ simulated values of $\widehat{N}$, while Figure \ref{fig:clustering:2} depicts histograms of the number of classification errors produced by our algorithm. By the number of classification errors, we simply mean the number of incorrectly classified time series, which is formally calculated as $\min_{\pi \in \Pi} \{ |G_1 \setminus \widehat{G}_{\pi(1)}| + \ldots + |G_N \setminus \widehat{G}_{\pi(N)}| \}$, where $\Pi$ is the set of all permutations of $\{1, 2, \ldots, \max\{N,\widehat{N}\} \}$ and we let $\widehat{G}_l = \emptyset$ for $\widehat{N} < l \le N$. The histograms of Figure \ref{fig:clustering:1} for $T=100$ and $T = 250$ clearly show that our method underestimates the number of groups. In particular, it fails to detect the difference between two out of three groups in a large number of simulations ($\widehat{N} = 2$ in $3448$ and $4883$ cases out of $5000$ for $T= 100$ and $T=250$, respectively). This is reflected in the corresponding histograms of Figure \ref{fig:clustering:2} which show that there are exactly $5$ classification errors in $3345$ ($4884$) of the $5000$ simulation runs for $T=100$ ($T=250$). In most of these cases, the estimated group structure $\{ \widehat{G}_1, \widehat{G}_{2}\}$ coincides with either $\{ G_1 \cup G_2,G_3\}$,  $\{ G_1, G_2\cup G_3\}$ or $ \{ G_1 \cup G_3,G_2\}$. In summary, we can conclude that the very small empirical probabilities for $T=100$ and $T=250$ in Table \ref{tab:clustering} are due to the algorithm underestimating the number of groups. Inspecting the histograms for $T=500$ in Figures \ref{fig:clustering:1} and \ref{fig:clustering:2}, one can see that the performance of the estimators $\widehat{N}$ and $\{ \widehat{G}_1,\ldots, \widehat{G}_{\widehat{N}} \}$ improves significantly as the time series length $T$ increases. %, even though the corresponding empirical probabilities in Table \ref{tab:clustering} are still somewhat below the target $(1-\alpha)$.
...
\item Measure the distance between $\widehat{m}_i$ and $\widehat{m}_j$ by the supremum metric 
...
\end{enumerate}
Our multiscale approach can be regarded as a refinement of this very natural and intuitive benchmark method: it replaces the simple bandwidth-dependent distance measure $d_{ij}^{\textnormal{bmk}}$ by the multiscale distance measure $d_{ij} = \max_{(u, h) \in \mathcal{G}_T}\widehat{\psi}^0_{ij, T}(u, h)$ and provides a way to estimate the number of clusters $N$. As the benchmark algorithm does not come with an estimate of the number of clusters $N$, we assume $N=3$ to be known in what follows. 
...
%  $\{ \widehat{G}_1,\ldots,\widehat{G}_N\} = \{ G_1,G_2,G_3\}$ for the proposed clustering approach with an estimated long-run variance ($\mathcal{T}_{\text{MS}}$) and with the true theoretical value of the long-run variance ($\mathcal{T}_{\text{MS, true lrv}}$) and the benchmark procedures $\mathcal{T}_{\text{bmk, }1} - \mathcal{T}_{\text{bmk, }6}$ corresponding to bandwidths $h_1 - h_6$ for different sample sizes $T$.
}\label{tab:clustering:comparison}
\renewcommand{\arraystretch}{1.2}  
\newcolumntype{Z}{>{\centering\arraybackslash}X}
\input{../output/clustering_comparison}
\end{center}}
\end{table}
...
\end{minipage}
\caption*{Note: In each figure, panel (a) shows the two augmented time series, panel (b) presents smoothed versions of the augmented time series, and panel (c) depicts the set of intervals $\mathcal{S}^{[i, j]}(\alpha)$ in grey and the subset of minimal intervals $\mathcal{S}^{[i, j]}_{\textnormal{min}}(\alpha)$ in black. }
\end{sidewaysfigure}
...
Let Assumptions \ref{C-err1}, \ref{C-err3} and \ref{C-reg1}--\ref{C-reg-err} be satisfied. Then for each $i$, 
...
We finally turn to $C_{n,T}$. By straightforward calculations, $| \sum_{t=1}^T w_{t,T}(u,h) | \le C \sqrt{T h_{\max}} = o(\sqrt{T})$. 
