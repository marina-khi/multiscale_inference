...
%\footnote{As a robustness check, we have repeated the analysis for $\phi = 0.1$. However, as the results a very similar, we do not report them here.}
...
%\footnote{As a robustness check, we have repeated the analysis for $\rho = 0.1$. However, as the results a very similar, we do not report them here.}
...
To generate data under the null $H_0: m_1 = \ldots = m_n$, we let $m_i = 0$ for all $i$ without loss of generality. As a robustness check, we consider another specification of the null hypothesis. In particular, we use the bump function
\begin{align}
m_i(u) =  b \cdot  \vartheta(u, 0.3, 0.1) - b  \cdot  \vartheta(u,0.7, 0.1)  \label{eq:bump-fct}
\end{align}
with $\vartheta(u,c,d) =  \ind \left\{\frac{|u - c|}{d}\leq 1 \right\} \left(1 - \frac{(u - c)^2}{d^2}\right)^2$ for $b= 0.25$ (bump function for various values of $b$ is depicted in Figure \ref{fig:bump_function}). Note that the normalisation constraint $\int_0^1 m_i(u) du = 0$ is directly satisfied in this case. In what follows, we refer to this case as $H_0^{\text{bump}}$. To produce the data under the alternative, we also use the bump function \eqref{eq:bump-fct} with $b \in \{ 0.25, 0.5, 0.75 \}$ for $m_1$ and let $m_i = 0$ for $i \neq 1$.  For each simulation exercise, we draw $5000$ samples.
...
%Since the errors $\varepsilon_{it}$ follow an AR($1$) process, we estimate the long-run error variance by the difference-based estimator proposed in \cite{KhismatullinaVogt2020}, setting the tuning parameters $q$ and $r$ to $25$ and $10$, respectively.
...
\footnotesize{
\begin{center}
\caption{Empirical size and (global) power of the multiscale test for different time series lengths $T$ and nominal sizes $\alpha$.
...
\footnotesize{
\begin{center}
\caption{Empirical size of the multiscale test for two specifications of the null hypothesis, $H_0: m_i = 0$ for all $i$, and $H_0^{\text{bump}}:$ $m_i$ is a bump function from \eqref{eq:bump-fct} with $b=0.25$ for all $i$, calculated for different time series lengths $T$ and nominal sizes $\alpha$.}\label{tab:size2}
\begin{subtable}[b]{0.4\textwidth}
\centering  
\caption{$H_0$}
\input{../output/15_ts_25_25_size}
...
%there is considerable noise in the data. Even for the largest bump size $b=0.75$, the noise level is so high that our test is barely able to detect the bump and reject the null. 
%For the smallest bump size $b = 0.25$ and time series length $T \in \{100,250\}$, the power is only moderate, reflecting the fact that the alternative with $b = 0.25$ is not very far away from the null.
...
Table \ref{tab:size2}\footnote{Note that the simulation results for the null hypothesis "$H_0: m_i = 0$ for all $i$" in panel (a) of Table \ref{tab:size2} are identical with those in Table \ref{tab:sizepower}. For ease of comparison, we have repeated these results as part of Table \ref{tab:size2}.} reports the empirical size of the multiscale test for two specification of the common trend under the null hypothesis, $H_0$ and $H_0^{\text{bump}}$. As can be seen, the numbers are a bit lower for the case of the bump function ($H_0^{\text{bump}}$) compared to the zero function ($H_0$). One of the possible reasons for that is loss of precision in the estimation of the long-run variance when the trend is present. In case of $H_0$, there is no trend, hence, the problem of estimating long-run variance becomes an easier one. Since our test depends on the estimator, such loss of precision results in more conservative size numbers in panel (b) compared to panel (a).

Furthermore, since the empirical power as presented in Table \ref{tab:sizepower} can be considered "global" and is not suited to capture local testing capabilities which are one of the main advantages of our multiscale test, we compute further metrics -- specifically, further types of power -- which we think are more apt to capture such local testing capabilities. Specifically, suppose our test rejects the local null hypothesis $H_0^{[i,j]}(u,h)$ under which the trends $m_i$ and $m_j$ are identical on the interval $[u-h,u+h]$. We say that the test finds
\begin{itemize}
...
\begin{itemize}
\item whether the found differences are real or spurious,
...
\footnotesize{
\begin{center}
\caption{Real power$^{[1]}$ of the multiscale test for different time series lengths $T$ and nominal sizes $\alpha$.
Each panel gives the results for a different bump height $b$, where $b = 0.25,0.5,0.75$ corresponds to three different alternatives of $m_1$.}\label{tab:realpower1}
...
\footnotesize{
\begin{center}
\caption{Real power$^{{[50]}}$ of the multiscale test for different time series lengths $T$ and nominal sizes $\alpha$.
Each panel gives the results for a different bump height $b$, where $b = 0.25,0.5,0.75$ corresponds to three different alternatives of $m_1$.}\label{tab:realpower50}
\begin{subtable}[b]{0.32\textwidth}
\centering  
\caption{$b=0.25$}
...
\footnotesize{
\begin{center}
\caption{Real power$^{[100]}$ of the multiscale test for different time series lengths $T$ and nominal sizes $\alpha$.
Each panel gives the results for a different bump height $b$, where $b = 0.25,0.5,0.75$ corresponds to three different alternatives of $m_1$.}\label{tab:realpower100}
\begin{subtable}[b]{0.32\textwidth}
\centering  
\caption{$b=0.25$}
...
\footnotesize{
\begin{center}
\caption{Spurious power of the multiscale test for different time series lengths $T$ and nominal sizes $\alpha$.
Each panel gives the results for a different bump height $b$, where $b = 0.25,0.5,0.75$ corresponds to three different alternatives of $m_1$.}\label{tab:spuriouspower}
\begin{subtable}[b]{0.32\textwidth}
\centering  
\caption{$b=0.25$}
...
The results for real power$^{[1]}$, real power$^{[50]}$, real power$^{[100]}$ and spurious power are presented in Tables \ref{tab:realpower1} -- \ref{tab:spuriouspower}. These tables jointly demonstrate that the proposed multiscale test (i) gains power very quickly as either the sample size $T$ or the signal strength $b$ increases, (ii) has notable local testing capabilities that range from “at least one correct detection” (Table \ref{tab:realpower1}) to the demanding “all pairwise differences found” (Table \ref{tab:realpower100}), and (iii) controls spurious rejections at or below nominal levels (Table \ref{tab:spuriouspower}). In particular, real power$^{[1]}$ (Table \ref{tab:realpower1}) exceeds $82\%$ for all nominal sizes $\alpha$ already for moderate sample sizes $T\geq 250$ and moderate signals $b\geq 0.5$, and approaches $100\%$ for larger $T$ or stronger signals. Real power$^{[50]}$ (Table \ref{tab:realpower50}) and real power$^{[100]}$ (Table \ref{tab:realpower100}) naturally require more information but still exceed $78\%$ for all nominal sizes $\alpha$ and $T=500$ and $b\geq 0.5$. Moreover, spurious detections (Table \ref{tab:spuriouspower}) stay are almost always below or at the nominal size $\alpha$, demonstrating control of false positives. These findings confirm the main property of our method: the test not only signals that trends differ but pinpoints where and between which series, without sacrificing size.
...
%As long as the grid is chosen sufficiently rich (in the sense of including a variety of bandwidth values ranging from very small to very large and many time points that are scattered rather densely over the unit interval), the particular choice of grid has a negligible effect on the test.
%Furthermore, even though the power numbers are a bit lower in case of a sparse grid compared to a , the test has reasonable power properties for all of the choices of the grid. %This demonstrates that
...
\subsection{Performance of the test in the presence of time-varying error variance}\label{subsec:sim:SV}

As a robustness check, in this section we investigate performance of our test in the presence of time-varying error variance. Specifically, we assume that for each $i$, the idiosyncratic error process $\{\varepsilon_{it}: 1 \le t \le T\}$ follows the AR(1) model $\varepsilon_{it} = a \varepsilon_{i t-1} + \sigma(t/T)\eta_{it}$, where $a = 0.25$, the innovations $\eta_{it}$ are i.i.d.\ normally distributed with mean $0$ and standard deviation $1$ and $\sigma^2(u) = -0.15(u-0.5)^2 + 0.075$ for $u \in [0, 1]$ (see also Figure \ref{fig:SV} for $T=500$). The choice of the the function $\sigma^2(u)$ is driven by the normalization $\int_0^1 \sigma^2(u)du = 0.25^2$ which is introduced to make this setup comparable to the ones from Section \ref{subsec:sim:main}. All of the other parameters remain as in Section \ref{subsec:sim:main}. The results are presented in Tables \ref{tab:size:SV} - \ref{tab:realpower100:SV}.

Table \ref{tab:size:SV} demonstrates that the empirical size of the test always lie below the target $\alpha$ even in the presence of time-varying error variance. Tables \ref{tab:size:SV} - \ref{tab:realpower100:SV} further show that the test has reasonable local power (specifically, real power$^{[1]}$, real power$^{[1]}$ and real power$^{[100]}$ defined in Section \ref{subsec:sim:main}).
\begin{figure}[h]
%\renewcommand{\arraystretch}{1.2}
\footnotesize{
\begin{center}
    \centering
    \includegraphics{../output/SV_plot.pdf}
    \end{center}\caption{Time-varying error variance $\sigma^2(u)$ for $u \in [0, 1]$.}\label{fig:SV}}
\end{figure}
...
\footnotesize{
\begin{center}
\caption{Empirical size of the multiscale test for different time series lengths $T$ and nominal sizes $\alpha$ in case of time-varying error variance.}\label{tab:size:SV}
\centering  
...
\footnotesize{
\begin{center}
\caption{Real power$^{[1]}$ of the multiscale test for different time series lengths $T$ and nominal sizes $\alpha$ in case of time-varying error variance.
Each panel gives the results for a different bump height $b$, where $b = 0.25,0.5,0.75$ corresponds to three different alternatives of $m_1$.}\label{tab:realpower1:SV}
\begin{subtable}[b]{0.32\textwidth}
\centering  
\caption{$b=0.25$}
...
\footnotesize{
\begin{center}
\caption{Real power$^{[50]}$ of the multiscale test for different time series lengths $T$ and nominal sizes $\alpha$ in case of time-varying error variance.
Each panel gives the results for a different bump height $b$, where $b = 0.25,0.5,0.75$ corresponds to three different alternatives of $m_1$.}\label{tab:realpower50:SV}
\begin{subtable}[b]{0.32\textwidth}
\centering  
\caption{$b=0.25$}
...
\footnotesize{
\begin{center}
\caption{Real power$^{[100]}$ of the multiscale test for different time series lengths $T$ and nominal sizes $\alpha$ in case of time-varying error variance.
Each panel gives the results for a different bump height $b$, where $b = 0.25,0.5,0.75$ corresponds to three different alternatives of $m_1$.}\label{tab:realpower100:SV}
\begin{subtable}[b]{0.32\textwidth}
\centering  
\caption{$b=0.25$}
...
\subsection{Comparison of our test with a naive competitor} 

In this section, we compare our method with a simple naive competitor that is based on the uniform confidence bands (UCBs) from \cite{Kim2016}. For our comparison, we consider the model $Y_{it} = m_i(\frac{t}{T}) + \bfbeta_i^\top \X_{it} +  \varepsilon_{it}$ without the fixed effects since the specification in \cite{Kim2016} does not allow for them. Other than that, the simulation setup is as specified in Section \ref{subsec:sim:main}. We consider $n=15$ time series, and as before, to generate data under the null $H_0: m_1 = \ldots = m_n$, we let $m_i = 0$ for $i=1, \ldots, n$, and to produce data under the alternative, we use the bump function $m_1$ defined in \eqref{eq:bump-fct} with bump size $b \in \{ 0.25, 0.5, 0.75 \}$ while setting $m_i = 0$ for $i = 2, \ldots, n$.
We implement our multiscale test as in Section \ref{subsec:sim:main}.
...
\begin{subfigure}[b]{0.31\textwidth}
    ...
    \caption{$b = 0.75$}
    \end{subfigure}
    \end{center}\caption{Examples of the estimated bump function $m_1$ (solid line in red) and $m_2 = 0$ (solid line in blue) and their corresponding UCBs (dashed lines) for $T=500$ and $b=0.25, 0.5, 0.75$.}\label{fig:UCB}}
\end{figure}

For the implementation of the competitor, we fix the nominal size $\alpha = 0.05$ and we estimate the trend function $m_i(t/T)$ separately for each of the time series $i = 1, \ldots, n$ using a local linear estimate with a bandwidth $h = 0.2$\footnote{The paper proposes a data-driven way to calculate the bandwidth using cross-validation, which needs to be repeated for each realisation of the data, and therefore, extremely computationally expensive. Hence, we have chosen several fixed bandwidths ($h = 0.1, 0.2, 0.3$) and report here results for the best-performing one.} and then construct the UCB of $m_i$ as described in \cite{Kim2016}. Examples of two instances of the estimated trends under the alternative together with their corresponding UCBs are shown in Figure \ref{fig:UCB}.
...
In what follows, we denote our multiscale test by $\mathcal{T}_{\text{MS}}$ and the text based on the UCBs from \cite{Kim2016} by $\mathcal{T}_{\text{UCB}}$. Table \ref{tab:sizepower_UCB} presents the results of our comparison study. Panel (a) compares the empirical size of the two tests, while panels (b), (c) and (d) provide a power comparison. As can be expected, our test has good power properties compared to the competing procedure. For example, for the smallest sample size $T=100$, the UCBs for two trend functions remain relatively wide compared to the strength of the signal so that we almost never detect any differences. For moderate sample size $T=250$ our test perform well already for a moderate signal $b=0.5$ while $\mathcal{T}_{\text{UCB}}$ produces wide UCBs that the test barely has any power. Only for the largest sample size $T=500$, the test $\mathcal{T}_{\text{UCB}}$ reaches a power of $1$ for the strongest signal, with our test being more powerful in all of the other settings. Furthermore, in this comparison we do not apply any correction for multiple testing, but clearly any type of such correction would result in UCBs being wider and the test $\mathcal{T}_{\text{UCB}}$ needing even stronger signals and larger sample sizes to reach considerable power.

\begin{table}[t!]
\renewcommand{\arraystretch}{1.2}  
\footnotesize{
\begin{center}
\caption{Size and power comparison of our multiscale test $\mathcal{T}_{\text{MS}}$ and the procedure $\mathcal{T}_{\text{UCB}}$ based on UCBs from \cite{Kim2016}.}\label{tab:sizepower_UCB}
...
As before, we use notation $\mathcal{T}_{\text{MS}}$ for our multiscale test and we denote the SiZer test of \cite{Park2009} by $\mathcal{T}_{\text{SiZer}}$. Table \ref{tab:sizer} presents the results of our comparison study. Panel (a) compares the empirical size of the two tests, while panels (b) and (c) provide a power comparison. 
