...
The trend functions $m_i$ are not identified in model \eqref{eq:model_full}. In particular, equation \eqref{eq:model_full} is equivalent to $Y_{it} = m_i^\diamond(t/T) + \boldsymbol{\beta}_i^\top \boldsymbol{X}_{it} + \alpha_i^\diamond + \varepsilon_{it}$, where $m_i^\diamond = m_i + c_i^\diamond$, $\alpha_i^\diamond = \alpha_i - c_i^\diamond$ and $c_i^\diamond$ is an arbitrary real constant. For identification, we impose the constraint $\int_0^1 m_i(u) du = 0$ for all $i$, which vertically shifts each curve $m_i$ such that the overall mean level $\int_0^1 m_i(u)$ is equal to $0$. We briefly give some comments on this identification strategy:  
{\color{black}
\begin{itemize}[leftmargin=0.5cm,itemsep=0pt, parsep=0pt, topsep=3pt]

\item The normalization $\int_0^1 m_i(u) du = 0$ for all $i$ does not eliminate any information from the model for the following reason: as pointed out above, the observed data $\{(Y_{it},X_{it})\}$ are consistent with the trends $m_i$ and the fixed effects $\alpha_i$ as well as with the vertically shifted versions $m_i^\diamond$ and $\alpha_i^\diamond$. Hence, the data do not contain any information on the overall level of the curves $m_i$. Therefore, we are free to subtract it without any loss of information. 

\item The normalization $\int_0^1 m_i(u) du = 0$ for all $i$ is quite natural in many application contexts. In climatology, for instance, it is common to analyze temperature anomalies (i.e., the deviation of the temperatures from a long-term average) rather than the raw temperatures themselves. This amounts to vertically shift the trend functions $m_i$ by a long-term mean level. Another example is the application on house price data in Section \ref{sec:app}. The term $\int_0^1 m_i(u) du$ can be interpreted as the long-term average house price level in country $i$, which may be very different across countries. The normalization $\int_0^1 m_i(u) du = 0$ corrects for differences in these long-term price levels. Put differently, the normalized trends give the deviation from the long-term price levels. 

\item Consider the case where the trends $m_i$ are parallel, i.e., $m_i = m + c_i$ with some constants $c_i$ and a common trend $m$ for all $i$. The constraint $\int_0^1 m_i(u) du = 0$ implies that $c_i = c := -\int_0^1 m(u) du$ for all $i$. Put differently, it implies that the trends are exactly the same. Hence, under our identification constraint, the null hypothesis that the trends are parallel to each other coincides with the hypothesis that the trends are identical. 
...
\item \label{C-reg2} The matrix $\ex[\Delta \X_{it} \Delta \X_{it}^\top]$ is invertible for each $i$\textcolor{black}{, where $\Delta \X_{it} = \X_{it} - \X_{it-1}$}.
...
\end{enumerate}
Assumption \ref{C-reg1} guarantees that the process $\{ \X_{it}: 1 \le t \le T \}$ is stationary and causal for each $i$. \textcolor{black}{Notably, stationarity of the regressors rules out the autoregressive case with $\X_{it} = (Y_{i,t-1},\ldots,Y_{i,t-d})^\top$.} Assumption \ref{C-reg3} restricts the serial dependence of the process $\{ \X_{it}: 1 \le t \le T \}$ for each $i$ in terms of the physical dependence measure. 
...
\end{align}
In Lemma C.5 %\ref{lemma-beta-rate} 
...
{\color{black}
\section{Simulation study}\label{sec:app-sim-summary}


In the supplementary material, we evaluate our test and clustering methods
by Monte Carlo experiments. 
In the first part of the simulation study, we investigate the finite sample performance of the multiscale test: we explore the size and power properties of the test, we run a series of robustness checks (regarding the choice of tuning parameters, etc.) and we compare the test with competing methods.  
In the second part, we have a closer look at the clustering algorithm: we investigate how accurate our estimators of the unknown classes and their unknown number are and we compare our approach with a simple benchmark procedure. We refer the reader to Section A of the supplement for the details. 
}
