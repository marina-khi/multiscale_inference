...
Finally, many thanks for pointing out to us that the link to the code was broken. We have fixed this. The full replication code is currently available at \url{https://github.com/marina-khi/multiple_trends_code}.
...
In the revision, we discuss the issue of a time-varying error variance (or more generally a time-varying autocovariance structure) \textcolor{red}{in the new Section ??}.  
...
\textcolor{red}{We have added some remarks on FWER vs.\ FDR control to the new Section ?? of the paper.}
...
However, we believe that the population growth is one of the key determinants of the real house prices, and thus, needs to be accounted for by including it in the model. Hence, we have decided to add this robustness check to the Supplement (\textcolor{red}{see p. ??}) rather than to the main body of the paper.
...
We have changed the first $o_p$ to $O_p$. 
