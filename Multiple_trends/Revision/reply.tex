...
\item We choose $n$ to match the value in the application (or $n=10$ to get a round number) and choose different values for $T$ (one of them very close to the time series length in the application, e.g., $T=150$). 

\item We assume that each covariate process $\{X_{it,j}\}$ $(j=1,\ldots,4)$ is an AR(1) process of the form %$X_{it,j} = a_{j} X_{it,j-1} + \nu_{it,j}$
\textcolor{red}{$X_{it,j} = a_{j} X_{i(t-1),j} + \nu_{it,j}$}
 and estimate the AR parameters $a_{j}$ as well as the innovation variances $\sigma^2_{j} = \mathbb{E}[\nu_{it,j}^2]$ from the data. (Note: As the estimated $a_j$ and $\sigma_j^2$ may differ across time series $i$, we simply take their average over $i$.) Assuming that the innovations $\nu_{it,j}$ are normally distributed and independent across $j$, we can then easily simulate sample paths of the covariates.

\item We assume that the errors $\varepsilon_{it}$ follow the AR(1) model $\varepsilon_{it} = a \varepsilon_{i, t-1} + \eta_{it}$, where we set $a$ and the innovation variance $\mathbb{E}[\eta_{it}^2]$ equal to the estimated values (averaged over $i$) from the application. 
...
\item We take the grid $\mathcal{G}_T$ to be the same as in the application. 
