...
\begin{itemize}
	\item For the benchmark procedure, instead of $\mathcal{L}^2$ distance calculated the following distance: $d_{ij} = \max_{1\leq t\leq T} |\hat{m}_i(t/T) - \hat{m}_j(t/T)|$, where the local linear estimates $\hat{m}_i$ are calculated using the smallest bandwidth from $H_T$. The results are also okay, not as good as for $\mathcal{L}^2$ distance, but still better than our method.
	\item Raised the variance of the innovations in the error term: $\ex[\eta_{it}^2] = 1$. None of the procedures could detect anything (even for $T=500$). Too strong noise. 
	\item Played a bit with the shape of the functions: with smaller scales and smaller bumps for the third function, neither our procedure, nor the benchmarks can recognize anything.
%	\item Performed the procedure without normalization
	\item I have also compared the results for the multiscale procedure with the known number of clusters ($N=3$) for $T=100, 250$ and the results are only marginally better. I guess the idea here is that if we are able to detect the true number of clusters, then the grouping is also pretty good. But in most cases (in almost all cases for small sample sizes), we just do not reject the null. Something to do with the incorrect long-run variance estimation? I was thinking of maybe having a look at clustering results with a true theoretical value for lrv, what do you think?
\end{itemize}}
...
%The first empirical application, detailed in the main paper, focuses on comparing housing price trends across different countries. This example is particularly illustrative of the practical utility of the developed methods in real-world economic analysis. It provides a compelling case for how the multiscale test and clustering algorithm can uncover complex time trend relationships within global housing markets, a subject of broad interest to economists, policymakers, and the public alike​.

The second data application, discussed in the supplement, extends the insights of the first by explicitly revisiting and comparing the hypothesis of a common trend in the GDP growth time series of several OECD countries to the established method by Zhang et al. (2012). While the primary study in the main paper focuses on a novel multiscale comparison of nonparametric trend curves, the supplementary analysis serves a dual purpose. First, this application aims to explore a different facet of economic analysis by examining macroeconomic indicators across a set of economically developed countries. While this also demonstrates the versatility and applicability of the method developed in the paper, it inherently addresses a question of economic convergence or divergence among OECD countries, which may be seen as a more nuanced economic question in need of a substantial additional analysis. Secondly, this application provides a direct comparison to a previously established method, thus positioning the new approach within the existing literature of the comparison of time series.

The decision to present the first application in the main body of the paper and relegate the second to the supplement can be attributed to several factors, one of them being the  restriction on the page limit. Unfortunately, including both applications in the main text would have exceeded the permissible length for the publication. We have decided to present only one application that best showcases the methodological novelty and breadth of the proposed approach (comparison of the housing price data), while still offering the second application as a robust supplement. Furthermore, we also consider the analysis of the housing time trends 

The choice to present the first application over the second in the main body of the paper mostly reflects a strategic consideration of the broader and more immediate appeal of housing market trends to a wider audience, including economists, policymakers, and the general public. Housing markets are a direct concern for a significant portion of the population, and the insights provided by the first application have immediate practical implications for understanding global economic health, policy making, and investment decisions. In contrast, the examination of GDP growth trends among OECD countries, while undeniably valuable for macroeconomic analysis and policy formulation, may appeal to a more specialized audience primarily concerned with macroeconomic stability and economic policy.

In summary, the second data application in the Supplement enriches the discourse initiated in the main paper by applying the developed methods to a different but equally important economic phenomenon. The choice to feature the housing market analysis in the main body underscores a prioritization of broader interest and applicability. However, both applications collectively highlight the versatility and the significant potential of the newly developed econometric methods for enhancing our understanding of complex economic trends across different contexts.


 Firstly, it applies the developed methodology to a real-world dataset, thereby illustrating its practical utility beyond the initial example. 


Furthermore, the choice underscores a prioritization of content that, while demonstrating the utility of the proposed methods in a real-world context, also maintains a focus on the theoretical innovation and methodological advancements introduced in the paper. The supplementary application not only reinforces the main paper's findings but also enriches the reader's understanding by offering a comparative analysis with an established method, thereby highlighting the new method's significance and applicability in a broader context. This strategic organization of content ensures that the main narrative remains focused and accessible while still providing comprehensive insights into the method's practical implications and advantages over existing approaches.
