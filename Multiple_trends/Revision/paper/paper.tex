...
\begin{enumerate}[label=(C\arabic*),leftmargin=1.05cm, itemsep=0pt, parsep=0pt, topsep=0pt]
\item \label{C-err1} 
...
be the hypothesis that $m_i$ and $m_j$ are identical on the interval $\interval$. $H_0^{[i, j]}(u, h)$ can be viewed as a local null hypothesis that characterises the behaviour of two trend functions locally on the interval $\interval$, whereas $H_0: m_1 = m_2 = \ldots = m_n$ is the global null hypothesis that is concerned with the comparison of all trends on the whole unit interval $[0, 1]$. We consider a large family of time intervals $\interval$ that fully cover the unit interval. More formally, we consider all intervals $\interval$ with $(u,h) \in \grid$, where $\grid$ is a set of location-scale points $(u,h)$ with the property that $\bigcup_{(u, h) \in \grid} \interval = [0,1]$. Technical conditions on the set $\grid$ are given below. The global null $H_0$ can now be formulated as 
\begin{align*}
...
%\end{align}
%where $s_T$ is the length of the subseries and $M = \lfloor T/s_T\rfloor$ is the largest integer not exceeding $T/s_T$. As per the optimality result in \cite{Carlstein1986}, we set $s_T \asymp T^{1/3}$.  
%%When implementing $\widehat{\sigma}_i^2$, we in particular choose $s_T = \lfloor T^{1/3}\rfloor$. 
%According to  Lemma B.6 
%%\ref{lemmaA:lrv} 
...
A statistic to test the local null hypothesis $H_0^{[i, j]}(u, h)$ for a given location-scale point $(u,h)$ and a given pair of time series $(i,j)$ can be constructed as follows: Let $\widehat{m}_{i,T}(u,h) = \sum_{t=1}^T w_{t,T}(u, h) \widehat{Y}_{it}$ be the local linear estimator of $m_i$ constructed from the pre-processed time series $\{\widehat{Y}_{it}: 1 \le t \le T\}$. Here, $w_{t,T}(u, h)$ are local linear kernel weights of the form
...
\begin{enumerate}[label=(C\arabic*),leftmargin=1.05cm,topsep=0pt]
\setcounter{enumi}{8}
...
\end{enumerate} 
We now consider the kernel average 
\begin{align}
\widehat{\psi}_{ij, T}(u, h) 
& := \widehat{m}_{i,T}(u,h) - \widehat{m}_{j,T}(u,h) = \sum\limits_{t=1}^T w_{t,T}(u, h)(\widehat{Y}_{it} - \widehat{Y}_{jt}), \label{eq:psi_hat_ij}
\end{align}
which can be regarded as a measure of distance between the two trend curves $m_i$ and $m_j$ on the interval $\interval = [u-h, u+h]$. We may thus use a normalized (absolute) version of $\widehat{\psi}_{ij, T}(u, h)$, in particular, the term 
...
\begin{enumerate}[label=(C\arabic*),leftmargin=1.2cm, itemsep=0pt, parsep=0pt, topsep=0pt]
\setcounter{enumi}{9}
...
\textcolor{red}{[Add example for grid!]}
...
\begin{remark}
Importantly, this choice of $q_{n,T}(\alpha)$ can be computed by Monte Carlo simulations in practice: under our assumption that the long-run error variance $\sigma_i^2$ does not depend on $i$ (i.e. $\sigma_i^2 = \sigma^2$ for all $i$), we can rewrite the statistics from \eqref{eq:phi_zero_ij} as
...
\item locally stationary covariates and errors, in particular time-varying error variances
\item FWER vs.\ FDR control
\item connection to uniform confidence bands
\end{itemize}
