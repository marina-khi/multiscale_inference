...
spurious_power_array <- array(NA, dim = c(length(different_T),
                                          length(different_b),
...
for (t_len in different_T){
  ...
  #Restricting the grid to only look at the spurious power between ts 1 and others
  gset_pos      <- grid$gset
  ...
  grid_spurious <- construct_grid(t = t_len, u_grid = u_grid, h_grid = h_grid, deletions = deletions)
  
  ijset_first <- expand.grid(i = 1, j = 1:n_ts)
  ijset_first <- ijset_first[ijset_first$i < ijset_first$j, ]
  
  ijset_other <- expand.grid(i = 2:n_ts, j = 1:n_ts)
  ijset_other <- ijset_other[ijset_other$i < ijset_other$j, ]
  
  #Calculating the test statistics between ts 1 and others
  tic()
  cl <- makePSOCKcluster(numCores)
  registerDoParallel(cl)
  foreach (val = 1:n_rep, .combine = "cbind") %dopar% {
    source("functions/functions.R")
    repl_spurious(rep_ = val, n_ts_ = n_ts, t_len_ = t_len,
         grid1_ = grid_spurious, grid2_ = grid,
         ijset1_ = ijset_first, ijset2_ = ijset_other,
         a_ = a, sigma_ = sigma,
  ...
  for (j in 1:length(different_b)){
    ...
    for (alpha in different_alpha){
      ...
      spurious_power_vec <- c(spurious_power_vec, num_of_rej) 

      cat("Ratio of spurious rejections is ",
          num_of_rej, "with b = ", different_b[j],
          ", alpha = ", alpha, "and T = ", t_len, "\n")
    }
    
    #Storing the results in a 3D array
    spurious_power_array[k, j, ] <- spurious_power_vec
  }
}
...
for (b in different_b){
  ...
  tmp <- as.matrix(spurious_power_array[, l, ])
  filename = paste0("output/revision/", n_ts, "_ts_", phi*100, "_", rho * 100, "_spurious_power_b_",
                    b * 100, ".tex")
  ...
  line <- paste0("%This simulation was done for the seed ", seed,
                 ...
                 ", with ", n_rep, " simulations for calculating spurious power and ", sim_runs,
