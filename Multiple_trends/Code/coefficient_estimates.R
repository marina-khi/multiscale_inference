...
for (t_len in different_T){
  ...
  for (j in 1:length(different_b)){
    simulated_beta <- simulated_beta_and_sigma[((j - 1) * (length(beta) + 1) + 1):(j * (length(beta) + 1) - 1), ]
    simulated_sigma <- simulated_beta_and_sigma[j * (length(beta) + 1), ]
    

    filename = paste0("output/revision/sigma_histogram_bump_function_T_", t_len, "_b_", different_b[j] * 100, ".pdf")
  ...
  foreach (val = 1:n_rep, .combine = "cbind") %dopar% {
    ...
    repl_precision(rep_ = val, n_ts_ = n_ts, t_len_ = t_len,
                   ...
                   beta_ = beta, a_x_vec_ = a_x_vec, phi_ = phi, rho_ = rho,
                   different_b_ = different_b,
                   q_ = q, r_ = r, type_of_m_ = "")
    # Loop one-by-one using foreach
  } -> simulated_beta_and_sigma2
  stopCluster(cl)
  toc()
  
  for (j in 1:length(different_b)){
    simulated_beta <- simulated_beta_and_sigma2[((j - 1) * (length(beta) + 1) + 1):(j * (length(beta) + 1) - 1), ]
    simulated_sigma <- simulated_beta_and_sigma2[j * (length(beta) + 1), ]
    
    
    filename = paste0("output/revision/sigma_histogram_zero_function_T_", t_len, "_b_", different_b[j] * 100, ".pdf")
    pdf(filename, width = 7, height = 5, paper="special")
