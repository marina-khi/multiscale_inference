...
t_len <- 100 #Different lengths of time series
different_alpha <- c(0.01, 0.05, 0.1) #Different confidence levels
b <- 0 #Zero is for calculating the size
...
#size_and_power_array_sizer <- array(NA, dim = c(length(different_T),
#                                                length(different_b),
#                                                length(different_alpha)),
#                                    dimnames = list(t = different_T,
#                                                    b = different_b,
...
foreach (val = 1:n_rep, .combine = "cbind") %dopar% {
  repl_revision2(rep_ = val, n_ts_ = n_ts, t_len_ = t_len, grid_ = grid,
                 a_ = a, sigma_ = sigma,
                 m_matrix_ = m_matrix, true_lrv_ = true_lrv, 
                 gaussian_sim = FALSE)
      # Loop one-by-one using foreach
