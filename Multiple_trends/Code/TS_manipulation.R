...
# for (s in 1:10){
#   for (p in 1:10){
#     #Taking 10x10 grid instead of 1x1 and yearly observation instead of monthly
#     tmp2 <- array(NA, dim = c(dim(tmp)[1] / 10, dim(tmp)[2] / 10, dim(tmp)[3] / 12))
#     for (i in 1:(dim(tmp)[1] / 10)){
#       for (j in 1:(dim(tmp)[2] / 10)){
#         for (k in 1:(dim(tmp)[3] / 12)){
#           tmp2[i, j, k] <- mean(tmp[10 * (i - 1) + s, 10 * (j - 1) + p, (12 * (k - 1) + 1):(12 * k)])
#         }
#       }
#     }
# 
...
#       }
#     }
#     if (n_ts > 90) {cat("For s =", s, " and p =", p, ", we have n_ts =", n_ts, "\n")}
#   }
# }
...
for (i in 1:(dim(tmp)[1] / 10)){
  for (j in 1:(dim(tmp)[2] / 10)){
    for (k in 1:(dim(tmp)[3] / 12)){
      tmp2[i, j, k] <- mean(tmp[10 * (i - 1) + 6, 10 * (j - 1) + 1, (12 * (k - 1) + 1):(12 * k)])
    }
  }
}
dimnames(tmp2) <- list(Col1 = lon[seq(6, length(lon), by = 10)],
                       Col2 = lat[seq(1, length(lat), by = 10)],
...
pdf("output/plots/dendrogram_10by10.pdf", width = 15, height = 6, paper = "special")
par(cex = 1, tck = -0.025)
