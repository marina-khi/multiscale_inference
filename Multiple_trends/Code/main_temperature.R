...
for (i in 1:(dim(tmp2)[1])){
  for (j in 1:(dim(tmp2)[2])){
    if (sum(is.na(tmp2[i, j, 111:(dim(tmp2)[3])])) == 0){
      n_ts <- n_ts + 1
    }
  }
}
...
t_len       <- dim(tmp2)[3] - 111 + 1 #Length of time series
...
for (i in 1:(dim(tmp2)[1])){
  for (j in 1:(dim(tmp2)[2])){
    if (sum(is.na(tmp2[i, j, 111:(dim(tmp2)[3])])) == 0){
      temp_matrix[, k] <- tmp2[i, j, 111:(dim(tmp2)[3])] - mean(tmp2[i, j, 111:(dim(tmp2)[3])])
...
dates <- dimnames(tmp2)$Col3[111:(dim(tmp2)[3])]
