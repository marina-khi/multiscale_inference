...
In this section we adapt the multiscale test method developed in Section \ref{subsec-method-test} to test more interesting hypothesis $H_0$


Consider the time trend model 
\[ Y_t = m \Big( \frac{t}{T} \Big) + \varepsilon_t \]
for $t=1,\ldots,T$ with $\ex[\varepsilon_t] = 0$. We want to test the null hypothesis 
\[ H_0: m \text{ is constant}. \]
%Put differently, for a pre-specfied confidence level, we want to identify a set of intervals $[u-h,u+h] \subseteq [0,1]$, where $m$ is not constant. We want to identify a set of intervals $S^\pm \subseteq S$ with the following property: for a pre-specified confidence level, we can conclude that $m$ is not constant on any of the intervals in $S^\pm$. 
As in Section {subsec-method-test}, this can be achieved with the help of the following multiscale test statistic: 
\[ \mathcal{M}_T = \max_{(u,h) \in \mathcal{G}_T} \Big\{ |\Psi_T(u,h)| -\lambda(h)\}, \]

% \sqrt{2 \log \big( \textstyle{\frac{1}{2h}} \big)} \Big
 
 where 
\[ \Psi_T(u,h) = \sum\limits_{t=1}^T w_{t,T}(u,h) \varepsilon_t \]
with
\[ w_{t,T}(u,h) =  \frac{1}{\hat{\sigma} \sqrt{Th}} \, K^\prime\Big( \frac{u - t/T}{h} \Big), \]
where $\hat{\sigma}^2$ is a suitable estimator of the long-run error variance $\sigma^2$. 
...
Suppose we observe $n$ different time series $\mathcal{Y}_i = \{Y_{it}: 1 \le t \le T\}$ for $1 \le i \le n$. The $i$-th time series is assumed to follow the time trend model
\[ Y_{it} = m_i\Big(\frac{t}{T}\Big) + \alpha_i + \varepsilon_{it} \]
for $1 \le t \le T$, where $\ex[\varepsilon_{it}] = 0$ and $\alpha_i$ are (deterministic or random) intercept terms. (We here set $\alpha_i = 0$ for simplicity and incorporate the intercepts $\alpha_i$ later into the procedure.) We want to test the null hypothesis that 
...
\[ \mathcal{M}_{ij,T} = \max_{L \in \mathcal{L}} \max_{\substack{1 \le t \le t ^\prime \le T \\ |t^\prime - t + 1| = L}} \Big\{ |\Delta_{ij}(t,t^\prime)| - \sqrt{2 \log \big( \textstyle{\frac{T}{L}} \big)} \Big\}, \]
where 
\[ \Delta_{ij}(t,t^\prime) = \frac{1}{\sqrt{|t^\prime-t+1|}} \sum\limits_{s=t}^{t^\prime} V_s \quad \text{with} \quad V_{ij,s} = \frac{(Y_{is} -\ex[Y_{is}]) - (Y_{js} -\ex[Y_{js}])}{2\sigma} \]
for $t^\prime \ge t$. The test statistic $\mathcal{M}_T$ is then defined as
\[ \mathcal{M}_T = \max_{1 \le i < j \le n} \mathcal{M}_{ij,T}. \]
