...
\renewcommand{\arraystretch}{1.2}
\input{Plots/30stations_sizetable_method_ll}
\end{center}}
\end{table}
...
\[ Y_{it} = \alpha_i^j + m_i\Big(\frac{t}{T}\Big) + \varepsilon_{it}, \]
where $m_i$ are unknown time trends of interest, $j \ in \{1, \ldots, 12\}$ denotes a month, $\alpha_i^j$ is a trend- and month-specific intercept and the error process $\{ \varepsilon_{it} \}$ is assumed to have an AR(1) structure $\varepsilon_{it} = a_i \varepsilon_{i,t-1} + \eta_{it}$. As previously, $\eta_{it}$ are i.i.d. innovations with mean $0$ and variance $\sigma_{\eta_i}^2$. For each $i$ we separately estimate the unknown parameters $a_i$ and $\sigma_{\eta_i}^2$  by the procedure described in Section \ref{subsec-error-var-ar}.
