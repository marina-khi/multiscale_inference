...
Write $\widehat{\Psi}_T(u,h) = \widehat{\psi}_T^A(u,h) + \widehat{\psi}_T^B(u,h)$ with $\widehat{\psi}_T^A(u,h) = \sum\nolimits_{t=1}^T w_{t,T}(u,h) \varepsilon_t$ and $\widehat{\psi}_T^B(u,h) = \sum\nolimits_{t=1}^T w_{t,T}(u,h) m_T(t/T)$. By assumption, there exists $(u,h) \in \mathcal{G}_T$ with $[u-h,u+h] \subseteq [0,1]$ such that $m_T(w) \ge c_T \sqrt{\log T/(Th)}$ for all $w \in [u-h,u+h]$. For this $(u,h)$ and sufficiently large $T$, it holds that 
\begin{equation}\label{eq1-proof-prop-test-2}
\widehat{\psi}_T^B(u,h) \ge c_T \sqrt{\frac{\log T}{Th}} \sum\limits_{t=1}^T w_{t,T}(u,h).
\end{equation}
Moreover, standard calculations show that for any $(u,h) \in \mathcal{G}_T$ with $[u-h,u+h] \subseteq [0,1]$, 
\begin{equation}\label{eq2-proof-prop-test-2}
\Big| \sum\limits_{t=1}^T w_{t,T}(u,h) - \kappa^{-1} \sqrt{Th} \Big| \le \frac{C}{Th}, 
\end{equation}
where $\kappa = (\int K^2(\varphi)d\varphi)^{1/2}$ and the constant $C$ is independent of $u$, $h$ and $T$. Combining \eqref{eq1-proof-prop-test-2} and \eqref{eq2-proof-prop-test-2}, we can infer that 
\begin{equation}\label{eq3-proof-prop-test-2}
\max_{(u,h) \in \mathcal{G}_T} \big| \widehat{\psi}_T^B(u,h) \big| \ge \frac{c_T \sqrt{\log T}}{2 \kappa} 
\end{equation}
for sufficiently large $T$. Furthermore, arguments very similar to those for the proof of Proposition \ref{propA-strong-approx} yield that
\begin{equation}\label{eq4-proof-prop-test-2}
\max_{(u,h) \in \mathcal{G}_T} |\widehat{\psi}_T^A(u,h)| = O_p(\sqrt{\log T}). 
\end{equation}
With the help of \eqref{eq3-proof-prop-test-2}, \eqref{eq4-proof-prop-test-2} and the fact that $\lambda(h) \le \lambda(h_{\min}) \le C \sqrt{\log T}$, we obtain that 
\begin{align}
\widehat{\Psi}_T 
 ...
 & \ge \frac{c_T \sqrt{\log T}}{2 \kappa \widehat{\sigma}} + O_p(\sqrt{\log T}). \label{eq5-proof-prop-test-2}
\end{align}  
Since $q_T(\alpha) = O(\sqrt{\log T})$ for any fixed $\alpha \in (0,1)$, \eqref{eq5-proof-prop-test-2} immediately implies that $\pr(\widehat{\Psi}_T \le q_T(\alpha)) = o(1)$. 
