...
\begin{frame}{Motivation}
	...
	\begin{figure}
  	...
  	\end{figure}}	
{\onslide<4> \begin{block}{Aim of the paper}
...
Suppose we have a set of intervals of different lengths, and we want to be able to say if two underlying trend functions are equal on each of these intervals.\pause

Then we would like a procedure that:
\vspace{-1mm}
\begin{itemize}
	\item allows us to test multiple hypotheses simultaneously;\pause
	\item taking the differences in lengths of the intervals into account;\pause
	\item controls the family-wise error rate (probability of making even one type I error); \pause
	\item while having an asymptotic power of $1$ against local alternatives;\pause
	\item is easy to use and ideally already has an implementation as an R-package.
\end{itemize}\pause
...
\end{equation*}
where $\eta_{it}$ has zero mean and unit variance.\pause
...
&+  \frac{\sigma}{Th_k} \sum\limits_{t=1}^T \ind \Big(\sqrt{\lambda_i\Big(\frac{t}{T}\Big)} \eta_{it} -  \sqrt{ \lambda_j\Big(\frac{t}{T}\Big)} \eta_{jt} \Big)
\end{align*}\pause
Applying a law of large numbers, we get that
\begin{align*}
\hat{s}_{ijk} = &\frac{1}{Th_k} \sum\limits_{t=1}^T \ind \bigg( \lambda_i \Big(\frac{t}{T}\Big)  - \lambda_j \Big(\frac{t}{T}\Big)\bigg) + o_P(1)
\end{align*}
\end{frame}
...
&= 1 - \Prob \Big( \forall (i,j,k) \in\mathcal{M}_0: |\widehat{\psi}_{ijk} | \le c_{ijk}(\alpha) \Big)\\
 ...
 & \le \alpha
\end{align*}\pause
Hence, we choose $q(\alpha)$ as the $(1-\alpha)$-quantile of the statistic 
\[ \hat{\Psi} = \max_{(i,j,k)} a_k \big( |\hat{\psi}_{ijk}^0| - b_k \big), \]
...
\begin{enumerate}
	...
	\vspace{-2mm} \[ \Phi = \max_{(i,j,k)} a_k \big( |\phi_{ijk}| - b_k \big), \] where $a_k$ and $b_k$ are scale-dependent constants and $\phi_{ijk}$ are weighted averages of the differences of standard normal random variables.\pause
	\item Compute a $(1-\alpha)$-quantile $q_{\text{Gauss}} (\alpha)$ of $\Phi$ by Monte Carlo simulations.\pause
...
A more modern approach of constructing the individual critical values $c_{ijk}(\alpha)$ (D{\"u}mbgen and Spokoiny (2001)): let them depend on the scale of the testing problem, i.e. the length $h_k$ of the time interval.\pause \,Specifically, 
\[c_{ijk}(\alpha) = c(\alpha,h_k) := b_k + q(\alpha)/a_k,\] where $a_k = \{\log(e/h_k)\}^{1/2} / \log \log(e^e / h_k)$ and $b_k = \sqrt{2 \log(1/h_k)}$ are scale-dependent constants and $q_T(\alpha)$ is chosen such that we control FWER.
