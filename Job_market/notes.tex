...
SEIR (Susceptible, Exposed, Infected, Removed) model is one of the most widely used models in the study of the spread of COVID-19: \cite{Yang2020}, \cite{Wu2020}, \cite{deBrower2020}.

However, this model has significant disadvantages. First, traditional SEIR model assumes a closed system (no individuals move in or out), which is a reasonable assumptions under the restrictions on travel. However, either introduced these restrictions very late, or lifted them quite early. Hence, the closed system assumption may not be plausible for substantial periods of time. Second, SEIR framework only takes into account human-to-human transmission of the infection, whereas the indirect transmission pathway from the environment to human hosts is also a highly possible route to spread the infection. Then, one of the main parameters of SEIR model is the average number of infections an infectious individual will cause and the setting does not allow for smooth changes of this parameter. For example, \cite{deBrower2020} assume that the lockdown enforcement results in a discrete, instantaneous change in this parameter, which is a very questionable assumption. Most countries implemented the lockdown measures gradually, from cancelling big events to full lockdown, which probably reduced the transmissibillity of the virus step by step. On the other hand, our completely nonparametric approach does not have this limitation.
