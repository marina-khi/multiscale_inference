...
#         #   plot(lambda_vec_2,  ylim = c(0, max(lambda_vec_2) + 100), xlab="u", ylab = "", mgp=c(2,0.5,0), type = "l")
...
#                  type="latex", file=paste0("plots/size_overdispersion_", sigma, "_const_c_", c, ".tex"), include.colnames = FALSE)
#   }


for (sigma in sigma_vec){
  power_matrix_global            <- matrix(NA, nrow = length(t_len_vec), ncol = number_of_cols)
  rownames(power_matrix_global) <- paste0("$T = ", t_len_vec, "$")
  power_matrix_local            <- matrix(NA, nrow = length(t_len_vec), ncol = number_of_cols)
  rownames(power_matrix_local)  <- paste0("$T = ", t_len_vec, "$")
  ...
  for (n_ts in n_ts_vec){
    ...
    for (t_len in t_len_vec){
      ...
      lambda_vec_2 <- lambda_fct((1:t_len) / t_len, c = 1000, height = 6000, position = 10)

      pdf(paste0("plots/lambda_fcts_different_heights_", t_len, ".pdf"), width=5, height=5, paper="special")
      par(mar = c(3, 2, 2, 0)) #Margins for each plot
      par(oma = c(0.2, 0.2, 0.2, 0.2)) #Outer margins
      ...
      #set.seed(1234) # This is for calculating power for different specifications on comparable datasets
      ...
      power_matrix_global[i, (k * length(alpha_vec) - (length(alpha_vec) - 1)):(k * length(alpha_vec))] <- power$global_power
      power_matrix_local[i, (k * length(alpha_vec) - (length(alpha_vec) - 1)):(k * length(alpha_vec))] <- power$local_power
      i <- i + 1
    }
    k <- k + 1
  }
    
  print.xtable(xtable(power_matrix_global, digits = c(3),
                      align = paste(replicate(number_of_cols + 1, "c"), collapse = "")),
               type="latex", file=paste0("plots/global_power_sigma_", sigma, "_higher_peak.tex"),
               include.colnames = FALSE)
  print.xtable(xtable(power_matrix_local, digits = c(3),
                      align = paste(replicate(number_of_cols + 1, "c"), collapse = "")),
               type="latex", file=paste0("plots/local_power_sigma_", sigma, "_higher_peak.tex"),
               include.colnames = FALSE)
}
...
for (sigma in sigma_vec){
  ...
  for (n_ts in n_ts_vec){
    ...
    for (t_len in t_len_vec){
      ...
      pdf(paste0("plots/lambda_fcts_shift_to_right_", t_len, ".pdf"), width=5, height=5, paper="special")
      par(mar = c(3, 2, 2, 0)) #Margins for each plot
      par(oma = c(0.2, 0.2, 0.2, 0.2)) #Outer margins
      plot(1:t_len, lambda_vec_1,  ylim = c(0, max(lambda_vec_1, lambda_vec_2) + 100), xlab="u", ylab = "", type = "l")
      lines(1:t_len, lambda_vec_2, type = "l", col = "red")
  ...
  print.xtable(xtable(power_matrix_global, digits = c(3),
                      align = paste(replicate(number_of_cols + 1, "c"), collapse = "")),
               type="latex", file=paste0("plots/global_power_sigma_", sigma, "_shifted_peak.tex"),
               include.colnames = FALSE)
  print.xtable(xtable(power_matrix_local, digits = c(3),
                      align = paste(replicate(number_of_cols + 1, "c"), collapse = "")),
               type="latex", file=paste0("plots/local_power_sigma_", sigma, "_shifted_peak.tex"),
               include.colnames = FALSE)
}
