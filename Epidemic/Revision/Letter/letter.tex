...
However, since the precise conditions on the growth of $p$ cannot be stated without reference to the technical assumptions in the Appendix, we have deferred the mathematically exact formulation of the conditions on $p$ to the Appendix (see Theorem A.1 and the discussion thereafter on p.26). 
...
For this reason, we have decided not to defer its definition to the Appendix but to introduce it in Section 3 as in the old version of the paper. Otherwise, an important detail of the test would be missing in the main text. We hope you are fine with this. To emphasize that the statistic $\hat{\psi}_{ijk,T}^0$ is required for the definition of the critical values of the multiscale test, we have added the following sentence after its introduction in equation (3.2) (which is equation (3.4) in the revised paper): ``This auxiliary statistic is needed to define the critical values of our multiscale test in what follows''.
