...
Even though this is possible in principle, we follow most other multiscale approaches in the literature (see e.g.\ ??, ??, ?? and many others) which work with local mean distances rather than local $L_q$ distances. The reason is mainly technical: The theory in our paper would be very different if we worked with local $L_q$ distances. 
...
In particular, we could not make use of the Gaussian approximation results from Chernozhukov ?? as far as we can see. 
