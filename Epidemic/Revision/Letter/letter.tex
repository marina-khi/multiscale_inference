...
%The union $U_{ij}$ gives a first rough idea about where the trends $\lambda_i$ and $\lambda_j$ differ from each other. In particular, 
...
of minimal intervals, where $\mathcal{F}_{\text{reject}}^{\text{min}}(i,j) = \{ \mathcal{I}_k \in \mathcal{F}_{\text{reject}}(i,j):$  there exists no $\mathcal{I}_{k^\prime} \in \intervals_{\text{reject}}(i,j)$ with $\mathcal{I}_{k^\prime} \subset \mathcal{I}_k \}$ denotes the set of minimal intervals. %We have added some discussion of the quantities $U_{ij}$ and $U_{ij}^{\min}$ to Section ?? of the revised paper. 
...
%It would of course be nice to formally relate $U_{ij}^{\min}$ to the set of time points 
%\[ U_{ij}^* = \{ u \in [0,1]: \lambda_i(u) \ne \lambda_j(u) \}, \]
%where $\lambda_i$ and $\lambda_j$ in fact differ from each other. In particular, it would be nice to show that the difference between the two sets $U_{ij}$ and $U_{ij}^*$ is small with high probability in some sense. In simple special cases, this is indeed possible. Here is an example: Suppose that $U_{ij}^*$ is an interval of the form $U_{ij}^* = (t_1,t_2)$ with $t_1,t_2 \in [0,1]$ and consider  a very simple family of intervals $\mathcal{F} = \{ \mathcal{I}_k:1 \le k \le K \}$. In particular, suppose that all intervals $\mathcal{I}_k \in \mathcal{F}$ have the same length $h$ (with $h \to 0$ more slowly than $T^{-1/3}$). In this case, Theorem A.1 and the new Proposition A.1 (which gives the power properties of the test) immediately imply the following: 
%\begin{equation}\label{Uij}
%\pr  \Big( \Delta(U_{ij}, U_{ij}^*) \le C h \Big) \ge 1-\alpha + o(1), \tag{$*$}
%\end{equation}
%where $\Delta(U_{ij},\hat{U}_{ij}) = \mathcal{L} \{(U_{ij} \setminus \hat{U}_{ij}) \cup (\hat{U}_{ij} \setminus U_{ij})\}$ is the Lebesgue measure of the symmetric difference between the two sets $U_{ij}$ and $U_{ij}^*$. According to \eqref{Uij}, the difference between $U_{ij}$ and $U_{ij}^*$ is small ($\le Ch = o(1)$) with high probability ($\ge 1 -\alpha + o(1)$). In this sense, $U_{ij}$ can be regarded as an approximation of $U_{ij}^*$. 


%Under more general conditions (in particular those from the paper), it is possible to prove the following extension of $\eqref{Uij}$:  
%\begin{equation}\label{Uij-general}
%\pr  \Big( \Delta(U_{ij}, U_{ij}^*) \le C h_{\max} \Big) \ge 1-\alpha + o(1), \tag{$**$}
%\end{equation}
%where $h_{\max}$ is the largest interval length in $\mathcal{F}$. The problem here is of course that this result is not very precise. In particular, if $h_{\max}$ converges very slowly to zero, then we only have a very rough bound on the difference $\Delta(U_{ij}, U_{ij}^*)$.  


%\newpage

%It turns out, however, that it is extremely difficult (and overly technical) to formulate such confidence statements in a rigorous way. In simple special cases, this is possible though. Here is a very simple example: Suppose that $U_{ij}^* = (t_1,t_2)$ with $t_1,t_2 \in [0,1]$. Moreover suppose we work with a very simple family of intervals $\mathcal{F} = \{ \mathcal{I}_k:1 \le k \le K \}$. In particular, suppose that all intervals $\mathcal{I}_k \in \mathcal{F}$ have the same length $h$ (with $h \to 0$ as $T \to \infty$). In this case, it follows immediately from Theorem A.1 and the new Proposition A.1 which gives the power properties of the test that the following holds: 
%\begin{equation}\label{Uij}
%\pr  \Big( \Delta(U_{ij}, U_{ij}^*) \le C h \Big) \ge 1-\alpha + o(1), \tag{$*$}
%\end{equation}
%where $\Delta(U_{ij},\hat{U}_{ij}) = \mathcal{L} \{(U_{ij} \setminus \hat{U}_{ij}) \cup (\hat{U}_{ij} \setminus U_{ij})\}$ is the Lebesgue measure of the symmetric difference between the two sets $U_{ij}$ and $U_{ij}^*$. Acocrding to \eqref{Uij}, the difference between $U_{ij}$ and $U_{ij}^*$ is small ($\le Ch = o(1)$) with high probability ($\ge 1 -\alpha + o(1)$). In this sense, $U_{ij}$ can be regarded as an approximation of $U_{ij}^*$. 
