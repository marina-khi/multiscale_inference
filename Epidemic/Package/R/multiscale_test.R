...
multiscale_test <- function(data, sigma_vec, n_ts = 1, grid = NULL,
                            alpha = 0.05, sim_runs = 1000, deriv_order = 0,
                            epidem = FALSE) {
  ...
  quantiles <- compute_quantiles(t_len, grid, n_ts, sigma_vector = sigma_vec,
                                 sim_runs = sim_runs,
  ...
  if (n_ts == 1) {
    ...
    psi   <- compute_statistics(t_len = t_len, data = data, gset = gset_cpp,
                                n = n, sigma = sigma, deriv_order = deriv_order)
    ...
    return(list(quant = quant, statistics = psi$stat, test_matrix = test,
                gset_with_vals = gset))

  } else {
    psi_ij <- compute_multiple_statistics(t_len = t_len, n_ts = n_ts,
                                          data = data, gset = gset_cpp, n = n,
                                          sigma_vec = sigma_vec,
                                          deriv_order = deriv_order,
                                          epidem = epidem)
    if (max(psi_ij$stat, na.rm = TRUE) > quant) {
      cat("We reject H_0 with probability", alpha, ". Psihat_statistic = ",
          max(psi_ij$stat, na.rm = TRUE), ". Number of pairwise rejections = ",
          sum(psi_ij$stat > quant, na.rm = TRUE),
          ". Gaussian quantile value = ", quant, "\n")
    } else {
      cat("We fail to reject H_0 with probability", alpha,
          ". Psihat_statistic = ", max(psi_ij$stat, na.rm = TRUE),
          ". Gaussian quantile value = ", quant, "\n")
    }
    ...
    for (i in seq_len((n_ts * (n_ts - 1) / 2))) {
      test_results           <- psi_ij$vals_cor_matrix[, i] > quant
      test_res[pos_full]     <- test_results
      test[[i]]              <- matrix(test_res, ncol = length(u_grid_full),
                                       byrow = TRUE)
      gset$test              <- test_results
      gset_with_values[[i]]  <- gset
    }

    return(list(quant = quant, statistics = psi_ij$stat, test_matrices = test,
                gset_with_vals = gset_with_values))
  }
}
