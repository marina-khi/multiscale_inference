#' multiscale-package: Multiscale Inference for Nonparametric Regression(s)
...
#'   in the model \eqn{Y_t = m(t/T) + \eps_t} with time series errors
#'   \eqn{\eps_t} is provided. The method was first proposed in
...
#'   \eqn{m_i} in the model \eqn{Y_{i, t} = m_i(t/T) + \eps_{i, t}} with
#'   time series errors \eqn{\eps_{i, t}} is provided. The method was first
#'   proposed in Khismatullina and Vogt (??). It allows to test for comparison
#'   of the trend \eqn{m_i} and further to cluster the regressions based on
#'   the estiamted difference between the trends.
#'
#'   In order to perform these methods, an estimator of the long-run error
#'   variance \eqn{\sigma^2 = \sum_{l=-\infty}^{\infty} Cov(\eps_0, \eps_l)}
#'   is required. Hence, the package also provides the difference-based
