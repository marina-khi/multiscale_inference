...
NumericVector kernel_averages(int t_len, NumericVector gset, NumericVector correct, NumericVector data,
                              double sigmahat, int n, int deriv_order){
   /* Inputs: 
      t_len        	length of time series
...
// [[Rcpp::export]]
NumericVector simulate_gaussian(int t_len, int n_ts, int sim_runs, Rcpp::NumericVector gset, int n,
                                Rcpp::NumericVector sigma_vec, int deriv_order = 0, bool epidem = false){
         ...
         for (i = 0; i < n_ts - 1; i++) {
            for (j = i + 1; j < n_ts; j++) {
               ...
               for(k = 0; k < n; k++){
                  ...
                  result[k] = correct_a[k] * (awert(result_temp) / (sqrt(2 * t_len * 2 * h) * sigmahat) - correct_b[k]);
               }
               Phi(i, j) = max(result);
            }
         }
...
// [[Rcpp::export]]
Rcpp::List compute_multiple_statistics(int t_len, int n_ts, Rcpp::NumericMatrix data, Rcpp::NumericVector gset,
                              int n, Rcpp::NumericVector sigma_vec, int deriv_order = 0,
                              bool epidem = false){
   Rcpp::NumericVector correct_b(n);
   Rcpp::NumericVector correct_a(n);
   Rcpp::NumericMatrix stat(n_ts, n_ts);
   Rcpp::NumericMatrix vals_cor_mat(n, n_ts * (n_ts - 1) / 2);
   NumericVector vals(2 * n);
   NumericVector vals_cor(n);
   ...
   if (epidem){
      ...
      for (i = 0; i < n_ts - 1; i++){
         for (j = i + 1; j < n_ts; j++){
            ...
            sigmahat = sqrt(sigma_vec(i) * sigma_vec(i) + sigma_vec(j) * sigma_vec(j));
            vals = kernel_averages(t_len, gset, correct_b, time_series, sigmahat, n, deriv_order);
            vals_cor = vals[Rcpp::Range(n, 2 * n - 1)];
            stat(i, j) = max(vals_cor);
            vals_cor_mat(_, l) = vals_cor;
            l++;
...
Rcpp::List compute_statistics(int t_len, Rcpp::NumericVector data, Rcpp::NumericVector gset,
                                 int n, double sigma, int deriv_order){
