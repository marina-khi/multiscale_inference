...
The results of the power simulations for the different heights of the peak (see Figure \ref{fig:lambda_fcts_height}) are presented in Table \ref{tab:sim:power:1}. The results of the power simulations for the different locations of the peak (see Figure \ref{fig:lambda_fcts_shift}) are presented in Table \ref{tab:sim:power:2}. As can be seen, our test is much more powerful when detecting shifts in the underlying trend curves than the difference in their intensity. This is what one would expect: In the second power simulation scenario, the mean functions are siginificantly different on wider interval (from $u = 0.2$ to $u = 0.6$), and even on the boundaries the difference is substantial, while in the first power simulation scenario, the main difference is located between $u = 0.2$ and $u = 0.4$.

As can also be seen from the tables, our test is more powerful with smaller number of time seires. This is not surprising, because the problem of making simultaneous confidence statements becomes much more difficult if the number of hypotheses under consideration increases substantially. For example, if we keep the  family of intervals $\intervals$ and the sample size $T$ constant, but only increase the number of time series by the factor of two, the number of pairwise comparisons (and hence the number of null hypotheses $H_0^{(i, j, k)}$) increases more than 4 times. With increase by the factor of $10$, we get the increase in comparisons more than $100$ times. However, even for $n = 50$ our test has almost $80\%$ power for $T = 500$ and $\alpha = 0.05$. 

In the second simulation scenario for sufficiently large sample sizes ($T = 250, 500$), the estimated power is always bigger than $0.9$ for all significance levels $\alpha$ and number of time series $n$. Note that in this setting for $T = 250, 500$ the power numbers get smaller as the significance level $\alpha$ gets larger. This reverse dependance can be explained by the way we calculate power: we count only those cases where we find that the first mean function is different from the others but we do not find any significant difference among all of the others. Hence, we suppose that in this setting the main driving force is the number of false rejections, i.e. the number of cases where the test incorrectly rejects $H_0^{(i, j, k)}$ for some $(i, j, k)$ with $i \neq 1$ and $j \neq 1$. Since this number is controlled by the significant level $\alpha$, it gets higher as the significance level $\alpha$ increases. And since we exclude these cases from calculating the power, the test become slightly less powerful.
