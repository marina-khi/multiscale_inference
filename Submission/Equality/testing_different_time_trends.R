testing_different_time_trends <- function(N_ts, y_data, month_column, alpha, kernel_method, sigmahat_vector_2){
  ...
  for (column in colnames(y_data)){
    ...
    smoothed_curve2 <- mapply(local_linear_smoothing, grid_points, MoreArgs = list(monthly_temp[[column]], grid_points, 0.1))
  ...
  axis(1, at = grid_points[seq(1, 380, by = 20)], labels = monthly_temp$date[seq(5, 385, by = 20)])
  ...
  filename = paste("Equality/distribution/distr_for_application_T_", T_data, "_and_N_ts_", N_ts, "_and_method_", kernel_method, ".RData", sep = "")
