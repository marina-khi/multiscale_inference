...
\item The global version aggregates the individual statistics $s_T(u,h)$ into the overall statistic $S_T = \max_{h \in H_T} S_T(h)$, where $S_T(h) = \max_{u \in U_T} |s_T(u,h)|$. The statistic $S_T$ is the counterpart to the multiscale statistic $\widehat{\Psi}_T$ in our approach. 
