...
In what follows, we investigate the finite sample performance of our multiscale test and compare it to the dependent SiZer methods from \cite{Rondonotti2004}, \cite{Rondonotti2007} and \cite{ParkHannigKang2009}. We consider the following versions of our multiscale test and SiZer:
...
\item[$\mathcal{T}_{\text{SiZer}}$:] the row-wise version of dependent SiZer as developed in \cite{Rondonotti2004}, \cite{Rondonotti2007} and \cite{ParkHannigKang2009}. We do not consider a global SiZer version since such a version was not introduced for the time series case in the aforementioned papers. 
...
To estimate the long-run error variance $\sigma^2$, we apply the procedure from Section \ref{sec-error-var} with $q=25$ and $\overline{r}=10$. As already discussed in Section \ref{sec-error-var}, $q=25$ should be an appropriate value for AR(1) errors that are not too strongly correlated, in particular, for $a_1 \in \{-0.5,-0.25,0.25,0.5\}$. When the errors are very strongly correlated, larger values of $q$ are required to produce precise estimates of $\sigma^2$. In the case of AR errors with $a_1 \in \{-0.9,0.9\}$, we thus set $q = 50$. The dependence of our long-run variance estimator on the tuning parameters $q$ and $\overline{r}$ is explored more systematically in Section \ref{subsec-sim-lrv}. 
To compute the critical values of the multiscale test $\mathcal{T}_{\text{MS}}$, we simulate $1000$ values of the statistic $\Phi_T$ defined in Section \ref{subsec-method-test} and compute their empirical $(1-\alpha)$ quantile $q_T(\alpha)$. The uncorrected and row-wise versions $\mathcal{T}_{\text{UC}}$ and $\mathcal{T}_{\text{RW}}$ of our multiscale test are implemented analogously. The SiZer test is implemented as described in \cite{ParkHannigKang2009}. The details are summarized in Section S.3 of the Supplementary Material. 

%The size and power numbers in the following tables and figures are calculated as follows: The actual size of the test $\mathcal{T}_j$, $j \in \{ \}$, is computed as the number of simulations in which the test rejects the null $H_0$ divided by the total number of simulations. 



\subsubsection{Size simulations} % of the multiscale test $\boldsymbol{\mathcal{T}_{\text{MS}}}$}


The first part of our simulation study investigates the properties of the four tests $\mathcal{T}_{\text{MS}}$, $\mathcal{T}_{\text{UC}}$, $\mathcal{T}_{\text{RW}}$ and $\mathcal{T}_{\text{SiZer}}$ under the null that the trend $m$ is constant. To start with, we focus on the multiscale test $\mathcal{T}_{\text{MS}}$ and explore its size properties under the null. Table \ref{tab:sim:size:MS1} presents the actual size for the AR parameters $a_1 \in \{-0.5,-0.25,0.25,0.5\}$, which is computed as the number of simulations in which $\mathcal{T}_{\text{MS}}$ rejects the null divided by the total number of simulations. %The long-run variance $\sigma^2$ needed to compute the test statistic of $\mathcal{T}_{\text{MS}}$ is estimated by the procedure from Section \ref{sec-error-var}, where we set $q = 25$ and $\overline{r} = 10$. As a robustness check, we have re-run the simulations for other choices of $q$ and $\overline{r}$, which yields very similar results. The dependence of the estimators of our long-run variance estimator on $q$ and $\overline{r}$ is further explored in Section \ref{subsec-sim-lrv}. 
As can be seen, the actual size of the multiscale test $\mathcal{T}_{\text{MS}}$ is fairly close to the nominal target $\alpha$ for all the considered AR parameters and sample sizes. Hence, the test has approximately the correct size.
...
In what follows, we distinguish between global and row-wise (or scale-wise) size: Global size is defined as the number of simulations in which the test rejects $H_0(u,h)$ for some $(u,h) \in \mathcal{G}_T^*$ divided by the total number of simulations. Hence, it is identical to the size as computed in Tables \ref{tab:sim:size:MS1} and \ref{tab:sim:size:MS2}. Row-wise size for scale $h^* \in H_T$, in contrast, is the number of simulations in which the test rejects $H_0(u,h^*)$ for some $(u,h^*) \in \mathcal{G}_T^*$. Hence, the size is calculated for each scale $h^*$ separately. Table \ref{tab:sim:size:compare} reports the global size of the four tests $\mathcal{T}_{\text{MS}}$, $\mathcal{T}_{\text{UC}}$, $\mathcal{T}_{\text{RW}}$ and $\mathcal{T}_{\text{SiZer}}$. As can be seen, the size of our multiscale test $\mathcal{T}_{\text{MS}}$ is fairly close to the target $\alpha=0.05$. The size numbers of the uncorrected version $\mathcal{T}_{\text{UC}}$ are reasonably close to $\alpha=0.05$ as well, even though they are a bit upward biased for $a_1=-0.5$ and downward biased for $a_1=0.5$. The global size of the row-wise methods $\mathcal{T}_{\text{RW}}$ and $\mathcal{T}_{\text{SiZer}}$, in contrast, is much larger than the target $\alpha=0.05$. Hence, as expected, the global tests $\mathcal{T}_{\text{MS}}$ and $\mathcal{T}_{\text{UC}}$ hold the size reasonably well, whereas the row-wise methods $\mathcal{T}_{\text{RW}}$ and $\mathcal{T}_{\text{SiZer}}$ are much too liberal. 


Figure \ref{fig:sim:size:compare} presents the row-wise size of the four tests by so-called parallel coordinate plots (cp.\ ?? and ??). Each curve in the figure specifies the row-wise size of one of the four tests as a function of $h$. 
...
As expected, the row-wise version $\mathcal{T}_{\text{RW}}$ of our multiscale test holds the size quite accurately across scales $h$. Only for the very small scales, the size is not kept very precisely, which is presumably due to the fact that the effective sample size (i.e.\ the number of data points in the kernel window) is very small, leading to imprecise results. The size of $\mathcal{T}_{\text{SiZer}}$ is also reasonably close to the target across scales, even though the size numbers are a bit upward biased. 
...
In the second part of our study, we compare the multiscale test $\mathcal{T}_{\text{MS}}$ with $\mathcal{T}_{\text{UC}}$, $\mathcal{T}_{\text{RW}}$ and $\mathcal{T}_{\text{SiZer}}$ in terms of power. We in particular investigate the ability of the tests to detect local increases in the trend $m$.\footnote{The same exercise could of course be carried out for decreases as well.} 
%As for the size comparisons, we restrict attention to the significance level $\alpha=0.05$ and the AR parameters $a_1 \in \{-0.5,0.5\}$. We investigate We have the following decision rules: 
%The tests are said to find an increase on the interval $[u-h,u+h]$ (correctly or mistakenly), if the test statistic for $H_0(u,h)$ is above the critical value. More precisely, 
The tests induce the following decision rules: For each $(u,h) \in \mathcal{G}_T^*$, 
\begin{center}
\begin{tabular}{rcl}
...
\end{tabular}
\end{center}
where $q_T^{\text{UC}}(\alpha)$, $q_T^{\text{RW}}(\alpha,h)$ and $q_T^{\text{SiZer}}(\alpha,h)$ are the critical values of $\mathcal{T}_{\text{UC}}$, $\mathcal{T}_{\text{RW}}$, $\mathcal{T}_{\text{SiZer}}$, respectively. Note that the critical values of $\mathcal{T}_{\text{RW}}$ and $\mathcal{T}_{\text{SiZer}}$ depend on the scale $h$ as these are row-wise procedures. 


\begin{table}[t!]
\centering
\footnotesize{
\caption{Global power comparison for the bump signal}\label{tab:sim:power:bump}
\newcolumntype{C}[1]{>{\hsize=#1\hsize\centering\arraybackslash}X}
\newcolumntype{Z}{>{\centering\arraybackslash}X}
...
\caption{Rowwise power comparison for bump signal.}\label{fig:sim:power:bump}
\end{figure}


We first consider a very simple trend function $m$ which allows us to make systematic power comparisons. The function is defined by $m(u) = 0.5 \cdot \ind(u \in [0.45,0.55]) \cdot (1 - \{\frac{(u-0.5}{0.05}\}^2)^2$ and is graphically illustrated in ??. As one can see, $m$ has a small bump around $u=0.5$ and is constant otherwise. More precisely, $m$ is increasing on $I^{+} = (0.45,0.5)$, decreasing on $I^{-} = (0.5,0.55)$ and constant elsewhere. We distinguish between the following types of power: 
\begin{enumerate}[label=(\roman*),leftmargin=0.85cm]
\item global power: the precentage of simulation runs in which the test indicates an increase on some interval $I_{u,h}=[u-h,u+h]$ where $m$ is indeed increasing, that is, on some $I_{u,h}$ with $I_{u,h} \cap I^+ \neq \emptyset$.
\item spurious global power: the percentage of simulation runs in which the test indicates an increase on some interval $I_{u,h}=[u-h,u+h]$ where $m$ is not increasing, that is, on some $I_{u,h}$ with $I_{u,h} \cap I^+ = \emptyset$.
\item row-wise power on scale $h^*$: the percentage of simulation runs in which the test indicates an increase on some interval $I_{u,h^*}=[u-h^*,u+h^*]$ where $m$ is indeed increasing, that is, on some $I_{u,h^*}$ with $I_{u,h^*} \cap I^+ \neq \emptyset$.
\item spurious row-wise power on scale $h^*$: the percentage of simulation runs in which the test indicates an increase on some interval $I_{u,h^*}=[u-h^*,u+h^*]$ where $m$ is not increasing, that is, on some $I_{u,h^*}$ with $I_{u,h^*} \cap I^+ = \emptyset$.
\end{enumerate}
In what follows, we investigate these types of power for the four test methods under consideration. As for the size comparisons, we restrict attention to the significance level $\alpha=0.05$ and the AR parameters $a_1 \in \{-0.5,0.5\}$. 


Table \ref{tab:sim:power:bump} reports global power and global spurious power for the four tests. As one can see, our multiscale test $\mathcal{T}_{\text{MS}}$ has higher power than the uncorrected version $\mathcal{T}_{\text{UC}}$. This confirms the theoretical optimality theory in \cite{DuembgenSpokoiny2001} (see also \cite{DuembgenWalther2008} and \cite{RufibachWalther2010}) according to which the aggregation scheme of the multiscale test $\mathcal{T}_{\text{MS}}$ should result in better power properties than the simpler scheme of $\mathcal{T}_{\text{UC}}$. As expected, the row-wise methods $\mathcal{T}_{\text{RW}}$ and $\mathcal{T}_{\text{SiZer}}$ have more power than the global tests. Indeed, $\mathcal{T}_{\text{SiZer}}$ is even a bit more powerful than $\mathcal{T}_{\text{RW}}$, which is presumably due to the fact that it is a bit too liberal under the null as observed in Figure \ref{fig:sim:size:compare}. The higher power of the row-wise procedures comes at some cost: Their spurious global power is much higher than that of the global tests. $\mathcal{T}_{\text{SiZer}}$ spuriously finds an increase in the tend $m$ in more than ??\% of the simulations, $\mathcal{T}_{\text{RW}}$ in more than ??\%. The multiscale test $\mathcal{T}_{\text{MS}}$ (and similarly its uncorrected version $\mathcal{T}_{\text{UC}}$) controls the probability of finding a spurious increase as indicated by Proposition \ref{prop-test-3}: its spurious global power is bounded by $100\cdot\alpha\% = 5\%$.  


Figure \ref{fig:sim:power:bump} gives a more detailed picture of the power properties of the four tests. The parallel coordinate plots specify the row-wise and spurious row-wise power of the tests, that is, they show how power is distributed across scales. Let us first have a look at the row-wise methods. As can be seen, $\mathcal{T}_{\text{SiZer}}$ is more powerful than $\mathcal{T}_{\text{RW}}$ on all scales under consideration. As already mentioned when discussing the global power results, this is presumably due to the fact that $\mathcal{T}_{\text{SiZer}}$ is somewhat too liberal under the null. Comparing the power curves of the two global methods gives an interesting insight: Our multiscale test $\mathcal{T}_{\text{MS}}$ has substantially more power than the uncorrected version $\mathcal{T}_{\text{UC}}$ on medium and large scales. On fine scales, in contrast, $\mathcal{T}_{\text{UC}}$ is slightly more powerful than $\mathcal{T}_{\text{MS}}$. This again illustrates the theoretical optimality theory in \cite{DuembgenSpokoiny2001}. According to this theory, the multiscale test $\mathcal{T}_{\text{MS}}$ should be as powerful as $\mathcal{T}_{\text{UC}}$ on small scales but more powerful on large scales. This is exactly what can be seen in the two panels of Figure \ref{fig:sim:power:bump}. 


The main take-aways from our simulation study are as follows: If one wants to have an exploratory data tool for finding local increases/decreases of a trend, the row-wise methods $\mathcal{T}_{\text{RW}}$ and $\mathcal{T}_{\text{SiZer}}$ both do a good job. However, if the main interest lies in rigorous statistical inference rather than mere data exploration, one needs to go with a global test method. Our simulation exercises have shown that our multiscale test $\mathcal{T}_{\text{MS}}$ is a global method which enjoys good size and power properties. In particular, as predicted by the theory, it is a more effective test than the uncorrected version $\mathcal{T}_{\text{UC}}$. 


%The main take-away lessons from our simulation exercises are as follows: If one wants to have an exploratory data tool for finding local increases/decreases of a trend, the row-wise methods do a good job. However, if one is interested not only in data exploration but ion rigorous statistical inference, one has to go with a global test method. Our simulation exercises have shown that our multiscale test $\mathcal{T}_{\text{MS}}$ is a global method which enjoys good size and power properties. In particular, as predicted by the optimal theory undelrying the aggregtion scheme of ??, it is a more effective test than the uncorrected version ??. Hence, it 


%the simulations have shown that the aggreatgtion scheme of ?? is superior to ??. In particular, it gives better power properties. Hence, our multiscale test offers a possiblity to make rigorous statistical inference about local increases / decreases of a trend, while enjoying the good power propetties as predicted by the optimality theory which underlies the aggregation scheme of ??. On the other hand, if one wants to have a data exploration tool and rigorosu statistical infenrece is not the goal, the row-wise methods are a more powerful alternative (which howwevr do not allow to make rigorous confidnece statemetns any more),. 

%TO summarize, the main findings of our simulation study are as follows. Wteher one wants to go for a global or a row-wise method depends on one's intention: If one wants to have a data exploration tool for finding localö increaers of a trend with as much power and one is not concerned about spurious findings, then the two row-wise methods ?ß and ?? are approrpiate. However, if one wants to make rigorous statistical inference. In particular, if one wants to make rigorous confidence statements about the increases found by the test, one has to go for the global methods. Moreover, as illustrated by the simulations, our multiscale method is a global test which gives as muc power as possiblöe due to the aggregatopn. 
...
%\newcolumntype{C}[1]{>{\hsize=#1\hsize\centering\arraybackslash}X}
%\newcolumntype{Z}{>{\centering\arraybackslash}X}
%\begin{tabular}{lll}%{\textwidth}{C{3} C{0.0001} ZZZZ C{0.0001} ZZZZ} 
%\toprule
%        & & \multicolumn{4}{c}{$a_1=-0.5$} & & \multicolumn{4}{c}{$a_1=0.5$} \\[0.1cm]
%        & & $\mathcal{T}_{\text{MS}}$ & $\mathcal{T}_{\text{UC}}$ & $\mathcal{T}_{\text{RW}}$ & $\mathcal{T}_{\text{SiZer}}$ & & $\mathcal{T}_{\text{MS}}$ & $\mathcal{T}_{\text{UC}}$ & $\mathcal{T}_{\text{RW}}$ & $\mathcal{T}_{\text{SiZer}}$ \\
%\cmidrule[0.4pt]{1-11}
%                          & test statistic & Rejects $H_0(u,h)$ if \\
%$\mathcal{T}_{\text{MS}}$   &  $\widehat{\Psi}_T = \max_{h \in H_T} \{ \widehat{\Psi}_T(h) - \lambda(h) \}$ & $|\widehat{\psi}_T(u,h) / \widehat{\sigma}| - \lambda(h) > q_T(\alpha)$. \\
%$\mathcal{T}_{\text{UC}}$   & $\widehat{\Psi}_{T,\text{uncorrected}} = \max_{h \in H_T} \widehat{\Psi}_T(h)$ & $|\widehat{\psi}_T(u,h) / \widehat{\sigma}| > q_T^{\text{UC}}(\alpha)$.  \\
%$\mathcal{T}_{\text{RW}}$   & $\widehat{\Psi}_T(h)$ for each $h \in H$ &  $|\widehat{\psi}_T(u,h) / \widehat{\sigma}| > q_T^{\text{RW}}(\alpha,h)$.  \\
%$\mathcal{T}_{\text{SiZer}}$ & $S_T(h)$ for each $h \in H$ & $|s_T(u,h)| > q_T^{\text{SiZer}}(\alpha,h)$.  \\
%\end{tabular}}
%\end{table}
