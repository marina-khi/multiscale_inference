...
\end{equation}
where $a_1,a_2,a_3,\ldots$ are unknown parameters and $\eta_t$ are i.i.d.\ innovations (which fulfill certain regularity conditions as detailed in the revised Section 4). 
%Notably, the error model ($*$) nests AR processes of any finite (unknown) order as a special case. 
As far as we can see, this is the most general class of error processes to which we can extend our methods: For our theory to work, we require the process $\{\varepsilon_t\}$ to have at least an AR($\infty$) representation because otherwise we do not get suitable Yule-Walker equations that we can exploit. 
...
%In the revised simulation study of Section 5.1, we consider a row-wise version $\mathcal{T}_{\text{RW}}$ of our multiscale test, which is the counterpart to row-wise dependent SiZer $\mathcal{T}_{\text{SiZer}}$ developed in \cite{Rondonotti2004}, \cite{Rondonotti2007} and \cite{ParkHannigKang2009}. A definition of the row-wise test $\mathcal{T}_{\text{RW}}$ is given on the top of p.21. The quantiles of $\mathcal{T}_{\text{RW}}$ under the null are approximated by our simulation-based procedure, whereas the quantiles of $\mathcal{T}_{\text{SiZer}}$ are approximated by the method of \cite{HannigMarron2006} and \cite{ParkHannigKang2009} which is based on extreme value theory. In our simulation study, we compare the size and power properties of $\mathcal{T}_{\text{RW}}$ and $\mathcal{T}_{\text{SiZer}}$. This gives some insights into how our simulation-based procedure compares to the approximation in \cite{HannigMarron2006} and \cite{ParkHannigKang2009}.
