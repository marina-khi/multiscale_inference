...
where $T$ is the sample size.
	...
	with $\Sigma$ being the kernel weighted covariance matrix of errors with generic element $$\sigma_{kl} = \gamma(|k - l|) \frac{1}{h} K\Big(\frac{k/T - u}{h}\Big) \frac{1}{h}  K\Big(\frac{l/T - u}{h}\Big),$$
	$W$ being the diagonal matrix $W = \Big\{diag\big(\frac{1}{h} K\big(\frac{i - t/T}{h}\big)\big)\Big\}$ and 
			\begin{equation}
	...
	\item For each of the generated $S = 1000$ samples calculate local linear estimators $\widehat{m}^\prime_h(u)$  
\end{enumerate}
\end{document}