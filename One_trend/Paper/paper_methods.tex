...
Under these conditions, a difference-based estimator of $\sigma^2$ can be obtained as follows: To start with, we construct an estimator of the short-run error variance $\gamma_\varepsilon(0) = \var(\varepsilon_0^2)$. As $m$ is Lipschitz continuous, it holds that $\Delta_L Y_{t,T} = \Delta_L \varepsilon_t + O(L/T)$. Hence, the differences $\Delta_L Y_{t,T}$ of the observed time series are close to the differences $\Delta_L \varepsilon_t$ of the unobserved error process as long as $L$ is not too large in comparison to $T$. Moreover, since $|\gamma_\varepsilon(L)| \le C \rho^{L}$, we have that $\ex[(\Delta_L \varepsilon_t)^2]/2 = \gamma_\varepsilon(0) - \gamma_\varepsilon(L) = \gamma_\varepsilon(0) + O(\rho^L)$. Taken together, these considerations yield that $\gamma_\varepsilon(0) = \ex[(\Delta_L Y_{t,T})^2]/2 + O(\{L/T\}^2 + \rho^L)$, which motivates to estimate $\gamma_\varepsilon(0)$ by  
\begin{equation}\label{est-g0-general}
\widehat{\gamma}_\varepsilon(0) = \frac{1}{2(T-L)} \sum\limits_{t=L+1}^T (\Delta_L Y_{t,T})^2,
\end{equation}
where we assume that $L = L_T \rightarrow \infty$ with $L_T/\log T \rightarrow \infty$ and $L_T/T \rightarrow 0$. Estimators of the autocovariances $\gamma_\varepsilon(\ell)$ for $\ell \ne 0$ can be derived by similar considerations. Since $\gamma_\varepsilon(\ell) = \gamma_\varepsilon(0) - \ex[(\Delta_\ell \varepsilon_t)^2]/2 = \gamma_\varepsilon(0) - \ex[(\Delta_\ell Y_{t,T})^2]/2 + O(\{\ell/T\}^2)$, we may in particular define  
...
Estimators of the long-run error variance $\sigma^2$ in model \eqref{model} have been developed for different kinds of error processes $\{\varepsilon_t\}$. A number of authors have analysed the case of MA($q$) or, more generally, $q$-dependent error terms. Difference-based estimators of $\sigma^2$ for this case were proposed in \cite{MuellerStadtmueller1988}, \cite{Herrmann1992} and \cite{Munk2017} among others. Under the assumption of $q$-dependence, $\gamma_\varepsilon(\ell) = 0$ for all $|\ell| > q$. Even though $q$-dependent time series are a reasonable error model in some applications, the condition that $\gamma_\varepsilon(\ell)$ is exactly equal to $0$ for sufficiently large lags $\ell$ is quite restrictive in many situations. Presumably the most widely used error model in practice is an AR($p$) process. Residual-based methods to estimate $\sigma^2$ in model \eqref{model} with AR($p$) errors can be found for example in \cite{Truong1991}, \cite{ShaoYang2011} and \cite{QiuShaoYang2013}. A difference-based method was proposed in \cite{Hall2003}. 
...
We first construct an estimator of the parameter vector $\boldsymbol{a} = (a_1,\ldots,a_p)^\top$. For any $q > p$, the ARMA($p,q$) process $\{ \Delta_q \varepsilon_t \}$ satisfies the Yule-Walker equations
\begin{align}
...
\end{equation}
where $\widehat{\boldsymbol{\Gamma}}_q$ and $\widehat{\boldsymbol{\gamma}}_q$ are defined analogously as $\boldsymbol{\Gamma}_q$ and $\boldsymbol{\gamma}_q$ with $\gamma_q(\ell)$ replaced by the sample autocovariances $\widehat{\gamma}_q(\ell) = (T-\ell-q)^{-1} \sum_{t=q+1}^{T-\ell} \Delta_q Y_{t,T} \Delta_q Y_{t+\ell,T}$ and $q = q_T$ goes to infinity sufficiently fast as $T \rightarrow \infty$, specifically, $q = q_T \rightarrow \infty$ with $q_T / \log T \rightarrow \infty$ and $q_T/T \rightarrow 0$. 
%More generally, we define the estimator
%\begin{equation}
%\widetilde{\boldsymbol{a}}_{\text{av}} = \frac{1}{\overline{q}-\underline{q}+1} \sum\limits_{q=\underline{q}}^{\overline{q}} \widetilde{\boldsymbol{a}}_q, 
%\end{equation}
%where $\underline{q} \le \overline{q}$ with $\underline{q}/\log T \rightarrow \infty$ and $\overline{q}/T \rightarrow 0$. We thus average the estimators $\widetilde{\boldsymbol{a}}_q$ for different values of $q$. Obviously, $\widetilde{\boldsymbol{a}}_q$ is a special case of $\widetilde{\boldsymbol{a}}_{\text{av}}$ with $\underline{q} = \overline{q} = q$. 
Once $\widetilde{\boldsymbol{a}}_q =(\widetilde{a}_{1,q},\ldots,\widetilde{a}_{p,q})^\top$ is computed, the long-run variance $\sigma^2$ can be estimated by 
\begin{equation} 
\widetilde{\sigma}^2_q = \frac{\widetilde{\nu}_q^2}{(1 - \sum_{j=1}^p \widetilde{a}_{j,q})^2}, 
\end{equation}
where $\widetilde{\nu}_q^2 = (2T)^{-1} \sum_{t=p+1}^T r_{t,T}^2$ with $r_{t,T} = \Delta_1 Y_{t,T} - \sum_{j=1}^p \widetilde{a}_{j,q} \Delta_1 Y_{t-j,T}$ is an estimator of the innovation variance $\nu^2$ and we make use of the fact that $\sigma^2 = \nu^2 / (1 - \sum_{j=1}^p a_j)^2$ for the AR($p$) process $\{\varepsilon_t\}$. 
...
(ii) $q$ should not be chosen too large in order to ensure that the trend $m$ is appropriately eliminated by taking $q$-th differences. As long as the trend $m$ is not very pronounced, the two requirements (i) and (ii) can be fulfilled without much difficulty. For example, by choosing $q = 20$ in the AR($1$) case just discussed, we do not only take care of (i) but also make sure that moderate trends $m$ are differenced out appropriately. This is illustrated by the simulations in Section \ref{subsec-sim-3}, which show that the estimator $\widetilde{\boldsymbol{a}}_q$ is rather insensitive to the choice of $q$ as long as the trend $m$ is not too strong. 


When the trend $m$ is very pronounced, in contrast, even moderate values of $q$ may be too large to eliminate the trend $m$ appropriately. As a result, the estimator $\widetilde{\boldsymbol{a}}_q$ will have a strong bias. In order to reduce this bias, we refine our estimation procedure as follows: By solving the recursive equations \eqref{c-recursion} with $\boldsymbol{a}$ replaced by $\widetilde{\boldsymbol{a}}_q$, we can compute estimators $\widetilde{c}_j$ of the coefficients $c_j$ and thus estimators $\widetilde{\boldsymbol{c}}_r$ of the vectors $\boldsymbol{c}_r$ for any $r > p$. Plugging the estimators $\widehat{\boldsymbol{\Gamma}}_r$, $\widehat{\boldsymbol{\gamma}}_r$, $\widetilde{\boldsymbol{c}}_r$ and $\widetilde{\nu}^2$ into \eqref{YW-eq}, we can estimate $\boldsymbol{a}$ by 
