#This is the main file for producing the simulation results for power, that are reported in Section 5.1.2.
rm(list=ls())
...
for (a1 in different_a1){
  ...
  result_spurious <- CalculatePower(T, a1, sigma_eta, alpha, Nsim = Nsim, SimRuns =SimRuns,
                                      sigma.type = 'true', remove.small.ess = TRUE,
                                      sim.design = 'bump', bump.height = height,
                                      region = 'increase', power.type = 'spurious')
  ...
  title(main = "Rowwise power", line = -1.4, cex.main = 0.9)
  ...
  title(main = "Rowwise spurious power", line = -1.4, cex.main = 0.9)
