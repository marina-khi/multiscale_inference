...
par(cex = 1, tck = -0.025)
par(mar = c(0.5, 2.5, 2.5, 0)) #Margins for each plot

plot(BIC_vec, type = "l")
title(main = expression(BIC==-T~Sigma[i==1]^n~log(sigma^2~(K))-log(nT)(K(n+T)+n)), line = 1)
...
for (n_cl in 10:max_n_cl){
  ...
  BIC <- - n_ts * t_len * log(sum(sigma_hat_vec)/(n_ts * t_len)) - n_cl * n_ts * t_len * (2 * bw_abs / t_len) * log(n_ts * t_len)
  BIC_vec_2 <- c(BIC_vec_2, BIC)
}

plot(BIC_vec_2, type = "l")
title(main = expression(BIC==-nT~log(sigma^2)-Delta[K]~log(nT)), line = 1)


#title(main = expression(BIC==-nT~log(sigma^2~(K))), line = 1)
